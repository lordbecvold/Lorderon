...
static const char * sc_apszPythonLibraryFilenames[] = { "UserDict.pyc", "__future__.pyc", "copy_reg.pyc", "linecache.pyc", "ntpath.pyc", "os.pyc", "site.pyc", "stat.pyc", "string.pyc", "traceback.pyc", "types.pyc", "\n",};
...
const std::string& ApplicationStringTable_GetString(DWORD dwID, LPCSTR szKey)
{
	...
	strcat(szIniFileName, "lorderon.dat");
...
HANDLE CreateMetin2GameMutex()
{
	...
	return CreateMutex(&sa, FALSE, "GameMutex");
}
...
void __PrintTimeStamp()
{
	...
	if (__IS_TEST_SERVER_MODE__)
		LogBoxf("LORDERON BINARY TEST DEBUG VERSION %s  ( MS C++ %d Compiled )", __TIMESTAMP__, _MSC_VER);
	else
		LogBoxf("LORDERON BINARY DEBUG VERSION %s ( MS C++ %d Compiled )", __TIMESTAMP__, _MSC_VER);
#else
	if (__IS_TEST_SERVER_MODE__)
		LogBoxf("LORDERON BINARY TEST VERSION %s  ( MS C++ %d Compiled )", __TIMESTAMP__, _MSC_VER);
	else
		LogBoxf("LORDERON BINARY DISTRIBUTE VERSION %s ( MS C++ %d Compiled )", __TIMESTAMP__, _MSC_VER);
#endif
}
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	for( int i=0; i < nArgc; i++ ) {
		...
		if (__IsLocaleVersion(szArgv[i])) 
		{
			...
			if (fp)
			{
				extern int LORDERON_GET_VERSION();
				fprintf(fp, "r%d\n", LORDERON_GET_VERSION());
				fclose(fp);
			}
			bQuit = true;
...
static void GrannyError(granny_log_message_type Type, granny_log_message_origin Origin, granny_int32x Line, char const *Error, void *UserData)
{

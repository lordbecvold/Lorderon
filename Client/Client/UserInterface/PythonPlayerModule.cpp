...
public:
	...
	{
		static TGradeUnit beltGradeByLevelTable[] = {0,	1, 1, 2, 2, 3, 4, 5, 6, 7};
		return beltGradeByLevelTable[refineLevel];
	}

	static const TGradeUnit* GetAvailableRuleTableByGrade()
	{
		static TGradeUnit availableRuleByGrade[c_Belt_Inventory_Slot_Count] = {1, 2, 4, 6, 3, 3, 4, 6, 5, 5, 5, 6, 7, 7, 7, 7};
...
PyObject * playerGetPartyMemberAffects(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (!CPythonPlayer::Instance().GetPartyMemberPtr(iPID, &pPartyMemberInfo))
		return Py_BuildValue("i", FALSE);
	return Py_BuildValue("iiiiiii",	pPartyMemberInfo->sAffects[0], pPartyMemberInfo->sAffects[1], pPartyMemberInfo->sAffects[2], pPartyMemberInfo->sAffects[3], pPartyMemberInfo->sAffects[4], pPartyMemberInfo->sAffects[5], pPartyMemberInfo->sAffects[6]);
}
...
PyObject * playerOpenWhisperBySearchName(PyObject* poSelf, PyObject* poArgs)
{
	char* c_szName;
	if (!PyTuple_GetString(poArgs, 0, &c_szName))
		return Py_BuildException();
	CPythonPlayer::Instance().OpenWhisperBySearchName(c_szName);
	return Py_BuildNone();
}
#endif
...
#if defined(ENABLE_RENEW_MESSENGER_WHISPER)
		{ "OpenWhisperBySearchName", playerOpenWhisperBySearchName, METH_VARARGS },
#endif

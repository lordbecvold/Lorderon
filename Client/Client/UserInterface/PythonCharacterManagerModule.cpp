...
PyObject * chrmgrGetVIDInfo(PyObject* poSelf, PyObject* poArgs)
{
	...
	char szInfo[1024];
	sprintf(szInfo, "VID %d (isRegistered=%d, isAlive=%d, isDead=%d) %s", nVID, rkChrMgr.IsRegisteredVID(nVID), rkChrMgr.IsAliveVID(nVID), rkChrMgr.IsDeadVID(nVID), szDetail);
...
void initchrmgr()
{
	...
	PyModule_AddIntConstant(poModule, "NAMECOLOR_METIN", CInstanceBase::NAMECOLOR_﻿METI﻿N﻿﻿);

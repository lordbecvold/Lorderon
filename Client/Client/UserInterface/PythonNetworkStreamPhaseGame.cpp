...
#include "PythonWhisper.h"
	...
	if (CPythonChat::WHISPER_TYPE_CHAT == whisperPacket.bType || CPythonChat::WHISPER_TYPE_GM == whisperPacket.bType)
	{
		...
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnRecvWhisper", Py_BuildValue("(issiiisssssss)", (int)whisperPacket.bType, whisperPacket.szNameFrom, line, (int)whisperPacket.job, (int)whisperPacket.level, (int)whisperPacket.empire, whisperPacket.guild, whisperPacket.language, whisperPacket.status, whisperPacket.location, whisperPacket.year, whisperPacket.month, whisperPacket.day));
	}
...
bool CPythonNetworkStream::RecvRefineInformationPacket()
{
	...
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OpenRefineDialog", Py_BuildValue("(iiii)", kRefineInfoPacket.pos, kRefineInfoPacket.refine_table.result_vnum, rkRefineTable.cost, rkRefineTable.prob));
...
bool CPythonNetworkStream::RecvRefineInformationPacketNew()
{
	...
	TRefineTable & rkRefineTable = kRefineInfoPacket.refine_table;
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OpenRefineDialog", Py_BuildValue("(iiiii)", kRefineInfoPacket.pos, kRefineInfoPacket.refine_table.result_vnum, rkRefineTable.cost, rkRefineTable.prob, kRefineInfoPacket.type));
	for (int i = 0; i < rkRefineTable.material_count; ++i)
	{
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "AppendMaterialToRefineDialog", Py_BuildValue("(ii)", rkRefineTable.materials[i].vnum, rkRefineTable.materials[i].count));
	}
...
bool CPythonNetworkStream::SendContactProfileManager(const char* szTargetName, BYTE state, const char* szText)
{
	TPacketCGContactProfileManager pack;
	pack.header = HEADER_CG_CONTACT_PROFILE_MANAGER;
	strncpy(pack.szTargetName, szTargetName, sizeof(pack.szTargetName));
	pack.state = state;
	strncpy(pack.szText, szText, sizeof(pack.szText));
	if (!Send(sizeof(pack), &pack))
		return false;
	return SendSequence();
}

bool CPythonNetworkStream::RecvContactProfileLoad()
{
	TPacketGCContactProfileLoad pack;
	if (!Recv(sizeof(pack), &pack))
		return false;
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_Profile_OnRecvData", Py_BuildValue("(siiisssssss)", pack.name, (int)pack.job, (int)pack.level, (int)pack.empire, pack.guild, pack.language, pack.status, pack.location, pack.year, pack.month, pack.day));
	return true;
}

...
enum
{
	...
	HEADER_CG_CONTACT_PROFILE_MANAGER 			= 117,
#endif
...
#endif
#if defined(ENABLE_RENEW_MESSENGER_WHISPER)
	HEADER_GC_CONTACT_PROFILE_LOAD = 139,
#endif
	...
	HEADER_GC_MAIN_CHARACTER4_BGM_VOL			= 138,
#ifdef ENABLE_TICKET_SYSTEM
	HEADER_GC_TICKET_LOGS 						= 139,
	HEADER_GC_TICKET_LOGS_REPLY					= 140,
#endif
...
#if defined(ENABLE_RENEW_MESSENGER_WHISPER)
typedef struct SPacketCGContactProfileManager
{
	BYTE	header;
	char	szTargetName[12 + 1];
	BYTE	state;
	char	szText[128 + 1];
} TPacketCGContactProfileManager;

typedef struct SPacketGCContactProfileLoad
{
	BYTE	header;
	char	name[12 + 1];
	DWORD	job;

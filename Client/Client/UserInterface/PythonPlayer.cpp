...
void CPythonPlayer::OpenWhisperBySearchName(const char * c_szName)
{
	PyCallClassMemberFunc(m_ppyGameWindow, "BINARY_OpenWhisperBySearchName", Py_BuildValue("(s)", c_szName));
}
#endif
...
void CPythonPlayer::__Update_NotifyGuildAreaEvent()
{
	...
	if (pkInstMain)
	{
		...
		DWORD dwAreaID = CPythonMiniMap::Instance().GetGuildAreaID(ULONG(kPixelPosition.x), ULONG(kPixelPosition.y));

...
public:
	...
	void Initialize()
	{
		...
		{
			m_bComInit = true;
			hr = CoCreateInstance( CLSID_TF_ThreadMgr, NULL, CLSCTX_INPROC_SERVER, __uuidof(ITfThreadMgr), (void**)&m_ptim );
		}
	}
	void Uninitialize()
	{
...
bool CIME::GetReadingWindowOrientation()
{
    ...
    if(!bHorizontalReading && (GETLANG() == LANG_CHT))
    {
        ...
        if (lRc == ERROR_SUCCESS)
        {
            ...
            if (lRc == ERROR_SUCCESS)
            {
                if ((dwVer <= MAKEIMEVERSION(5, 0) && ((BYTE)dwMapping == 0x22 || (BYTE)dwMapping == 0x23)) || ((dwVer == MAKEIMEVERSION(5, 1) || dwVer == MAKEIMEVERSION(5, 2)) && (BYTE)dwMapping >= 0x22 && (BYTE)dwMapping <= 0x24))
                {
...
STDAPI CTsfUiLessMode::CUIElementSink::OnActivated(DWORD dwProfileType, LANGID langid, REFCLSID clsid, REFGUID catid, REFGUID guidProfile, HKL hkl, DWORD dwFlags)
{
...
void CTsfUiLessMode::MakeCandidateStrings(ITfCandidateListUIElement* pcandidate)
{
	...
	if(uPageCnt > 0)
	{
		...
		if(IndexList)
		{
			...
			dwPageSize = (uCurrentPage < uPageCnt-1) ? min(uCount, IndexList[uCurrentPage+1]) - dwPageStart: uCount - dwPageStart;
		}
	}

...
void CFlare::Draw(float fBrightScale, int nWidth, int nHeight, int nX, int nY)
{
	...
	for (unsigned int i = 0; i < m_vFlares.size(); i++)
	{
		...
		D3DXCOLOR d3dColor(m_vFlares[i]->m_pColor[0] * fBrightScale, m_vFlares[i]->m_pColor[1] * fBrightScale, m_vFlares[i]->m_pColor[2] * fBrightScale, m_vFlares[i]->m_pColor[3] * fBrightScale);

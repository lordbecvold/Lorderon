...
class DragonSoulWindow(ui.ScriptWindow):
	...
	def UseItemFastSlotTest(self, slot):
		if self.inventoryPageIndex >= 0 and self.inventoryPageIndex <= 2:
			return self.__InventoryLocalSlotPosToGlobalSlotPos(player.DRAGON_SOUL_INVENTORY, slot)
		else:
			chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.SIMYA_REFINE_UYARI)
			return -1
...
class DragonSoulRefineWindow(ui.ScriptWindow):
	...
	DS_SUB_HEADER_DIC = {REFINE_TYPE_GRADE : player.DS_SUB_HEADER_DO_UPGRADE, REFINE_TYPE_STEP : player.DS_SUB_HEADER_DO_IMPROVEMENT, REFINE_TYPE_STRENGTH : player.DS_SUB_HEADER_DO_REFINE}
	REFINE_STONE_SLOT, DRAGON_SOUL_SLOT = xrange(2)
	INVALID_DRAGON_SOUL_INFO = -1

	def __init__(self):
		self.otomatikRefineSayi = 0
		self.otomatikRefineZaman = 0.0
		ui.ScriptWindow.__init__(self)
		self.tooltipItem = None
		self.sellingSlotNumber = -1
		self.isLoaded = 0
	...
	def __LoadWindow(self):
		...
		try:
			...
			self.checkBox.SetPosition(18,205)
			self.checkBox.SetEvent(ui.__mem_func__(self.otoRefineCheck), "ON_CHECK", True)
			self.checkBox.SetEvent(ui.__mem_func__(self.otoRefineCheck), "ON_UNCKECK", False)
			self.checkBox.SetCheckStatus(constInfo.IS_OTOSIMYA)
			self.checkBox.SetTextInfo(localeInfo.SIMYA_OTO_YAZISI)
			self.checkBox.Show()
		except:
			import exception
	...
	def otoRefineCheck(self, checkType, autoFlag):
			constInfo.IS_OTOSIMYA = autoFlag

	def otomatikRefineIptal(self):
		if constInfo.IS_OTOSIMYA == True:
			constInfo.IS_OTOSIMYA = False

	def OnUpdate(self):
		if constInfo.IS_OTOSIMYA == True:
			if not self.IsShow():
				self.otomatikRefineIptal()
				return
			if self.otomatikRefineZaman + 0.8 > float(app.GetTime()):
				return
			...
			for i in xrange(player.DRAGON_SOUL_PAGE_SIZE):
				...
				if itemVnum > 0:
					...
					if count == 5:
						...
						count = 0
			self.otomatikRefineSayi += 1
			self.otomatikRefineZaman = float(app.GetTime())

...
class Console(object):
	def __init__(self, output):
		...
		self.collision = 0
		self.bgPartDict = {"terrain":background.PART_TERRAIN,"object":background.PART_OBJECT,"cloud":background.PART_CLOUD,"tree":background.PART_TREE,"water":background.PART_WATER,"sky":background.PART_SKY,}
		self.bgSortDict = {"distance":background.DISTANCE_SORT,"texture":background.TEXTURE_SORT,}
		self.game = 0
		...
		if self.functionDict.has_key(sCmd):
			...
			if sCmd == 'help' or sCmd == 'h':
				try:
					...
					argcount = v.im_func.func_code.co_argcount - 1

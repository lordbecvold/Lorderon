...
class WindowEquipmentBlock(ui.BoardWithTitleBar):
	def __init__(self):
		...
		self.SetTitleName(translate.DUEL_TITLENAME)
		self.SetCloseEvent(ui.__mem_func__(self.Close))
		self.SetCenterPosition()
		self.AddFlag('movable')
		self.SetTop()
		self.btns = []
		self.Add(translate.DUEL_EQUIP_BLOCK, self.Enable)
		self.Add(translate.DUEL_EQUIP_UNBLOCK, self.Disable)	
...
class WindowLiveInformations(ui.ScriptWindow):
	...
	def __CreateDialog(self):
		...
		color = "|cFFff5148"
		self.checkBoxTable	=	{
									0	:	[uiPvP.CheckBox(self, box_x, y+65, lambda arg = 0: self.SetType(arg)), 0, self.SetText(text_x, y+65, color + translate.DUEL_BLOCK_CHANGE_ITEM)],
									1	:	[uiPvP.CheckBox(self, box_x, y+65+20, lambda arg = 1: self.SetType(arg)), 0, self.SetText(text_x, y+65+20, color + translate.DUEL_BLOCK_BUFF)],
									2	:	[uiPvP.CheckBox(self, box_x, y+65+20+20, lambda arg = 2: self.SetType(arg)), 0, self.SetText(text_x, y+65+20+20, color + translate.DUEL_BLOCK_POTION_RED)],
									3	:	[uiPvP.CheckBox(self, box_x, y+65+20+20+20, lambda arg = 3: self.SetType(arg)), 0, self.SetText(text_x, y+65+20+20+20, color + translate.DUEL_BLOCK_HORSE_MOUNT)],
									4	:	[uiPvP.CheckBox(self, box_x+155, y+65, lambda arg = 4: self.SetType(arg)), 0, self.SetText(text_x+155, y+65, color + translate.DUEL_BLOCK_PET)],
									5	:	[uiPvP.CheckBox(self, box_x+155, y+65+20, lambda arg = 5: self.SetType(arg)), 0, self.SetText(text_x+155, y+65+20, color + translate.DUEL_BLOCK_POLY)],
									6	:	[uiPvP.CheckBox(self, box_x+155, y+65+20+20, lambda arg = 6: self.SetType(arg)), 0, self.SetText(text_x+155, y+65+20+20, color + translate.DUEL_BLOCK_PARTY)],
									7	:	[uiPvP.CheckBox(self, box_x+155, y+65+20+20+20, lambda arg = 7: self.SetType(arg)), 0, self.SetText(text_x+155, y+65+20+20+20, color + translate.DUEL_BLOCK_EXCHANGE)]
								}
	...
	def ShowInformations(self, value):	
		...
		self.MoneyText.SetText(translate.NumberToMoneyString(str(value[11])))
		self.Show()
...
class Initializate(ui.ScriptWindow):
	...
	def __CreateDialog(self):
		...
		self.checkBoxTable	=	{
									0	:	[uiPvP.CheckBox(self, 490, 50, lambda arg = 0: self.SetType(arg)), 0, self.SetText(377, 50, translate.DUEL_BLOCK_CHANGE_ITEM)],
									1	:	[uiPvP.CheckBox(self, 490, 50+25, lambda arg = 1: self.SetType(arg)), 0, self.SetText(377, 50+25, translate.DUEL_BLOCK_BUFF)],
									2	:	[uiPvP.CheckBox(self, 490, 50+25+25, lambda arg = 2: self.SetType(arg)), 0, self.SetText(377, 50+25+25, translate.DUEL_BLOCK_POTION_RED)],
									3	:	[uiPvP.CheckBox(self, 490, 50+25+25+25, lambda arg = 3: self.SetType(arg)), 0, self.SetText(377, 50+25+25+25, translate.DUEL_BLOCK_HORSE_MOUNT)],
									4	:	[uiPvP.CheckBox(self, 490, 50+25+25+25+25, lambda arg = 4: self.SetType(arg)), 0, self.SetText(377, 50+25+25+25+25, translate.DUEL_BLOCK_PET)],
									5	:	[uiPvP.CheckBox(self, 490, 50+25+25+25+25+25, lambda arg = 5: self.SetType(arg)), 0, self.SetText(377, 50+25+25+25+25+25, translate.DUEL_BLOCK_POLY)],
									6	:	[uiPvP.CheckBox(self, 490, 50+25+25+25+25+25+25, lambda arg = 6: self.SetType(arg)), 0, self.SetText(377, 50+25+25+25+25+25+25, translate.DUEL_BLOCK_PARTY)],
									7	:	[uiPvP.CheckBox(self, 490, 50+25+25+25+25+25+25+25, lambda arg = 7: self.SetType(arg)), 0, self.SetText(377, 50+25+25+25+25+25+25+25, translate.DUEL_BLOCK_EXCHANGE)]
								}
	...
	def OpenDialog(self, VID, Name, Guild, Level, Race, PlayTime, HP, SP):
		...
		self.HP  = str(HP)
		self.SP  = str(SP)
		self.SetTitle(translate.DUEL_TARGET_NAME + Name)
		self.name.SetText(Name)
		self.guild.SetText(self.Guild)
		self.level.SetText(self.Level)
		self.playtime.SetText(translate.FormatTime(self.PlayTime))
	...
	def Selected(self, value):
		...
		self.MoneyText.SetText(translate.NumberToMoneyString(str(value[8])))
		global mCheck
		mCheck = 1
	...
	def SetType(self, arg):
		global mCheck
		if mCheck == 1:
			chat.AppendChat(chat.CHAT_TYPE_INFO, translate.DUEL_VICTIM_ERROR)
			return
		else:
	...
	def GetSlotMoney(self):
		...
		if mCheck == 1:
			...
			self.inputDialog.SetAcceptText(translate.DUEL_MONEY_ADD)
			self.inputDialog.SetCancelText(translate.DUEL_MONEY_CANCEL)
			self.inputDialog.SetTitle(translate.DUEL_MONEY_TITLE)
			self.inputDialog.SetDescription(translate.DUEL_MONEY_CURRENT + translate.NumberToMoneyString(str(ch.GetElk())))
	...
	def SetMoney(self):
		...
		if not updateMoney.isdigit():
			chat.AppendChat(chat.CHAT_TYPE_INFO, translate.DUEL_MONEY_ERROR_1)
			return True
		if int(updateMoney) <= 0:
			chat.AppendChat(chat.CHAT_TYPE_INFO, translate.DUEL_MONEY_ERROR_2)
			return True
		...
		if updateMoney.isdigit() and mCheck != 1:
			if int(updateMoney) <= int(currentMoney):
				self.MoneyText.SetText(translate.NumberToMoneyString(str(updateMoney)))
				BINARY_DUEL_GET_["amount"][0] = (int(updateMoney))
				self.inputDialog.Hide()
			else:
				chat.AppendChat(chat.CHAT_TYPE_INFO, translate.DUEL_MONEY_ERROR_3)
				return True

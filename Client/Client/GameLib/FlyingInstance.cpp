...
{
	D3DXQUATERNION q;
	D3DXQuaternionRotationYawPitchRoll(&q, D3DXToRadian(m_pData->m_v3AngVel.y)*CTimer::Instance().GetElapsedSecond(), D3DXToRadian(m_pData->m_v3AngVel.x)*CTimer::Instance().GetElapsedSecond(), D3DXToRadian(m_pData->m_v3AngVel.z)*CTimer::Instance().GetElapsedSecond());
			...
			case CFlyingData::FLY_ATTACH_TYPE_SINE:
				{
					...
					D3DXVECTOR3 p(-sinf(D3DXToRadian(rfad.fRoll))*rfad.fAmplitude*sinf(angle), 0.0f, -cosf(D3DXToRadian(rfad.fRoll))*rfad.fAmplitude*sinf(angle));
					Vec3TransformQuaternionSafe(&p,&p,&q);
					p+=m_v3Position;
					D3DXMatrixRotationQuaternion(&m,&q);
					m._41=p.x;
			...
			case CFlyingData::FLY_ATTACH_TYPE_EXP:
				{
					...
					D3DXVECTOR3 p(-sinf(D3DXToRadian(rfad.fRoll))*rfad.fAmplitude*exp(-angle)*angle, 0.0f, -cosf(D3DXToRadian(rfad.fRoll))*rfad.fAmplitude*exp(-angle)*angle);
					Vec3TransformQuaternionSafe(&p,&p,&q);
					p+=m_v3Position;
					D3DXMatrixRotationQuaternion(&m,&q);
					m._41=p.x;

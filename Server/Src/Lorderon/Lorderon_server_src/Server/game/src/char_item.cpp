...
bool CHARACTER::DoRefine(LPITEM item, bool bMoneyOnly)
{
	...
	if (RefineChance > 0)
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("refine_weapon_free_lvl_20"));
			return false;
		}
		cost = 0;
	...
	if (result_vnum == 0)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("role_refine_availible"));
		return false;
	}
	...
	if (!pProto)
	{
		...
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_time_is_not_refinable"));
		return false;
	}
	if (GetGold() < cost)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_have_gold_refine"));
		return false;
	}
	if (!bMoneyOnly && !RefineChance)
	{
		for (int i = 0; i < prt->material_count; ++i)
		{
			if (CountSpecifyItem(prt->materials[i].vnum) < prt->materials[i].count)
			{
				...
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_have_refine_items"));
				return false;
			}
		}
		for (int i = 0; i < prt->material_count; ++i)
...
#endif
	{
		...
		if (pkNewItem)
		{
			...
			{		
				...
				snprintf(szUpgradeAnnouncement, sizeof(szUpgradeAnnouncement), "<Server>[%s] kovnul [%s]", GetName(), pkNewItem->GetName()); 
...
bool CHARACTER::DoRefineWithScroll(LPITEM item)
{
	...
	else if (pkItemScroll->GetValue(0) == MEMO_SCROLL)
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_have_role_1"));
			return false;
		}
	}
	else if (pkItemScroll->GetValue(0) == BDRAGON_SCROLL)
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_use_this_item_0"));
			return false;
		}
	}
	TItemTable * pProto = ITEM_MANAGER::instance().GetTable(item->GetRefinedVnum());
	...
	if (pkItemScroll->GetValue(0) == HYUNIRON_CHN || pkItemScroll->GetValue(0) == YONGSIN_SCROLL || pkItemScroll->GetValue(0) == YAGONG_SCROLL) 
	{
		...
		if (test_server)
		{
			ChatPacket(CHAT_TYPE_INFO, "[TEST-SERVER-INFO] Success_Prob %d, RefineLevel %d ", success_prob, item->GetRefineLevel());
		}
...
bool CHARACTER::RefineInformation(BYTE bCell, BYTE bType, int iAdditionalCell)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("refine_items_tower_msg"));
		return false;
	}
	...
	if (item->GetType() == ITEM_USE && item->GetSubType() == USE_TUNING)
	{
		if (bType == 0)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_time_is_not_refinable_1"));
			return false;
		}
		else
		{
			...
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_combine_items_1"));
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("refine_msg_1"));
				return false;
			}
		}
	}
...
bool CHARACTER::RefineItem(LPITEM pkItem, LPITEM pkTarget)
{
	...
	else if (pkItem->GetSubType() == USE_DETACHMENT && IS_SET(pkTarget->GetFlag(), ITEM_FLAG_REFINEABLE))
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("stone_dont_delete"));
			return false;
		}
	}
	return false;
...
bool CHARACTER::GiveRecallItem(LPITEM item)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_secure_place"));
		return false;
	}
	int pos;
	if (item->GetCount() == 1)	
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_inventory_is_full"));
		return false;
	}
	return true;
}
...
void CHARACTER::ProcessRecallItem(LPITEM item)
{
	...
	switch (idx)
	{
		...
		case 304:
			if( GetLevel() < 90 )
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("your_level_is_low_2"));
				return;
			}
			else
				break;
...
void CHARACTER::__OpenPrivateShop()
{
	...
	switch (bodyPart)
	{
		...
		case 2:
			...
			break;
		default:
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("char_item_msg_1"));
			break;
	}
#endif
}
...
int CalculateConsume(LPCHARACTER ch)
{
	...
	int consumeLife = 0;
	{
		...
		if (curLife < needLife)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_have_low_hp"));
			return -1;
		}
...
int CalculateConsumeSP(LPCHARACTER lpChar)
{
	...
	if (curSP < needSP)
	{
		lpChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_have_low_magic_points"));
		return -1;
	}
	return needSP;
}
...
bool CHARACTER::UseItemEx(LPITEM item, TItemPos DestCell)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_use_this_in_duel"));
		return false;
	}
#ifdef ENABLE_NEWSTUFF
		...
		case ITEM_CAMPFIRE:
			{
				...
				if (!tree)
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_use_fire"));
					return false;
				}
				if (tree->IsAttr((long)(GetX()+fx), (long)(GetY()+fy), ATTR_WATER))
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_place_fire_in_wather"));
					return false;
				}
								...
								switch (item->GetVnum())
								{
									case 71049: 
										if (g_bEnableBootaryCheck)
										{
											...
											{
												ChatPacket(CHAT_TYPE_INFO, LC_TEXT("cant_open_safebox"));
											}
										}
										else
										{
		...
		case ITEM_TREASURE_KEY:
			{
				...
				{
					ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("broken_key"));
					return false;
				}
				if (item->GetValue(0) == item2->GetValue(0))
				{
						...
						for (int i = 0; i < count; i++){
							switch (dwVnums[i])
							{
								case CSpecialItemGroup::GOLD:
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you %d get_yang"), dwCounts[i]);
									break;
								case CSpecialItemGroup::EXP:
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_get_exp"));
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%d get_exp_1"), dwCounts[i]);
									break;
								case CSpecialItemGroup::MOB:
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("warn_you_1"));
									break;
								case CSpecialItemGroup::SLOW:
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("smoke_text_regen"));
									break;
								case CSpecialItemGroup::DRAIN_HP:
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("duel_started"));
									break;
								case CSpecialItemGroup::POISON:
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("green_smoke_txt"));
									break;
								...
								default:
									...
									{
										if (dwCounts[i] > 1)
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("text_msg_char_item"), item_gets[i]->GetName(), dwCounts[i]);
										else
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_get_new_item"), item_gets[i]->GetName());
									}
							}
						}
					}
					...
					{
						ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("skill_chart_text_13"));
						return false;
					}
				}
				else
		...
		case ITEM_GIFTBOX:
			{
				...
				if (0 != g_BoxUseTimeLimitValue)
				{
					...
					{
						ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ENABLE_NEWSTUFF char_item"));
						return false;
					}
				}
				...
				if( dwBoxVnum > 51500 && dwBoxVnum < 52000 )	
				{
					...
					{
						ChatPacket(CHAT_TYPE_INFO,LC_TEXT("you_must_have_complete_alch_quest"));
						return false;
					}
				}
				if (GiveItemFromSpecialItemGroup(dwBoxVnum, dwVnums, dwCounts, item_gets, count))
				{
					...
					for (int i = 0; i < count; i++){
						...
						case CSpecialItemGroup::BLEEDING:
							ChatPacket(CHAT_TYPE_INFO, LC_TEXT("wolfman_shit_char_item"));
							break;
#endif
						case CSpecialItemGroup::MOB_GROUP:
							ChatPacket(CHAT_TYPE_INFO, LC_TEXT("warn_you_1"));
		...
		case ITEM_SKILLFORGET:
			{
				...
				{
					ITEM_MANAGER::instance().RemoveItem(item);
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("points_restarted"));
				}
				else
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("points_dont_reset"));
			}
			break;
		case ITEM_SKILLBOOK:
			{
				if (IsPolymorphed())
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_no_mobility"));
					return false;
				}
				DWORD dwVnum = 0;
				...
				if (item->GetVnum() > 50800 && item->GetVnum() <= 50820)
				{
					...
					switch (item->GetSubType())
					{
						case USE_ABILITY_UP:
							...
							{
								ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_use_item"));
								return false;
							}
							{
								switch (item->GetValue(0))
					...
					case USE_AFFECT :
						{
							...
							else
							{
								if (item->IsPCBangItem() == true)
								{
									...
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_user_this_place"));
										return false;
									}
								}
					...
					case USE_POTION_NODELAY:
						{
							if (CArenaManager::instance().IsArenaMap(GetMapIndex()) == true)
							{
								...
								{
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_cant_user_in_arena"));
									return false;
								}
								switch (item->GetVnum())
								{
									...
									case 71020 :
										if (quest::CQuestManager::instance().GetEventFlag("arena_potion_limit_count") < 10000)
										{
											if (m_nPotionLimit <= 0)
											{
												ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_is_under_limit"));
												return false;
											}
										}
										break;
							...
							if (used)
							{
								...
								{
									if (test_server)
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("used_cake"));
					...
					case USE_TIME_CHARGE_PER:
						{
							...
							if (pDestItem->IsDragonSoul())
							{
								...
								if (ret > 0)
								{
									...
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%d sec_added"), ret);
									item->SetCount(item->GetCount() - 1);
								...
								else
								{
									...
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_dragon_soul_error"));
							...
							case ITEM_MARRIAGE_RING:
								{
									...
									if (pMarriage)
									{
										...
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_have_ring_1"));
								}
								break;
							...
							case 50004: 
								{
									...
									else
									{
										...
										{
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("compas_err_pos"));
											return false;
										}
									}
									int dist = 0;
									...
									if (distance < 1000.0f)
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("qust_complete"));
											...
											for (int i = 0; i < EVENT_STONE_MAX_INFO; ++i)
											{
												...
												if (prob <= info[i].prob)
												{
													if (info[i].dwVnum == 50001)
													{
														...
														pdw[0] = info[i].dwVnum;
														pdw[1] = info[i].count;
														DBManager::instance().ReturnQuery(QID_LOTTO, GetPlayerID(), pdw, "INSERT INTO lotto_list VALUES(0, 'server%s', %u, NOW())", get_table_postfix(), GetPlayerID());
													}
													else
									...
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("qust_complete_s"));
									...
									{
										char chatbuf[CHAT_MAX_LEN + 1];
										int len = snprintf(chatbuf, sizeof(chatbuf), "StoneDetect %u %d %d", (DWORD)GetVID(), dist, (int)GetDegreeFromPositionXY(GetX(), item->GetSocket(2), item->GetSocket(1), GetY()));
										if (len < 0 || len >= (int) sizeof(chatbuf))
											len = sizeof(chatbuf) - 1;
										++len;  
										TPacketGCChat pack_chat;
									...
									if (pMap != NULL)
									{
										...
										if (f.m_mapStone.size() > 0)
										{
											...
											if (pTarget != NULL)
											{
												...
												ChatPacket(CHAT_TYPE_COMMAND, "StoneDetect %u %d %d", (DWORD)GetVID(), val, (int)GetDegreeFromPositionXY(GetX(), pTarget->GetY(), pTarget->GetX(), GetY()));
											}
											else
											{
												ChatPacket(CHAT_TYPE_INFO, LC_TEXT("char_item_msg_2"));
											}
										}
										else
										{
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("char_item_msg_3"));
										}
							...
							case 27987:
								{
									...
									if (r <= 50)
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("stone_shard_get"));
										AutoGiveItem(27990);
									}
									else
									{
										...
										if (r <= prob_table[0])
										{
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("muschel_is_empty"));
										}
										else if (r <= prob_table[1])
										{
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("perla_get"));
											AutoGiveItem(27992);
										}
										else if (r <= prob_table[2])
										{
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("perla_get_blue"));
											AutoGiveItem(27993);
										}
										else
										{
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("perla_get_blue_red"));
											AutoGiveItem(27994);
										}
									}
								}
							...
							case 50303:
								{
									...
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_change_value"));
										return false;
									}
									int lv = GetSkillLevel(SKILL_LEADERSHIP);
									if (lv < item->GetValue(0))
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_ability_is_on_max_level"));
										return false;
									}
									if (lv >= item->GetValue(1))
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_contorl_ability_nice"));
										return false;
									}
							...
							case 50306:
								{
									if (IsPolymorphed())
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_can_reda_poly_book"));
										return false;
									}
									if (GetSkillLevel(SKILL_COMBO) == 0 && GetLevel() < 30)
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_must_be_level_30"));
										return false;
									}
									if (GetSkillLevel(SKILL_COMBO) == 1 && GetLevel() < 50)
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_must_be_level_50"));
										return false;
									}
									if (GetSkillLevel(SKILL_COMBO) >= 2)
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_ability_complete"));
										return false;
									}
							...
							case 50313:
								{
									...
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_is_not_readeble"));
										return false;
									}
							...
							case 50061 : 
								{
									...
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_ability_complete_1"));
										return false;
									}
									if (LearnSkillByBook(dwSkillVnum, iPct))
									{
							...
							case 50325: case 50326: 
								{
									...
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_have_low_lvl"));
										return false;
									}
									...
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_ability_is_low"));
										return false;
									}
									if (GetSkillLevel(dwSkillVnum) >= iSkillLevelHighLimit)
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_book_is_not_trainable1"));
										return false;
									}
									if (LearnSkillByBook(dwSkillVnum, iPct))
									{
							...
							case ITEM_HORSE_SKILL_TRAIN_BOOK:
								{
									...
									if (GetLevel() < 50)
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_ready_train_horse_skill"));
										return false;
									}
									if (!test_server && get_global_time() < GetSkillNextReadTime(dwSkillVnum))
									{
										if (FindAffect(AFFECT_SKILL_NO_BOOK_DELAY))
										{
											...
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_leave_cruse"));
										}
										else
										{
									...
									if (GetPoint(POINT_HORSE_SKILL) >= 20 || GetSkillLevel(SKILL_HORSE_WILDATTACK) + GetSkillLevel(SKILL_HORSE_CHARGE) + GetSkillLevel(SKILL_HORSE_ESCAPE) >= 60 || GetSkillLevel(SKILL_HORSE_WILDATTACK_RANGE) + GetSkillLevel(SKILL_HORSE_CHARGE) + GetSkillLevel(SKILL_HORSE_ESCAPE) >= 60)
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("skill_chart_text_4"));
										return false;
									}
									if (number(1, 100) <= iPct)
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("horse_is_summoned"));
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_get_point"));
									...
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("horse_is_not_summoned"));
									}
#ifdef ENABLE_BOOKS_STACKFIX
									item->SetCount(item->GetCount() - 1);
#else
							...
							case 70103: 
								{
									...
									if (delta / 10 > 0)
									{
										ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("you_way_lowed"));
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_get_better"), delta/10);
									}
								}
								break;
							case 71107: 
								{
									...
									if (get_global_time() - last_use_time < interval * 60 * 60)
									{
										...
										{
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("every_8hour"));
											return false;
										}
										else
										{
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("test_servr_expired"));
										}
									}
									if (GetAlignment() == 200000)
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("your_aligment_not_uppable"));
										return false;
									}
									...
									ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("you_way_lowed"));
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_get_better"), val);
									char buf[256 + 1];
							...
							case 70206:   
								{
									...
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_change_hair"));
									}
									else
									{
										...
										if (pPC)
										{
											...
											{
												ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%d level_color_use"), last_dye_level+3);
											}
										}
									}
								}
									...
									if (GiveItemFromSpecialItemGroup(dwBoxVnum, dwVnums, dwCounts, item_gets, count))
									{
										for (int i = 0; i < count; i++)
										{
											if (dwVnums[i] == CSpecialItemGroup::GOLD)
												ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_get %d gold"), dwCounts[i]);
										}
										item->SetCount(item->GetCount() - 1);
									}
								}
							...
							case ITEM_VALENTINE_CHOCOLATE:
								{
									...
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("item_only_for_man"));
										return false;
									}
									if (GiveItemFromSpecialItemGroup(dwBoxVnum, dwVnums, dwCounts, item_gets, count))
										item->SetCount(item->GetCount()-1);
...
#ifdef ENABLE_WOLFMAN_CHARACTER
											case CSpecialItemGroup::BLEEDING:
												ChatPacket(CHAT_TYPE_INFO, LC_TEXT("wolfman_shit_char_item_1"));
												break;
#endif
											case CSpecialItemGroup::MOB_GROUP:
												ChatPacket(CHAT_TYPE_INFO, LC_TEXT("warn_you_1"));
							...
							case ITEM_ELK_VNUM: 
								{
									...
									ITEM_MANAGER::instance().RemoveItem(item);
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you %d get_yang"), iGold);
							...
							case 70021:
								{
									...
									{
										char szNotice[256];
										snprintf(szNotice, sizeof(szNotice), LC_TEXT("king_weapon_magic"), EMPIRE_NAME(GetEmpire()));
										SendNoticeMap(szNotice, GetMapIndex(), false);
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("king_effec_active"));
									}
								}
								break;
							case 27995:
							...
							case 71051 : 
								{
									...
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_not_bonusable_item"));
										return false;
									}
									if (item2->IsExchanging() || item2->IsEquipped()) 
										return false;
									...
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_added_bonus"));
										return false;
									}
									if (item2->AddRareAttribute() == true)
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("your_cant_add_bonus"));
							...
							case 71052 :
								{
									...
									if (ITEM_COSTUME == item2->GetType()) 
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_added_new_special_bonus"));
										return false;
									}
									if (item2->IsExchanging() || item2->IsEquipped())
										return false;
									...
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_change"));
									}
								}
								break;
							...
							case FUCKING_BRAZIL_ITEM_AUTO_HP_RECOVERY_S:
								{
									...
									if (item->GetCount() > 1)
									{
										...
										if (-1 == pos)
										{
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("char_item_msg_15"));
											break;
										}
										item->SetCount( item->GetCount() - 1 );
										LPITEM item2 = ITEM_MANAGER::instance().CreateItem( item->GetVnum(), 1 );
					...
					case USE_TALISMAN:
						{
							...
							{
								ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_use_here"));
								return false;
							}
							if (CArenaManager::instance().IsArenaMap(GetMapIndex()) == true)
							{
							...
							if (m_pkWarpEvent)
							{
								ChatPacket(CHAT_TYPE_INFO, LC_TEXT("your_ready_on_teleport"));
								return false;
							}
							...
							if (item->GetValue(0) == TOWN_PORTAL) 
							{
								...
								{
									if (test_server)
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("teleport_on_last_town"));
							...
							else if (item->GetValue(0) == MEMORY_PORTAL) 
							{
								if (item->GetSocket(0) == 0)
								{
									if (GetDungeon())
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_is_cant_use_in_jail"), item->GetName(), "");
										return false;
									}
					...
					case USE_DETACHMENT:
						{
							...
							{
								ChatPacket(CHAT_TYPE_INFO, LC_TEXT("stone_cant_refine"));
								return false;
							}
							if (item2->GetVnum() >= 28430 && item2->GetVnum() <= 28443)  
							{
								...
								{
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_get_better_refine"));
								}
							}
							else
							{
					...
					case USE_RESET_COSTUME_ATTR:
						{
							...
							ChatPacket(CHAT_TYPE_INFO, LC_TEXT("char_item_msg_4"));
							item->SetCount(item->GetCount() - 1);
							break;
						}
					...
					case USE_ADD_ATTRIBUTE2:
						{
							...
							switch (item->GetSubType())
							{
								case USE_CLEAN_SOCKET:
									{
										...
										{
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_is_not_have_borken_stone"));
											return false;
										}
										int j = 0;
										for (i = 0; i < ITEM_SOCKET_MAX_NUM; ++i)
								...
								case USE_CHANGE_ATTRIBUTE2 : 
									...
									if (item2->GetAttributeCount() == 0)
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_item_bonus_empty"));
										return false;
									}
									if ((GM_PLAYER == GetGMLevel()) && (false == test_server) && (g_dwItemBonusChangeTime > 0))
									{
										...
										if (pPC)
										{
											...
											{
												ChatPacket(CHAT_TYPE_INFO, LC_TEXT("char_item_msg_5"), dwChangeItemAttrCycle, dwChangeItemAttrCycle - (dwNowSec - dwLastChangeItemAttrSec));
												return false;
											}
									...
									else
									{
										if (item->GetVnum() == 71151 || item->GetVnum() == 76023)
										{
												...
												|| (item2->GetType() == ITEM_ARMOR && item2->GetSubType() == ARMOR_BODY))
											{
												...
												if (false == bCanUse)
												{
													ChatPacket(CHAT_TYPE_INFO, LC_TEXT("char_item_msg_6"));
													break;
												}
											}
											else
											{
												ChatPacket(CHAT_TYPE_INFO, LC_TEXT("char_item_msg_7"));
												break;
											}
										}
										item2->ChangeAttribute();
									}
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_change_bonus"));
									{
										char buf[21];
										snprintf(buf, sizeof(buf), "%u", item2->GetID());
								...
								case USE_ADD_ATTRIBUTE :
									...
									if (item2->GetAttributeCount() < 5)
									{
										...
										{
											item2->AddAttribute();
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_add_bonus_item"));
											int iAddedIdx = item2->GetAttributeCount() - 1;
											LogManager::instance().ItemLog(GetPlayerID(), item2->GetAttributeType(iAddedIdx), item2->GetAttributeValue(iAddedIdx), item->GetID(), "ADD_ATTRIBUTE_SUCCESS", buf, GetDesc()->GetHostName(), item->GetOriginalVnum());
										}
										else
										{
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("add_bonus_filed"));
									...
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_add_new_bonus"));
									}
									break;
								case USE_ADD_ATTRIBUTE2 :
									if (item2->GetAttributeSetIndex() == -1)
									...
									if (item2->GetAttributeCount() == 4)
									{
										...
										if (number(1, 100) <= aiItemAttributeAddPercent[item2->GetAttributeCount()])
										{
											...
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_add_bonus_item"));
											int iAddedIdx = item2->GetAttributeCount() - 1;
											LogManager::instance().ItemLog(GetPlayerID(), item2->GetAttributeType(iAddedIdx), item2->GetAttributeValue(iAddedIdx), item->GetID(), "ADD_ATTRIBUTE2_SUCCESS", buf, GetDesc()->GetHostName(), item->GetOriginalVnum());
										}
										else
										{
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_not_bonusable_item"));
										...
										if (item2->IsAccessoryForSocket())
										{
											if (item2->GetAccessorySocketMaxGrade() < ITEM_ACCESSORY_SOCKET_MAX_NUM)
											{
...
#endif
												{
													...
													ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_added_this"));
													LogManager::instance().ItemLog(this, item, "ADD_SOCKET_SUCCESS", buf);
												}
												else
												{
													ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_not_added_this"));
											...
											{
												ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_add_next_slot"));
											}
										}
										else
										{
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_is_cant_use_item"));
										}
									}
									break;
								case USE_PUT_INTO_BELT_SOCKET:
									...
									if (item2->IsAccessoryForSocket() && item->CanPutInto(item2))
									{
										...
										if (item2->GetAccessorySocketGrade() < item2->GetAccessorySocketMaxGrade())
										{
											if (number(1, 100) <= aiAccessorySocketPutPct[item2->GetAccessorySocketGrade()])
											{
												...
												ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_is_sucess"));
												LogManager::instance().ItemLog(this, item, "PUT_SOCKET_SUCCESS", buf);
											}
											else
											{
												ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_is_filed"));
										...
										else
										{
											if (item2->GetAccessorySocketMaxGrade() == 0)
												ChatPacket(CHAT_TYPE_INFO, LC_TEXT("slot_added_by_diamant"));
											else if (item2->GetAccessorySocketMaxGrade() < ITEM_ACCESSORY_SOCKET_MAX_NUM)
											{
												ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_is_not_stone_add"));
												ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_can_use_diamant"));
											}
											else
												ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_use_exor"));
										}
									}
									else
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_is_not_be_equiped"));
									}
									break;
							}
							if (item2->IsEquipped())
					...
					case USE_BAIT:
						{
							if (m_pkFishingEvent)
							{
								ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_use_worm"));
								return false;
							}
							...
							{
								ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_change_worm"), item->GetName());
							}
							else
							{
								ChatPacket(CHAT_TYPE_INFO, LC_TEXT("new_worm_equiped"), item->GetName());
							}
					...
					case USE_RECIPE :
						{
							...
							if (dwSourceCount1 != 0)
							{
								if (pSource1 == NULL)
								{
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_have_material_for_elixir"));
									return false;
								}
							}
							if (dwSourceCount2 != 0)
							...
							if (pSource1 != NULL)
							{
								...
								{
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("dont_have (%s) material"), pSource1->GetName());
									return false;
								}
							...
							if (pSource2 != NULL)
							{
								...
								{
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("dont_have (%s) material"), pSource2->GetName());
									return false;
								}
							...
							{
								ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_have_empty_botles"));
								return false;
							}
							pBottle->SetCount(pBottle->GetCount() - 1);
							if (number(1, 100) > item->GetValue(5))
							{
								ChatPacket(CHAT_TYPE_INFO, LC_TEXT("error_in_crafting_elixir"));
								return false;
							}
		...
		case ITEM_METIN:
			{
				...
				for (i = 0; i < ITEM_SOCKET_MAX_NUM; ++i)
				{
					...
					{
						ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_stone_is_in_item"));
						return false;
					}
				}
				if (item2->GetType() == ITEM_ARMOR)
				{
					...
					{
						ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_add_new_stone"));
						return false;
					}
				}
				else if (item2->GetType() == ITEM_WEAPON)
				{
					...
					{
						ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_stone_cant_be_in_armor"));
						return false;
					}
				}
				else
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_item_is_full"));
					return false;
				}
				for (i = 0; i < ITEM_SOCKET_MAX_NUM; ++i)
...
#endif
						{
							ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_added_stone"));
							item2->SetSocket(i, item->GetVnum());
						}
						else
						{
							ChatPacket(CHAT_TYPE_INFO, LC_TEXT("stone_add_filed"));
...
bool CHARACTER::UseItem(TItemPos Cell, TItemPos DestCell)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_use_this_items"));
		return false;
	}
	if (IsStun())
		return false;
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("other_sex_can_use"));
		return false;
	}
#if defined(WJ_COMBAT_ZONE)
	...
	if (IS_SUMMON_ITEM(item->GetVnum()))
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_is_cant_use"));
			return false;
		}
		if (CThreeWayWar::instance().IsThreeWayWarMapIndex(GetMapIndex()))
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_use_this_in_this_place"));
			return false;
		}
		...
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_can_use_this"), g_nPortalLimitTime);
		...
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_warp_in_trade"));
			return false;
		}
		{
			if (iPulse - GetRefineTime() < PASSES_PER_SEC(g_nPortalLimitTime))
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_can_use_agin_1"), g_nPortalLimitTime);
				return false;
			}
		}
		{
			...
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_must_wait_10_sec_after_trade"), g_nPortalLimitTime);
				return false;
			}
		...
		if (item->GetVnum() != 70302)
		{
			...
			if (nDistant > nDist)
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_in_the_place"));
				if (test_server)
		...
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_can_use %d navigation"), g_nPortalLimitTime);
			return false;
		}
	}
	if ((item->GetVnum() == 50200) || (item->GetVnum() == 71049))
	{
		if (GetExchange() || GetMyShop() || GetShopOwner() || IsOpenSafebox() || IsCubeOpen())
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_open_safebox_in_trade"));
			return false;
		}
	}
...
bool CHARACTER::DropItem(TItemPos Cell, BYTE bCount)
{
	...
	if (!CanHandleItem())
	{
		if (NULL != DragonSoul_RefineWindow_GetOpener())
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_move_items_with_crafting"));
		return false;
	}
#ifdef ENABLE_NEWSTUFF
	if (0 != g_ItemDropTimeLimitValue)
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ENABLE_NEWSTUFF char_item_msg_8"));
			return false;
		}
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_item_cant_drop_on_floor"));
		return false;
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("item_despawn_after_10_sec"));
#ifdef ENABLE_NEWSTUFF
...
#endif
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("yang_drop_despwn_msg"), 150/60);
		}
		Save();
		return true;
	}
...
bool CHARACTER::MoveItem(TItemPos Cell, TItemPos DestCell, BYTE count)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("char_item_msg_9 belt_move"));
		return false;
	}
	if (Cell.IsEquipPosition())
	{
		...
		if (iWearCell == WEAR_WEAPON)
		{
			...
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("unequip_weapon_costume_full_inv"));
				return false;
			}
	...
	if (DestCell.IsEquipPosition())
	{
		if (GetItem(DestCell))	
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_stone_is_in_item_here"));
			return false;
		}
...
namespace NPartyPickupDistribute
{
	...
	struct FMoneyDistributor
	{
		...
		FMoneyDistributor(LPCHARACTER center, int iMoney) : total(0), c(center), x(center->GetX()), y(center->GetY()), iMoney(iMoney)
		{ }
		void operator ()(LPCHARACTER ch)
		{
			if (ch!=c)
...
bool CHARACTER::PickupItem(DWORD dwVID)
{
	...
	if (item->DistanceValid(this))
	{
		if (item->GetType() == ITEM_QUEST)
		{
			if (quest::CQuestManager::instance().GetPCForce(GetPlayerID())->IsRunning() == true)
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_pick_item_if_use_quest"));
				return false;
			}
		}
			...
			else
			{
				if (item->IsStackable() && !IS_SET(item->GetAntiFlag(), ITEM_ANTIFLAG_STACK))
				{
					...
					for (int i = 0; i < INVENTORY_MAX_NUM; ++i)
					{
						...
						if (item2->GetVnum() == item->GetVnum())
						{
							...
							if (bCount == 0)
							{
								ChatPacket(CHAT_TYPE_INFO, LC_TEXT("item_is_in_your_inventory: %s"), item2->GetName());
								M2_DESTROY_ITEM(item);
				...
				if (item->IsDragonSoul())
				{
					if ((iEmptyCell = GetEmptyDragonSoulInventory(item)) == -1)
					{
						...
						ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_inventory_is_full_dont_pick"));
						return false;
					}
				}
#ifdef ENABLE_SPECIAL_STORAGE
		...
		else if (!IS_SET(item->GetAntiFlag(), ITEM_ANTIFLAG_GIVE | ITEM_ANTIFLAG_DROP) && GetParty())
		{
			...
			if (item->IsDragonSoul())
			{
				if (!(owner && (iEmptyCell = owner->GetEmptyDragonSoulInventory(item)) != -1))
				{
					...
					if ((iEmptyCell = GetEmptyDragonSoulInventory(item)) == -1)
					{
						owner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_inventory_is_full_dont_pick"));
						return false;
					}
				}
			}
			...
			if (owner == this)
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("item_is_in_your_inventory: %s"), item->GetName());
			else
			{
				owner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("player: %s pick_item %s"), GetName(), item->GetName());
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("player: %s get_item"), owner->GetName(), item->GetName());
			}
			if (item->GetType() == ITEM_QUEST)
...
bool CHARACTER::EquipItem(LPITEM item, int iCandidateCell)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_use_this_on_horse"));
		return false;
	}
	if (iWearCell != WEAR_ARROW && IsPolymorphed())
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_change_equip"));
		return false;
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_un_horse_clrl_g"));
		return false;
	}
	DWORD dwCurTime = get_dword_time();
	if (iWearCell != WEAR_ARROW && iWearCell != WEAR_COSTUME_MOUNT && (dwCurTime - GetLastAttackTime() <= 1500 || dwCurTime - m_dwLastSkillTime <= 1500))
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("if_you_can_make_this_you_must_stop"));
		return false;
	}
#ifdef ENABLE_WEAPON_COSTUME_SYSTEM
	if (iWearCell == WEAR_WEAPON)
	...
	if (item->IsDragonSoul())
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, "char_item_msg_10");
			return false;
		}
...
#endif
		if ((ITEM_UNIQUE == item->GetType() && UNIQUE_SPECIAL_RIDE == item->GetSubType() && IS_SET(item->GetFlag(), ITEM_FLAG_QUEST_USE)) || (ITEM_UNIQUE == item->GetType() && UNIQUE_SPECIAL_MOUNT_RIDE == item->GetSubType() && IS_SET(item->GetFlag(), ITEM_FLAG_QUEST_USE))
...
LPITEM CHARACTER::AutoGiveItem(DWORD dwItemVnum, BYTE bCount, int iRarePct, bool bMsg)
{
	...
	if (p->dwFlags & ITEM_FLAG_STACKABLE && p->bType != ITEM_BLEND)
	{
		...
		else if (dwItemVnum == 50300) 
		{
		}
		else if (p->bType == ITEM_METIN && p->bSubType == METIN_NORMAL)
		{
			for (int i = 0; i < SPECIAL_INVENTORY_MAX_NUM; ++i)
			{
	...
	switch (GetRaceNum())
	{
		...
		case 20103:
			if (item->GetVnum() == ITEM_REVIVE_HORSE_1)
			{
				if (!IsDead())
				{
					from->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_respawn_horse"));
					return false;
				}
				return true;
			}
...
void CHARACTER::ReceiveItem(LPCHARACTER from, LPITEM item)
{
	...
	switch (GetRaceNum())
	{
		...
		case DEVILTOWER_BLACKSMITH_ACCESSORY_MOB:
			...
			{
				from->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_time_is_not_refinable"));
			}
			break;
		case BLACKSMITH_MOB:
		case BLACKSMITH2_MOB:
		...
		case 20109:
			if (item->GetVnum() == ITEM_REVIVE_HORSE_1 || item->GetVnum() == ITEM_REVIVE_HORSE_2 || item->GetVnum() == ITEM_REVIVE_HORSE_3)
			{
				from->ReviveHorse();
				item->SetCount(item->GetCount()-1);
				from->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_horse_is_hungry"));
			}
...
bool CHARACTER::ItemProcess_Hair(LPITEM item, int iDestCell)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_equip_this_1"));
		return false;
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_have_this_hairstyle"));
		return true;
	}
	item->SetCount(item->GetCount() - 1);
...
bool CHARACTER::ItemProcess_Polymorph(LPITEM item)
{
	...
	if (IsPolymorphed())
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_is_in_poly"));
		return false;
	}
	if (true == IsRiding())
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_polymorph"));
		return false;
	}
	DWORD dwVnum = item->GetSocket(0);
	if (dwVnum == 0)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_is_fail_items"));
		item->SetCount(item->GetCount()-1);
		return false;
	}
		...
		case 71093 :
			{
				...
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("low_level_on_use_poly"));
					return false;
				}
...
bool CHARACTER::CanEquipNow(const LPITEM item, const TItemPos& srcCell, const TItemPos& destCell)
{
	...
	for (int i = 0; i < ITEM_LIMIT_MAX_NUM; ++i)
	{
		...
		switch (itemTable->aLimits[i].bType)
		{
			case LIMIT_LEVEL:
				if (GetLevel() < limit)
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("your_lvl_is_low"));
					return false;
				}
				break;
			case LIMIT_STR:
				...
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_have_level_on_use_this"));
					return false;
				}
				break;
			case LIMIT_INT:
				...
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_have_small_brain"));
					return false;
				}
				break;
			case LIMIT_DEX:
			...
			case LIMIT_CON:
				...
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_can_add_points_to_vit"));
					return false;
				}
				break;
		}
	...
	if (item->GetWearFlag() & WEARABLE_UNIQUE)
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_use_this_item_5"));
			return false;
		}
		if (marriage::CManager::instance().IsMarriageUniqueItem(item->GetVnum()) && !marriage::CManager::instance().IsMarried(GetPlayerID()))
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_is_only_for_partners"));
			return false;
		}
	}
	return true;
...
bool CHARACTER::CanUnequipNow(const LPITEM item, const TItemPos& srcCell, const TItemPos& destCell) /*const*/
{
	...
	if (ITEM_BELT == item->GetType())
		VERIFY_MSG(CBeltInventoryHelper::IsExistItemInBeltInventory(this), "char_item_msg_11 belt_inventory");
		...
		return false;
	{
		...
		if (item->IsDragonSoul())
			...
			pos = GetEmptyInventory(item->GetSize());
		VERIFY_MSG( -1 == pos, "you_inventory_is_full");
	}
	return true;
}

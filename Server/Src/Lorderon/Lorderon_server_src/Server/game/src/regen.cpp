...
static void regen_spawn_dungeon(LPREGEN regen, LPDUNGEON pDungeon, bool bOnce)
{
	...
	for (i = 0; i < num; ++i)
	{
		...
		{
			ch = CHARACTER_MANAGER::instance().SpawnMob(regen->vnum, regen->lMapIndex, regen->sx, regen->sy, regen->z_section, false, regen->direction == 0 ? number(0, 7) * 45 : (regen->direction - 1) * 45);
			if (ch)
			{
				++regen->count;
				ch->SetDungeon(pDungeon);
...
bool regen_do(const char* filename, long lMapIndex, int base_x, int base_y, LPDUNGEON pDungeon, bool bOnce)
{
	...
	while (true)
	{
		...
		if (!read_line(fp, &tmp))
			break;
		if (tmp.type == REGEN_TYPE_MOB || tmp.type == REGEN_TYPE_GROUP || tmp.type == REGEN_TYPE_GROUP_GROUP || tmp.type == REGEN_TYPE_ANYWHERE)
		{
			if (!bOnce)
			{
				regen = M2_NEW REGEN;
	...
	while (true)
	{
		...
		if (tmp.type == REGEN_TYPE_MOB || tmp.type == REGEN_TYPE_GROUP || tmp.type == REGEN_TYPE_GROUP_GROUP || tmp.type == REGEN_TYPE_ANYWHERE)
		{
			...
			if (regen->type == REGEN_TYPE_MOB)
			{
				...
				{
					SECTREE_MANAGER::instance().InsertNPCPosition(lMapIndex, p->m_table.bType, p->m_table.szLocaleName, (regen->sx+regen->ex) / 2 - base_x, (regen->sy+regen->ey) / 2 - base_y);
				}
			}
			if (regen->time != 0)
			{

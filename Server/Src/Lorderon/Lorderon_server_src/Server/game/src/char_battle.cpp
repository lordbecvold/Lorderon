...
{
	bool isAutoLoot =
		(pkAttacker->GetPremiumRemainSeconds(PREMIUM_AUTOLOOT) > 0 || pkAttacker->IsEquipUniqueGroup(UNIQUE_GROUP_AUTOLOOT)) ? true : false; 
	PIXEL_POSITION pos;
	if (!isAutoLoot)
	...
	if (pkKiller && IsPC())
	{
		...
		else
		{
			...
			if (GetEmpire() != pkKiller->GetEmpire())
			{
				...
				{ }
				char buf[256];
				snprintf(buf, sizeof(buf), "%d %d %d %s %d %d %d %s", GetEmpire(), GetAlignment(), GetPKMode(), GetName(), pkKiller->GetEmpire(), pkKiller->GetAlignment(), pkKiller->GetPKMode(), pkKiller->GetName());
			...
			else
			{
				if (!isAgreedPVP && !isUnderGuildWar && !IsKillerMode() && GetAlignment() >= 0 && !isDuel && !isForked)
				{
					...
					if (number(1, 100) < iNoPenaltyProb)
						pkKiller->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("char_battle_msg_1"));
					else
					{
		...
		if (!IS_SET(m_pointsInstant.instant_flag, INSTANT_FLAG_NO_REWARD))
		{
			...
			else
			{
				if (pkKiller->m_dwUnderGuildWarInfoMessageTime < get_dword_time())
				{
					...
					pkKiller->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("char_battle_msg_2"));
				}
			}
		}
	}
...
bool CHARACTER::Damage(LPCHARACTER pAttacker, int dam, EDamageType type) 
{
		...
		if (pAttacker)
		{
			...
			if (iPenetratePct)
			{
				...
				if (number(1, 100) <= iPenetratePct)
				{
					IsPenetrate = true;
					if (test_server)
						ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[TEST-SERVER-INFO]char_battle_msg_3 %d"), GetPoint(POINT_DEF_GRADE) * (100 + GetPoint(POINT_DEF_BONUS)) / 100);
		...
		if (pAttacker)
		{
			...
			if (iPenetratePct)
			{
				...
				if (number(1, 100) <= iPenetratePct)
				{
					IsPenetrate = true;
					if (test_server)
						ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[TEST-SERVER-INFO]char_battle_msg_4 %d"), GetPoint(POINT_DEF_GRADE) * (100 + GetPoint(POINT_DEF_BONUS)) / 100);
					dam += GetPoint(POINT_DEF_GRADE) * (100 + GetPoint(POINT_DEF_BONUS)) / 100;
#ifdef ENABLE_EFFECT_PENETRATE
					EffectPacket(SE_PENETRATE);
#endif
	...
	if (pAttacker)
	{
		...
		if (test_server)
		{
			...
			if(pAttacker)
			{
				pAttacker->ChatPacket(CHAT_TYPE_INFO, "-> %s, DAM %d HP %d(%d%%) %s%s", GetName(), dam, GetHP(), iTmpPercent, IsCritical ? "crit " : "", IsPenetrate ? "pene " : "", IsDeathBlow ? "deathblow " : "");
			}
			ChatPacket(CHAT_TYPE_PARTY, "<- %s, DAM %d HP %d(%d%%) %s%s", pAttacker ? pAttacker->GetName() : 0, dam, GetHP(), iTmpPercent, IsCritical ? "crit " : "", IsPenetrate ? "pene " : "", IsDeathBlow ? "deathblow " : "");
		}
		if (m_bDetailLog)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s[%d] attack_pos: %d %d"), pAttacker->GetName(), (DWORD) pAttacker->GetVID(), pAttacker->GetX(), pAttacker->GetY());
		}
	}
	if (!cannot_dead)
	{
...
static void GiveExp(LPCHARACTER from, LPCHARACTER to, int iExp)
{
	...
	if (test_server)
	{
		sys_log(0, "Bonus Exp : Ramadan Candy: %d MallExp: %d PointExp: %d", to->GetPoint(POINT_RAMADAN_CANDY_BONUS_EXP), to->GetPoint(POINT_MALL_EXPBONUS), to->GetPoint(POINT_EXP));
	}
...
namespace NPartyExpDistribute
{
	struct FPartyTotaler
	{
		...
		int member_count;
		int x, y;
		FPartyTotaler(LPCHARACTER center) : total(0), member_count(0), x(center->GetX()), y(center->GetY())
		{};
		void operator () (LPCHARACTER ch)
		{
	...
	struct FPartyDistributor
	{
		...
		int m_iMode;
		int m_iMemberCount;
		FPartyDistributor(LPCHARACTER center, int member_count, int total, DWORD iExp, int iMode) : total(total), c(center), x(center->GetX()), y(center->GetY()), _iExp(iExp), m_iMode(iMode), m_iMemberCount(member_count)
		{
			if (m_iMemberCount == 0)
				m_iMemberCount = 1;
		};
...
LPCHARACTER CHARACTER::GetNearestVictim(LPCHARACTER pkChr)
{
	...
	while (it != m_map_kDamage.end())
	{
		...
		if (!pAttacker)
			continue;
		if (pAttacker->IsAffectFlag(AFF_EUNHYUNG) || pAttacker->IsAffectFlag(AFF_INVISIBILITY) || pAttacker->IsAffectFlag(AFF_REVIVE_INVISIBLE))
			continue;

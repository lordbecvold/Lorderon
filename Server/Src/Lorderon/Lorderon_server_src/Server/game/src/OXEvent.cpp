...
bool COXEventManager::ShowQuizList(LPCHARACTER pkChar)
{
	...
	pkChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("question_count: %d"), c);
	return true;
}
...
EVENTFUNC(oxevent_timer)
{
	...
	{
		case 0:
			SendNoticeMap(LC_TEXT("player_have_10_ces_answr_2"), OXEVENT_MAP_INDEX, true);
			flag++;
			return PASSES_PER_SEC(10);

		case 1:
			SendNoticeMap(LC_TEXT("final_answ"), OXEVENT_MAP_INDEX, true);

			if (info->answer == true)
			{
				COXEventManager::instance().CheckAnswer(true);
				SendNoticeMap(LC_TEXT("answ_is_yes"), OXEVENT_MAP_INDEX, true);
			}
			else
			{
				COXEventManager::instance().CheckAnswer(false);
				SendNoticeMap(LC_TEXT("answ_is_no"), OXEVENT_MAP_INDEX, true);
			}

			SendNoticeMap(LC_TEXT("b_andw_kick_from_ox"), OXEVENT_MAP_INDEX, true);
		...
		case 2:
			...
			SendNoticeMap(LC_TEXT("you_ready_on_question"), OXEVENT_MAP_INDEX, true);
			flag = 0;
			break;
	}
	return 0;
...
bool COXEventManager::Quiz(unsigned char level, int timelimit)
{
	...
	SendNoticeMap(LC_TEXT("question_say"), OXEVENT_MAP_INDEX, true);
	SendNoticeMap(m_vec_quiz[level][idx].Quiz, OXEVENT_MAP_INDEX, true);
	SendNoticeMap(LC_TEXT("ox_msg_cent"), OXEVENT_MAP_INDEX, true);
...
bool COXEventManager::CheckAnswer(bool answer)
{
	...
	for (; iter != m_map_attender.end();)
	{
		...
		if (pkChar != NULL)
		{
			...
			{
				pkChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("success_nasw"));
				char chatbuf[256];

...
void interpret_command(LPCHARACTER ch, const char * argument, size_t len)
{
	...
	if (ch->GetPosition() < cmd_info[icmd].minimum_position)
	{
		...
		{
			case POS_MOUNTING:
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("cmd_msg_1"));
				break;
			case POS_DEAD:
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("cmd_msg_2"));
				break;
			case POS_SLEEPING:
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("cmd_msg_3"));
				break;
			case POS_RESTING:
			case POS_SITTING:
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("cmd_msg_4"));
				break;
			default:

...
void Cube_open (LPCHARACTER ch)
{
	...
	if (ch->IsCubeOpen())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("open_crafting_window"));
		return;
	}
	if ( ch->GetExchange() || ch->GetMyShop() || ch->GetShopOwner() || ch->IsOpenSafebox() || ch->IsCubeOpen() )
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_is_imposible_to_use"));
		return;
	}
...
bool Cube_make (LPCHARACTER ch)
{
	...
	if (!(ch)->IsCubeOpen())
	{
		(ch)->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("crafting_cant_open"));
		return false;
	}
	npc = ch->GetQuestNPC();
	if (NULL == npc)
	...
	if (NULL == cube_proto)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_have_materials_to_crafting"));
		return false;
	}
	int cube_gold = cube_proto->gold;
	if (cube_gold < 0 || ch->GetGold() < cube_gold)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_have_yang_of_full_inventory"));	
		return false;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("crafting_80_filed"));	

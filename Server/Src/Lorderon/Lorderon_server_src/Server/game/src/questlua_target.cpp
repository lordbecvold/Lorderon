...
namespace quest
{
	ALUA(target_pos)
	{
		...
		CTargetManager::instance().CreateTarget(ch->GetPlayerID(), iQuestIndex, lua_tostring(L, 1), TARGET_TYPE_POS, x, y, (int) lua_tonumber(L, 4), lua_isstring(L, 5) ? lua_tostring(L, 5) : NULL, lua_isnumber(L, 6) ? (int)lua_tonumber(L, 6): 1);
		return 0;
	}
	ALUA(target_vid)
	{
		...
		CTargetManager::instance().CreateTarget(ch->GetPlayerID(), iQuestIndex, lua_tostring(L, 1), TARGET_TYPE_VID, (int) lua_tonumber(L, 2), 0, ch->GetMapIndex(), lua_isstring(L, 3) ? lua_tostring(L, 3) : NULL, lua_isnumber(L, 4) ? (int)lua_tonumber(L, 4): 1);
		return 0;
	}

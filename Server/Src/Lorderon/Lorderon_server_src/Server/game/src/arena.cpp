...
EVENTFUNC(ready_to_start_event)
{
	...
	if (chA == NULL || chB == NULL)
	{
		...
		if (chA != NULL)
		{
			chA->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("duel_cancel_enemy_leave"));
			sys_log(0, "ARENA: Oppernent is disappered. MyPID(%d) OppPID(%d)", pArena->GetPlayerAPID(), pArena->GetPlayerBPID());
		}
		if (chB != NULL)
		{
			chB->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("duel_cancel_enemy_leave"));
			sys_log(0, "ARENA: Oppernent is disappered. MyPID(%d) OppPID(%d)", pArena->GetPlayerBPID(), pArena->GetPlayerAPID());
		}
		pArena->SendChatPacketToObserver(CHAT_TYPE_NOTICE, LC_TEXT("duel_cancel_enemy_leave"));
		pArena->EndDuel();
		return 0;
	}
	...
	switch (info->state)
	{
		case 0:
			{
				...
				if (count > 10000)
				{
					chA->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("pottion_is_not_have_limit"));
					chB->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("pottion_is_not_have_limit"));
				}
				else
				{
					chA->SetPotionLimit(count);
					chB->SetPotionLimit(count);
					chA->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("pottion_use_sec"), chA->GetPotionLimit());
					chB->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("pottion_use_sec"), chB->GetPotionLimit());
				}
				chA->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("player_have_10_sec_answ"));
				chB->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("player_have_10_sec_answ"));
				pArena->SendChatPacketToObserver(CHAT_TYPE_INFO, LC_TEXT("player_have_10_sec_answ"));
				info->state++;
		...
		case 1:
			{
				chA->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("duel_started"));
				chB->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("duel_started"));
				pArena->SendChatPacketToObserver(CHAT_TYPE_INFO, LC_TEXT("duel_started"));
		...
		default:
			{
				chA->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("duel_cancel_arena_problem"));
				chB->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("duel_cancel_arena_problem"));
				pArena->SendChatPacketToObserver(CHAT_TYPE_INFO, LC_TEXT("duel_cancel_arena_problem"));
...
EVENTFUNC(duel_time_out)
{
	...
	if (chA == NULL || chB == NULL)
	{
		...
		pArena->SendChatPacketToObserver(CHAT_TYPE_INFO, LC_TEXT("duel_cancel_enemy_leave"));
		pArena->EndDuel();
		return 0;
	}
	else
	{
		...
		{
			case 0:
				pArena->SendChatPacketToObserver(CHAT_TYPE_NOTICE, LC_TEXT("duel_time_expired"));
				pArena->SendChatPacketToObserver(CHAT_TYPE_NOTICE, LC_TEXT("player_have_10_ces_answr"));
				chA->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("duel_time_expired"));
				chA->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("player_have_10_ces_answr"));
				chB->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("duel_time_expired"));
				chB->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("player_have_10_ces_answr"));
				TPacketGCDuelStart duelStart;
				duelStart.header = HEADER_GC_DUEL_START;
...
bool CArena::OnDead(DWORD dwPIDA, DWORD dwPIDB)
{
	...
	{
		SendChatPacketToObserver(CHAT_TYPE_NOTICE, LC_TEXT("duel_cancel_arena_problem_1"));
		restart = false;
	}
	else if (pCharA == NULL && pCharB != NULL)
	{
		pCharB->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("war_have_problem_cancel"));
		SendChatPacketToObserver(CHAT_TYPE_NOTICE, LC_TEXT("duel_cancel_1"));
		restart = false;
	}
	else if (pCharA != NULL && pCharB == NULL)
	{
		pCharA->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("war_have_problem_cancel"));
		SendChatPacketToObserver(CHAT_TYPE_NOTICE, LC_TEXT("duel_cancel_1"));
		restart = false;
	}
	else if (pCharA != NULL && pCharB != NULL)
	{
		if (m_dwPIDA == dwPIDA)
		{
			...
			{
				pCharA->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("%s duel_winer"), pCharA->GetName());
				pCharB->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("%s duel_winer"), pCharA->GetName());
				SendChatPacketToObserver(CHAT_TYPE_NOTICE, LC_TEXT("%s duel_winer"), pCharA->GetName());
				sys_log(0, "ARENA: Duel is end. Winner %s(%d) Loser %s(%d)", pCharA->GetName(), GetPlayerAPID(), pCharB->GetName(), GetPlayerBPID());
			}
			else
			{
				restart = true;
				pCharA->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s player_winer"), pCharA->GetName());
				pCharA->ChatPacket(CHAT_TYPE_NOTICE, "%s %d : %d %s", pCharA->GetName(), m_dwSetPointOfA, m_dwSetPointOfB, pCharB->GetName());
				pCharB->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s player_winer"), pCharA->GetName());
		...
		else if (m_dwPIDB == dwPIDA)
		{
			...
			{
				pCharA->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("%s duel_winer"), pCharB->GetName());
				pCharB->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("%s duel_winer"), pCharB->GetName());
				SendChatPacketToObserver(CHAT_TYPE_NOTICE, LC_TEXT("%s duel_winer"), pCharB->GetName());
				sys_log(0, "ARENA: Duel is end. Winner(%d) Loser(%d)", GetPlayerBPID(), GetPlayerAPID());
			}
			else
			{
				restart = true;
				pCharA->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s player_winer"), pCharB->GetName());
				pCharA->ChatPacket(CHAT_TYPE_NOTICE, "%s %d : %d %s", pCharA->GetName(), m_dwSetPointOfA, m_dwSetPointOfB, pCharB->GetName());
				pCharB->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s player_winer"), pCharB->GetName());
				pCharB->ChatPacket(CHAT_TYPE_NOTICE, "%s %d : %d %s", pCharA->GetName(), m_dwSetPointOfA, m_dwSetPointOfB, pCharB->GetName());
				SendChatPacketToObserver(CHAT_TYPE_NOTICE, "%s %d : %d %s", pCharA->GetName(), m_dwSetPointOfA, m_dwSetPointOfB, pCharB->GetName());
	...
	{
		if (pCharA != NULL)
			pCharA->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("player_have_10_ces_answr_1"));
		if (pCharB != NULL)
			pCharB->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("player_have_10_ces_answr_1"));
		SendChatPacketToObserver(CHAT_TYPE_INFO, LC_TEXT("player_have_10_ces_answr_1"));
		if (m_pEvent != NULL) {
			event_cancel(&m_pEvent);
		}
		TArenaEventInfo* info = AllocEventInfo<TArenaEventInfo>();
	...
	{
		if (pCharA != NULL)
			pCharA->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("player_have_10_ces_answr_2"));
		if (pCharB != NULL)
			pCharB->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("player_have_10_ces_answr_2"));
		SendChatPacketToObserver(CHAT_TYPE_INFO, LC_TEXT("player_have_10_ces_answr_2"));
		if (m_pEvent != NULL) {
			event_cancel(&m_pEvent);
		}
		TArenaEventInfo* info = AllocEventInfo<TArenaEventInfo>();
...
void CArena::OnDisconnect(DWORD pid)
{
	...
	{
		if (GetPlayerB() != NULL)
			GetPlayerB()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("war_is_gone_player_teleported"));
	...
	{
		if (GetPlayerA() != NULL)
			GetPlayerA()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("war_is_gone_player_teleported"));

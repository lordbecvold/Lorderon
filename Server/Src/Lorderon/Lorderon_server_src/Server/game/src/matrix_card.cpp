...
bool MatrixCardCheck(const char * src, const char * answer, unsigned long rows, unsigned cols)
{
	...
	sys_log(0, "MatrixCardCheck %c%d %c%d %c%d %c%d answer %s", MATRIX_CARD_ROW(rows, 0) + 'A', MATRIX_CARD_COL(cols, 0) + 1, MATRIX_CARD_ROW(rows, 1) + 'A', MATRIX_CARD_COL(cols, 1) + 1, MATRIX_CARD_ROW(rows, 2) + 'A', MATRIX_CARD_COL(cols, 2) + 1, MATRIX_CARD_ROW(rows, 3) + 'A', MATRIX_CARD_COL(cols, 3) + 1, answer);
...
int main(int argc, char* argv[])
{
	...
	printf("%c%d %c%d %c%d %c%d\r\n", ROW(rand_rows, 0) + 'A', COL(rand_cols, 0) + 1, \ ROW(rand_rows, 1) + 'A', COL(rand_cols, 1) + 1, \ ROW(rand_rows, 2) + 'A', COL(rand_cols, 2) + 1, \ ROW(rand_rows, 3) + 'A', COL(rand_cols, 3) + 1);
	char answer[9];

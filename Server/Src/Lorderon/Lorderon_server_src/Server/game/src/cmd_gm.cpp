	...
	if (item)
	{
		if (item->IsDragonSoul())
		{
			...
			else
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "cmd_gm_msg_1");
				}
				else
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_inventory_is_full"));
			}
		}
...
ACMD(do_buy_marble)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "you_dont_have_enough_Yang");
		return;
	}
...
ACMD(do_mob_ld)
{
	...
	CHARACTER_MANAGER::instance().SpawnMob(vnum, ch->GetMapIndex(), x*100, y*100, ch->GetZ(), pkMob->m_table.bType == CHAR_TYPE_STONE, dir);
}
...
ACMD(do_state)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "   STUN:%d SLOW:%d FALL:%d", tch->GetPoint(POINT_IMMUNE_STUN), tch->GetPoint(POINT_IMMUNE_SLOW), tch->GetPoint(POINT_IMMUNE_FALL));
...
ACMD(do_who)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "Total [%d] %d / %d / %d (this server %d)", iTotal, paiEmpireUserCount[1], paiEmpireUserCount[2], paiEmpireUserCount[3], iLocal);
}
...
ACMD(do_makeguild)
{
	...
	gm.CreateGuild(cp);
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("guild_created"), cp.name);
}
...
ACMD(do_event_flag)
{
	...
	if (!strcmp(arg1, "mob_item") || !strcmp(arg1, "mob_exp") || !strcmp(arg1, "mob_gold") || !strcmp(arg1, "mob_dam") || !strcmp(arg1, "mob_gold_pct") || !strcmp(arg1, "mob_item_buyer") || !strcmp(arg1, "mob_exp_buyer") || !strcmp(arg1, "mob_gold_buyer") || !strcmp(arg1, "mob_gold_pct_buyer"))
...
ACMD(do_b1)
{
	...
	for (int i = 0; i < 30; ++i)
	{
		...
		LPCHARACTER tch = CHARACTER_MANAGER::instance().SpawnMobRange(number(701, 706), ch->GetMapIndex(), 304222 + (int)fx, 742858 + (int)fy, 304222 + (int)fx, 742858 + (int)fy, true, false);
		tch->SetAggressive();
	}
	for (int i = 0; i < 5; ++i)
	{
		...
		float fx, fy;
		GetDeltaByDegree(rot, 800, &fx, &fy);
		LPCHARACTER tch = CHARACTER_MANAGER::instance().SpawnMobRange(8009, ch->GetMapIndex(), 304222 + (int)fx, 742858 + (int)fy, 304222 + (int)fx, 742858 + (int)fy, true, false);
		tch->SetAggressive();
	}
}
...
ACMD(do_b4)
{
	...
	for (int i = 0; i < 30; ++i)
	{
		...
		LPCHARACTER tch = CHARACTER_MANAGER::instance().SpawnMobRange(number(701, 706), ch->GetMapIndex(), 307900 + (int)fx, 744500 + (int)fy, 307900 + (int)fx, 744500 + (int)fy, true, false);
		tch->SetAggressive();
	}
	for (int i = 0; i < 5; ++i)
	{
		...
		float fx, fy;
		GetDeltaByDegree(rot, 1200, &fx, &fy);
		LPCHARACTER tch = CHARACTER_MANAGER::instance().SpawnMobRange(8009, ch->GetMapIndex(), 307900 + (int)fx, 744500 + (int)fy, 307900 + (int)fx, 744500 + (int)fy, true, false);
		tch->SetAggressive();
	}
}
...
ACMD(do_gwlist)
{
	ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("cmd_gm_msg_55"));
...
ACMD(do_guild_state)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s: guild_not_exist"), arg1);
	}
}
...
ACMD(do_priv_empire)
{
	...
	sys_log(0, "_give_empire_privileage(empire=%d, type=%d, value=%d, duration=%d) by command", empire, type, value, duration);
...
ACMD(do_priv_guild)
{
	...
	if (*arg1)
	{
		...
		if (!g)
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("guil_name_not_exist"));
		else
		{
			char buf[1024+1];
...
ACMD(do_block_chat_list)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("cmd_not_found"));
		return;
	}
	DBManager::instance().ReturnQuery(QID_BLOCK_CHAT_LIST, ch->GetPlayerID(), NULL, "SELECT p.name, a.lDuration FROM affect%s as a, player%s as p WHERE a.bType = %d AND a.dwPID = p.id", get_table_postfix(), get_table_postfix(), AFFECT_BLOCK_CHAT);
}
...
ACMD(do_block_chat)
{
	...
	if (lBlockDuration < 0)
	{
		if (ch)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "lBlockDuration cmd_gm_msg_2");
		...
		case 'c':
			{
				...
				if (t->dwGroupVnum)
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_can_have_one_guild"));
						return;
					}
				}
				...
				{
					{
						...
						{
							ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_can_build_here"));
							return;
						}
					}
				}
				if (test_server || GMLevel == GM_PLAYER)
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("build_filed"));
						return;
					}
					if (ch->GetGold() < (int)t->dwPrice)
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_have_yang_on_build"));
						return;
					}
					int i;
					for (i = 0; i < OBJECT_MATERIAL_MAX_NUM; ++i)
					{
						...
						{
							ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_have_items"));
							return;
						}
					}
				}
				...
				bool isSuccess = pkLand->RequestCreateObject(dwVnum, ch->GetMapIndex(), map_x, map_y, x_rot, y_rot, z_rot, true);
				if (!isSuccess)
				{
					if (test_server)
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_build_this_here"));
					return;
				}
				if (test_server || GMLevel == GM_PLAYER)
				{
...
ACMD(do_horse_level)
{
	...
	if (NULL == victim)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_player_not_exist"));
		return;
	}
	str_to_number(level, arg2);
...
ACMD(do_end_duel)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("duel_not_closed"));
	}
	else
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("duel_started_1"));
	}
}
...
ACMD(do_duel)
{
	...
	if (pChar1 != NULL && pChar2 != NULL)
	{
		...
		if (pParty != NULL)
		{
			...
			{
				pChar1->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_leave_grupe"));
		...
		if (pParty != NULL)
		{
			...
			{
				pChar2->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("cmd_gm_msg_3"));
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("problem_msg"));
		}
	}
	else
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("enemy_not_found"));
	}
}
...
ACMD(do_stat_plus_amount)
{
	...
	if (nRemainPoint <= 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_add_next_point"));
		return;
	}
	int nPoint = 0;
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_have_next_points"));
		return;
	}
	if (nPoint < 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_data_si_wrong"));
		return;
	}
	...
	{
		case POINT_HT : 
			if (nPoint + ch->GetPoint(POINT_HT) > 105)
			{
				nPoint = 105 - ch->GetPoint(POINT_HT);
			}
			break;
		case POINT_IQ :
			if (nPoint + ch->GetPoint(POINT_IQ) > 105)
			{
				nPoint = 105 - ch->GetPoint(POINT_IQ);
			}
			break;
		case POINT_ST : 
			if (nPoint + ch->GetPoint(POINT_ST) > 105)
			{
				nPoint = 105 - ch->GetPoint(POINT_ST);
			}
			break;
		case POINT_DX : 
			if (nPoint + ch->GetPoint(POINT_DX) > 105)
			{
				nPoint = 105 - ch->GetPoint(POINT_DX);
			}
			break;
		default :
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_type_all_cmd"));
			return;
			break;
	}
#endif
...
ACMD(do_break_marriage)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("server_error_1"), pids.pid1, pids.pid2);
...
{
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("kingdom_info"));
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("map_info %d - %d %d %d"), GetSungziMapIndex(), GetPassMapIndex(1), GetPassMapIndex(2), GetPassMapIndex(3));
	...
	for (int n = 1; n < 4; ++n)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "%s killscore %d usercount %d", EMPIRE_NAME(n), CThreeWayWar::instance().GetKillScore(n), c.GetCount(n));
	}
}

ACMD(do_threeway_war_myinfo)
{
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_wait_pls"));
	ch->ChatPacket(CHAT_TYPE_INFO, "Deadcount %d", CThreeWayWar::instance().GetReviveTokenForPlayer(ch->GetPlayerID()));
}
...
ACMD (do_item_full_set)
{
	...
	case JOB_SURA:
		{
			item = ITEM_MANAGER::instance().CreateItem(11329 ); //Pancir draciho boha +9
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(13149 ); //Titansky stit +9
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(15449 ); //Ohnive boty +9
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(16579 ); //Turmalinovy nahrdelnik+9
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(12719 ); //Helma z dracich supin +9
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(14579 ); //Turmalinovy naramek +9
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(17579 ); //Turmalinove nausnice+9
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(319 ); //Mec draciho boha+9
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(18099 ); //Turmalinovy opasek +9
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(39036 ); //Vetrne boty
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(71143 ); //Prsten radosti
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(85004 ); //SERPA
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
		}
		break;
	case JOB_WARRIOR:
		{
			item = ITEM_MANAGER::instance().CreateItem(11319 ); //Brneni draciho boha+9
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(13149 ); //Titansky stit +9
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
			...
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(12699 ); //Ocelova draci helma +9
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(14579 ); //Turmalinovy naramek +9
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
	...
	case JOB_SHAMAN:
		{
			item = ITEM_MANAGER::instance().CreateItem(11349 ); //Aaty draciho boha +9
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(13149 ); //Titansky stit +9
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
			...
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(12729 ); //Draci klobouk +9
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(14579 ); //Turmalinovy naramek +9
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
			...
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(369 ); //Zvon draciho boha+9
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(18099 ); //Turmalinovy opasek +9
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
	...
	case JOB_ASSASSIN:
		{
			item = ITEM_MANAGER::instance().CreateItem(11339 ); //Oblek draciho boha +9
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(13149 ); //Titansky stit +9
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
			...
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(12709 ); //Draci kape +9
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(14579 ); //Turmalinovy naramek +9
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
			...
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(349 ); //Dyka dracho boha+9
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(18099 ); //Turmalinovy opasek +9
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
...
ACMD (do_use_item)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "cmd_gm_msg_4");
	}
}
...
ACMD(do_delete_tickets_closed)
{
	...
	if (!strcmp(arg1, "true"))
	{
		std::auto_ptr<SQLMsg> dwExtract(DBManager::instance().DirectQuery("SELECT COUNT(*) FROM ticket.list WHERE status = 2"));

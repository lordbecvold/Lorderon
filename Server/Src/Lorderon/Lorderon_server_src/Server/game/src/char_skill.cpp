...
bool CHARACTER::LearnGrandMasterSkill(DWORD dwSkillVnum)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_read_this_book"));
		return false;
	}
	sys_log(0, "learn grand master skill[%d] cur %d, next %d", dwSkillVnum, get_global_time(), GetSkillNextReadTime(dwSkillVnum));
	if (pkSk->dwType == 0)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_train_this_ability"));
		return false;
	}
	if (GetSkillMasterType(dwSkillVnum) != SKILL_GRAND_MASTER)
	{
		if (GetSkillMasterType(dwSkillVnum) > SKILL_GRAND_MASTER)
			...
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ability_is_not_max_level"));
		return false;
	}
	...
	{
		ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("you_must_learn"));
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("training_filed"));
		LogManager::instance().CharLog(this, dwSkillVnum, "GM_READ_FAIL", "");
		return false;
	}
	ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("reading_sucess"));
	ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("capacity_is_upgraded"));
	ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_is_skill_master"));
...
bool CHARACTER::LearnSkillByBook(DWORD dwSkillVnum, BYTE bProb)
{
	...
	if (FN_should_check_exp(this))
	{
        ...
        if (exp_for_fb > 0) {
            if (GetExp() < need_exp) {
                ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_have_much_exp"));
                return false;
            }
        }
	}
	if (pkSk->dwType != 0)
	{
		...
		{
			if (GetSkillMasterType(dwSkillVnum) > SKILL_MASTER)
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_have_exp_on_read"));
			else
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_user_this_skill"));
			return false;
		}
	}
			...
			if (pPC)
			{
				...
				if (number(1, 100) > percent)
				{
...
#endif
					{
						SkillLevelUp(dwSkillVnum, SKILL_UP_BY_BOOK);
						pPC->SetFlag(flag, 0);
						ChatPacket(CHAT_TYPE_INFO, LC_TEXT("reading_sucess"));
					...
					else
					{
						...
						{
							case 1:
								ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("i_can_read_skill"));
								break;
							case 2:
								ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("this_is_fun"));
								break;
							case 3:
							default:
								ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("training_is_only_read"));
								break;
						}
						ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%d you_must_reead_books"), need_bookcount - read_count);
						return true;
					}
				}
			}
...
void CHARACTER::SkillLevelUp(DWORD dwVnum, BYTE bMethod)
{
	...
	if (pkSk->preSkillVnum)
		if (GetSkillMasterType(pkSk->preSkillVnum) == SKILL_NORMAL && GetSkillLevel(pkSk->preSkillVnum) < pkSk->preSkillLevel)
			return;
	if (!GetSkillGroup())
		return;
	if (bMethod == SKILL_UP_BY_POINT)
	...
	char szSkillUp[1024];
	snprintf(szSkillUp, sizeof(szSkillUp), "SkillUp: %s %u %d %d[Before:%d] type %u", GetName(), pkSk->dwVnum, m_pSkillLevels[pkSk->dwVnum].bMasterType, m_pSkillLevels[pkSk->dwVnum].bLevel, SkillPointBefore, pkSk->dwType);
...
{
	FFindNearVictim(LPCHARACTER center, LPCHARACTER attacker, const CHARACTER_SET& excepts_set = empty_set_) : m_pkChrCenter(center),
...
EVENTINFO(chain_lightning_event_info)
{
	...
	chain_lightning_event_info() : dwVictim(0), dwChr(0)
	{ }
};
...
struct FuncSplashDamage
{
	...
	void operator () (LPENTITY ent)
	{
		...
		{
			m_pkChr->ChatPacket(CHAT_TYPE_INFO, "[TEST-SERVER-INFO]char_skill_msg_1 POINT_NONE");
		}
		iAmount = -iAmount;
...
EVENTINFO(skill_gwihwan_info)
{
	DWORD pid;
	BYTE bsklv;
	skill_gwihwan_info() : pid(0), bsklv(0) { }
};
...
int CHARACTER::ComputeSkillAtPosition(DWORD dwVnum, const PIXEL_POSITION& posTarget, BYTE bSkillLevel)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, "[TEST-SERVER-INFO]char_skill_msg_2 POINT_NONE");
	}
	...
	else
	{
		...
		if (iDur > 0)
		{
			...
			AddAffect(pkSk->dwVnum, pkSk->bPointOn, iAmount, pkSk->dwAffectFlag, iDur, (long) pkSk->kDurationSPCostPoly.Eval(), !bAdded);
			bAdded = true;
		}
		else
		{
...
int CHARACTER::ComputeSkill(DWORD dwVnum, LPCHARACTER pkVictim, BYTE bSkillLevel)
{
	...
	{
		if (test_server)
			sys_log(0, "ComputeSkill: Victim too far, skill %d : %s to %s (distance %u limit %u)", dwVnum, GetName(), pkVictim->GetName(), (long)DISTANCE_SQRT(GetX() - pkVictim->GetX(), GetY() - pkVictim->GetY()), pkSk->dwTargetRange);
		return BATTLE_NONE;
	}
	if (0 == bSkillLevel)
	{
	...
	int iAmount3 = (int) pkSk->kPointPoly3.Eval();
	if (test_server && IsPC())
		sys_log(0, "iAmount: %d %d %d , atk:%f skLevel:%f k:%f GetSkillPower(%d) MaxLevel:%d Per:%f", iAmount, iAmount2, iAmount3, pkSk->kPointPoly.GetVar("atk"), pkSk->kPointPoly.GetVar("k"), k, GetSkillPower(pkSk->dwVnum, bSkillLevel), pkSk->bMaxLevel, pkSk->bMaxLevel/100);
	if (GetUsedSkillMasterType(pkSk->dwVnum) >= SKILL_GRAND_MASTER)
	{
		iAmount = (int) pkSk->kMasterBonusPoly.Eval();
	}
	if (test_server && iAmount == 0 && pkSk->bPointOn != POINT_NONE)
	{
		ChatPacket(CHAT_TYPE_INFO, "[TEST-SERVER-INFO]char_skill_msg_3 POINT_NONE");
	}
	if (IS_SET(pkSk->dwFlag, SKILL_FLAG_REMOVE_BAD_AFFECT))
	{
		if (number(1, 100) <= iAmount2)
	...
	else
	{
		if (dwVnum == SKILL_MUYEONG)
		{
			...
			iDur += GetPoint(POINT_PARTY_BUFFER_BONUS);
			if (pkVictim == this)
				AddAffect(dwVnum, POINT_NONE, 0, AFF_MUYEONG, iDur, (long) pkSk->kDurationSPCostPoly.Eval(), true);
			return BATTLE_NONE;
		}
		bool bAdded = false;
		pkSk->kDurationPoly.SetVar("k", k);
		...
		if (iDur > 0)
		{
			...
			if (pkSk->bPointOn2 != POINT_NONE)
			{
				...
				{
					if (test_server)
						sys_log(0, "SKILL_AFFECT: %s %s Dur:%d To:%d Amount:%d", GetName(), pkSk->szName, iDur2, pkSk->bPointOn2, iAmount2);
				...
				pkVictim->AddAffect(pkSk->dwVnum, pkSk->bPointOn, iAmount, affact_flag, iDur, (long) pkSk->kDurationSPCostPoly.Eval(), false);
			}
			else
			{
				if (test_server)
					sys_log(0, "SKILL_AFFECT: %s %s Dur:%d To:%d Amount:%d", GetName(), pkSk->szName, iDur, pkSk->bPointOn, iAmount);
				pkVictim->AddAffect(pkSk->dwVnum, pkSk->bPointOn, iAmount, pkSk->dwAffectFlag, iDur, (long) pkSk->kDurationSPCostPoly.Eval(), !bAdded);
			}
			bAdded = true;
		}
		else
...
bool CHARACTER::UseSkill(DWORD dwVnum, LPCHARACTER pkVictim, bool bUseGrandMaster)
{
	...
	if (dwVnum == SKILL_HORSE_SUMMON)
	{
		...
		if (GetHorseLevel() <= 0)
			...
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_skill_cant_learn"));
		return true;
	}
	if (false == bCanUseHorseSkill && true == IsRiding())
		return false;
	...
	{
		if (false == m_SkillUseInfo[dwVnum].UseSkill(bUseGrandMaster, (NULL != pkVictim && SKILL_HORSE_WILDATTACK != dwVnum) ? pkVictim->GetVID() : NULL, ComputeCooltime(iCooltime * 1000), iSplashCount, lMaxHit))
		{
			if (test_server)
...
{
	if (ms < 3 * 60)
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("skill_chart_text_1"));
	else if (ms < 5 * 60)
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("skill_chart_text_2"));
	else if (ms < 10 * 60) 
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("skill_chart_text_3"));
	else if (ms < 30 * 60) 
	{
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("skill_chart_text_4"));
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("skill_chart_text_5"));
	}
	else if (ms < 1 * 3600) 
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("skill_chart_text_6"));
	else if (ms < 2 * 3600) 
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("skill_chart_text_7"));
	else if (ms < 3 * 3600)
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("skill_chart_text_8"));
	else if (ms < 6 * 3600)
	{
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("skill_chart_text_9"));
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("skill_chart_text_10"));
	}
	else if (ms < 12 * 3600)
	{
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("skill_chart_text_11"));
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("skill_chart_text_12"));
	}
	else if (ms < 18 * 3600)
	{
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("skill_chart_text_13"));
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("skill_chart_text_14"));
	}
	else 
	{
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("skill_chart_text_15"));
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("skill_chart_text_16"));
	}
}
...
EVENTINFO(mob_skill_event_info)
{
	...
	mob_skill_event_info() : ch(), pos(), vnum(0), index(0), level(0)
	{ }
};

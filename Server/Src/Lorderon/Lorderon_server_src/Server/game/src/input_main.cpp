...
void SendBlockChatInfo(LPCHARACTER ch, int sec)
{
	if (sec <= 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("input_main_msg_1"));
		return;
	}
	long hour = sec / 3600;
	sec -= hour * 3600;
	...
	char buf[128+1];
	if (hour > 0 && min > 0)
		snprintf(buf, sizeof(buf), LC_TEXT("%d you %d have_chat_block"), hour, min, sec);
	else if (hour > 0 && min == 0)
		snprintf(buf, sizeof(buf), LC_TEXT("%d %d input_main_msg_2"), hour, sec);
	else if (hour == 0 && min > 0)
		snprintf(buf, sizeof(buf), LC_TEXT("%d %d input_main_msg_3"), min, sec);
	else
		snprintf(buf, sizeof(buf), LC_TEXT("%d input_main_msg_4"), sec);
...
int CInputMain::Whisper(LPCHARACTER ch, const char * data, size_t uiBytes)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_get_chat_block"));
		return (iExtraLen);
	}
	...
	else
	{
		...
		else
		{
			...
			if (0!=processReturn)
			{
				if (ch->GetDesc())
				{
					...
					if (pTable)
					{
						...
						int len;
						if (3==processReturn) 
							len = snprintf(buf, sizeof(buf), LC_TEXT("you_cant_warp_in_trade"), pTable->szLocaleName);
						else
							len = snprintf(buf, sizeof(buf), LC_TEXT("%s input_main_msg_5"), pTable->szLocaleName);
			...
			if (buflen > 0)
			{
				...
				pack.bType = bType;
#if defined(ENABLE_RENEW_MESSENGER_WHISPER)
				std::auto_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("SELECT status, location, year, month, day FROM player.player WHERE id = '%u'", ch->GetPlayerID()));
				MYSQL_ROW row = mysql_fetch_row(pMsg->Get()->pSQLResult);
				pack.job = ch->GetJob();
				pack.level = ch->GetLevel();
				pack.empire = ch->GetEmpire();
				strlcpy(pack.guild, (ch->GetGuild()) ? ch->GetGuild()->GetName() : EMPTY_GUILD, sizeof(pack.guild));
				strlcpy(pack.language, LANGUAGE_DEFAULT, sizeof(pack.language));	
				strlcpy(pack.status, row[0], sizeof(pack.status));	
				strlcpy(pack.location, row[1], sizeof(pack.location));
				strlcpy(pack.year, row[2], sizeof(pack.year));
				strlcpy(pack.month, row[3], sizeof(pack.month));
				strlcpy(pack.day, row[4], sizeof(pack.day));
#endif
...
struct FEmpireChatPacket
{
	...
	void operator () (LPDESC d)
	{
		...
		if (d->GetEmpire() == bEmpire || bEmpire == 0 || d->GetCharacter()->GetGMLevel() > GM_PLAYER || d->GetCharacter()->IsEquipUniqueGroup(UNIQUE_GROUP_RING_OF_LANGUAGE))
		{
...
struct FYmirChatPacket
{
	...
	FYmirChatPacket(packet_chat& p, const char* chat, size_t len_chat, const char* name, size_t len_name, int iMapIndex, BYTE empire, bool ring) : packet(p), m_szChat(chat), m_lenChat(len_chat), m_szName(name), m_iMapIndex(iMapIndex), m_bEmpire(empire), m_ring(ring)
	{
	...
	void operator() (LPDESC d)
	{
		...
		if (d->GetCharacter()->GetMapIndex() != m_iMapIndex)
			return;
		if (m_ring || d->GetEmpire() == m_bEmpire || d->GetCharacter()->GetGMLevel() > GM_PLAYER || d->GetCharacter()->IsEquipUniqueGroup(UNIQUE_GROUP_RING_OF_LANGUAGE))
		{
...
int CInputMain::Chat(LPCHARACTER ch, const char * data, size_t uiBytes)
{
	...
	if (0!=processReturn)
	{
		...
		{
			if (3==processReturn)
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("input_main_msg_6"), pTable->szLocaleName);
			else
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s input_main_msg_7"), pTable->szLocaleName);
		}
		return iExtraLen;
	}
	char chatbuf[CHAT_MAX_LEN + 1];
#ifdef ENABLE_CHAT_COLOR_SYSTEM
	static const char* colorbuf[] = {"|cFFffa200|H|h[LORDERON-TEAM]|h|r", "|cFFff0000|H|h[Shinsoo]|h|r", "|cFFffc700|H|h[Chunjo]|h|r", "|cFF000bff|H|h[Jinno]|h|r"};
	...
	if (pinfo->type == CHAT_TYPE_SHOUT)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_muset_be_lvl %d on_use_chat"), g_iShoutLimitLevel);
			return (iExtraLen);
		}
	...
	switch (pinfo->type)
	{
		case CHAT_TYPE_TALKING:
			{
				...
				if (false)
				{
					std::for_each(c_ref_set.begin(), c_ref_set.end(), FYmirChatPacket(pack_chat, buf, strlen(buf), ch->GetName(), strlen(ch->GetName()), ch->GetMapIndex(), ch->GetEmpire(), ch->IsEquipUniqueGroup(UNIQUE_GROUP_RING_OF_LANGUAGE)));
				}
				else
				{
		...
		case CHAT_TYPE_PARTY:
			{
				if (!ch->GetParty())
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_have_grupe"));
				else
				{
					TEMP_BUFFER tbuf;
		...
		case CHAT_TYPE_GUILD:
			{
				if (!ch->GetGuild())
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_have_guild"));
				else
				{
		...
		case MESSENGER_SUBHEADER_CG_ADD_BY_VID:
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("player_not_online"));
					return sizeof(TPacketCGMessengerAddByVID);
				}
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_gm_in_firend"));
					return sizeof(TPacketCGMessengerAddByVID);
				}
				if (ch->GetDesc() == d) 
					return sizeof(TPacketCGMessengerAddByVID);
		...
		case MESSENGER_SUBHEADER_CG_ADD_BY_NAME:
			{
				...
				if (!tch)
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s player_not_online_o"), name);
				else
				{
					if (tch == ch) 
						return CHARACTER_NAME_MAX_LEN;
...
void CInputMain::Exchange(LPCHARACTER ch, const char * data)
{
	...
	if ((to_ch = CHARACTER_MANAGER::instance().Find(pinfo->arg1)))
	{
		...
		{
			to_ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%d you_cant_open_safebox_on_sec"), g_nPortalLimitTime);
			return;
		}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%d you_cant_open_safebox_on_sec"), g_nPortalLimitTime);
		return;
	}
	switch (pinfo->sub_header)
	{
		case EXCHANGE_SUBHEADER_CG_START:	
			if (!ch->GetExchange())
			{
				if ((to_ch = CHARACTER_MANAGER::instance().Find(pinfo->arg1)))
				{
					if (iPulse - ch->GetSafeboxLoadTime() < PASSES_PER_SEC(g_nPortalLimitTime))
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%d you_can_go_back_for"), g_nPortalLimitTime);
						if (test_server)
							ch->ChatPacket(CHAT_TYPE_INFO, "[TEST-SERVER-INFO][Safebox]Pulse %d LoadTime %d PASS %d", iPulse, ch->GetSafeboxLoadTime(), PASSES_PER_SEC(g_nPortalLimitTime));
						return;
					}
					if (iPulse - to_ch->GetSafeboxLoadTime() < PASSES_PER_SEC(g_nPortalLimitTime))
					{
						to_ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%d you_can_go_back_for"), g_nPortalLimitTime);
						if (test_server)
							to_ch->ChatPacket(CHAT_TYPE_INFO, "[TEST-SERVER-INFO][Safebox]Pulse %d LoadTime %d PASS %d", iPulse, to_ch->GetSafeboxLoadTime(), PASSES_PER_SEC(g_nPortalLimitTime));
						return;
					}
					if (ch->GetGold() >= GOLD_MAX)
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("input_main_msg_8"));
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_trade_with_you_trading"));
						return;
					}
		...
		case EXCHANGE_SUBHEADER_CG_ELK_ADD:	
			if (ch->GetExchange())
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("input_main_msg_9 report msg"));
					sys_err("[OVERFLOW_GOLD] ELK_ADD (%lld) id %u name %s ", ch->GetExchange()->GetCompany()->GetOwner()->GetGold(), ch->GetExchange()->GetCompany()->GetOwner()->GetPlayerID(), ch->GetExchange()->GetCompany()->GetOwner()->GetName());
					return;
				}
				if (ch->GetExchange()->GetCompany()->GetAcceptStatus() != true)
	...
	for (int i = 0; i < iCount; ++i, ++e)
	{
		...
		if (fDistWithSyncOwner > fLimitDistWithSyncOwner)
		{
			...
			else
			{
				...
				sys_err( "Too far SyncPosition DistanceWithSyncOwner(%f)(%s) from Name(%s) CH(%d,%d) VICTIM(%d,%d) SYNC(%d,%d)", fDistWithSyncOwner, victim->GetName(), ch->GetName(), ch->GetX(), ch->GetY(), victim->GetX(), victim->GetY(), e->lX, e->lY );
				ch->GetDesc()->SetPhase(PHASE_CLOSE);
				return -1;
			}
		}
		...
		if (tvDiff->tv_sec == 0 && tvDiff->tv_usec < g_lValidSyncInterval)
		{
			...
			{
				LogManager::instance().HackLog( "SYNC_POSITION_HACK", ch );
				sys_err( "Too often SyncPosition Interval(%ldms)(%s) from Name(%s) VICTIM(%d,%d) SYNC(%d,%d)", tvDiff->tv_sec * 1000 + tvDiff->tv_usec / 1000, victim->GetName(), ch->GetName(), victim->GetX(), victim->GetY(), e->lX, e->lY );
				ch->GetDesc()->SetPhase(PHASE_CLOSE);
				return -1;
			}
		}
		...
		{
			LogManager::instance().HackLog( "SYNC_POSITION_HACK", ch );
			sys_err( "Too far SyncPosition Distance(%f)(%s) from Name(%s) CH(%d,%d) VICTIM(%d,%d) SYNC(%d,%d)", fDist, victim->GetName(), ch->GetName(), ch->GetX(), ch->GetY(), victim->GetX(), victim->GetY(), e->lX, e->lY );
			ch->GetDesc()->SetPhase(PHASE_CLOSE);
			return -1;
		}
		else
...
void CInputMain::SafeboxCheckin(LPCHARACTER ch, const char * c_pData)
{
	...
	{
	    ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("input_main_msg_10"));
	    return;
	}
	if (!pkSafebox->IsEmpty(p->bSafePos, pkItem->GetSize()))
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_item_has_not_movable"));
		return;
	}
	if (pkItem->GetVnum() == UNIQUE_ITEM_SAFEBOX_EXPAND)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_item_dont_move_to_safebox"));
		return;
	}
	...
	if (pkItem->IsEquipped())
	{
		...
		if (iWearCell == WEAR_WEAPON)
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("unequip_weapon_costume_full_inv"));
				return;
			}
		}
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("input_main_msg_11"));
		return;
	}
...
void CInputMain::SafeboxCheckout(LPCHARACTER ch, const char * c_pData, bool bMall)
{
	...
	{
#ifdef ENABLE_SPECIAL_STORAGE
		if (DRAGON_SOUL_INVENTORY == p->ItemPos.window_type || UPGRADE_INVENTORY == p->ItemPos.window_type || BOOK_INVENTORY == p->ItemPos.window_type || STONE_INVENTORY == p->ItemPos.window_type)
#else
		if (DRAGON_SOUL_INVENTORY == p->ItemPos.window_type)
#endif
		{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("input_main_msg_13"));
			return;
		}
		pkSafebox->Remove(p->bSafePos);
...
void CInputMain::PartyInviteAnswer(LPCHARACTER ch, const char * c_pData)
{
	if (ch->GetArena())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("input_main_msg_14"));
		return;
	}
	TPacketCGPartyInviteAnswer * p = (TPacketCGPartyInviteAnswer*) c_pData;
	LPCHARACTER pInviter = CHARACTER_MANAGER::instance().Find(p->leader_vid);
	if (!pInviter)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("input_main_msg_15"));
	else if (!p->accept)
...
void CInputMain::PartySetState(LPCHARACTER ch, const char* c_pData)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("input_main_msg_16"));
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("input_main_msg_17"));
		return;
	}
	if (!ch->GetParty()->IsMember(p->pid))
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("input_main_msg_18"));
		return;
	}
	DWORD pid = p->pid;
...
void CInputMain::PartyRemove(LPCHARACTER ch, const char* c_pData)
{
	if (ch->GetArena())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("input_main_msg_19"));
		return;
	}
	if (!CPartyManager::instance().IsEnablePCParty())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("input_main_msg_20"));
		return;
	}
	if (ch->GetDungeon())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("input_main_msg_21"));
		return;
	}
	...
	if (pParty->GetLeaderPID() == ch->GetPlayerID())
	{
		if (ch->GetDungeon())
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("input_main_msg_22"));
		}
		else
		{
			if(pParty->IsPartyInDungeon(351))
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("input_main_msg_23"));
				return;
			}
			...
			else
			{
				...
				if (B)
				{
					B->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_kicked_from_grupe"));
				}
				pParty->Quit(p->pid);
			}
		}
	...
	else
	{
		if (p->pid == ch->GetPlayerID())
		{
			if (ch->GetDungeon())
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_destroy_grupe"));
			}
			else
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("input_main_msg_26"));
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_kick_player_form_grupe"));
		}
	}
}
...
void CInputMain::AnswerMakeGuild(LPCHARACTER ch, const char* c_pData)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("input_main_msg_28"), quest::CQuestManager::instance().GetEventFlag("guild_disband_delay"));
		return;
	}
	if (get_global_time() - ch->GetQuestFlag("guild_manage.new_withdraw_time") < CGuildManager::instance().GetWithdrawDelay())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("guild_can_create_for %d days"), quest::CQuestManager::instance().GetEventFlag("guild_withdraw_delay"));
		return;
	}
	if (ch->GetGuild())
		return;
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_name_canot_use"));
		return;
	}
	DWORD dwGuildID = gm.CreateGuild(cp);
	if (dwGuildID)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("guild_is_success_created"), cp.name);
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_guild_has_exist"));
}
...
void CInputMain::PartyUseSkill(LPCHARACTER ch, const char* c_pData)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("only_leader_grupe_can_use_this"));
		return;
	}
		...
		case PARTY_SKILL_WARP:
			{
				...
				if (pch)
					...
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<target_not_found"));
			}
			break;
	}
}
		...
		case GUILD_SUBHEADER_CG_DEPOSIT_MONEY:
			{
				...
				if (gold < 0)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_have_golds"));
					return SubPacketLen;
				}
				if (ch->GetGold() < gold)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("your_dont_have_golds"));
					return SubPacketLen;
				}
		...
		case GUILD_SUBHEADER_CG_ADD_MEMBER:
			{
				...
				if (!newmember)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_player_have_guild_p"));
					return SubPacketLen;
				}
		...
		case GUILD_SUBHEADER_CG_REMOVE_MEMBER:
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_can_kick_m_in_war"));
					return SubPacketLen;
				}
				...
				if (member)
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_player_non_in_guild"));
						return SubPacketLen;
					}
					if (!pGuild->HasGradeAuth(m->grade, GUILD_AUTH_REMOVE_MEMBER))
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_remove_m_from_guild"));
						return SubPacketLen;
					}
				...
				else
				{
					...
					if (pGuild->RequestRemoveMember(pid))
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("m_has_kicked"));
					else
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_player_have_guild_p"));
				}
			}
		...
		case GUILD_SUBHEADER_CG_CHANGE_GRADE_NAME:
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("youd_dont_have_perm"));
				}
				else if (*c_pData == GUILD_LEADER_GRADE)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("guild_sort_not_changet_by_leader"));
				}
				else if (!check_name(gradename))
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_use_specialchars"));
				}
				else
				{
		...
		case GUILD_SUBHEADER_CG_CHANGE_GRADE_AUTHORITY:
			{
				...
				if (m->grade != GUILD_LEADER_GRADE)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_have_perm_to_admin"));
				}
				else if (*c_pData == GUILD_LEADER_GRADE)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("perms_cant_changed"));
				}
				else
				{
		...
		case GUILD_SUBHEADER_CG_OFFER:
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_guild_has_reached_mag_lvl"));
				}
				else
				{
					offer /= 100;
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_depos_bzk"), offer);
					}
					else
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("bzk_cant_dep"));
					}
				}
			}
			return SubPacketLen;
		case GUILD_SUBHEADER_CG_CHARGE_GSP:
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("guild_dont_have_gold"));
					return SubPacketLen;
				}
				if (!pGuild->ChargeSP(ch, offer))
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("dragon_god_not_helped_guild"));
				}
			}
			return SubPacketLen;
		case GUILD_SUBHEADER_CG_POST_COMMENT:
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_send_guild_msg"));
				}
				else
				{
		...
		case GUILD_SUBHEADER_CG_CHANGE_MEMBER_GRADE:
			{
				...
				if (m->grade != GUILD_LEADER_GRADE)
					...
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("guild_pos_cant_change"));
				else if (grade == 1)
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_change_leader"));
				else
		...
		case GUILD_SUBHEADER_CG_CHANGE_MEMBER_GENERAL:
			{
				...
				if (m->grade != GUILD_LEADER_GRADE)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_select_guild_leader"));
				}
				else
				{
					if (!pGuild->ChangeMemberGeneral(pid, is_general))
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_select_other_m_to_leader"));
					}
				}
			}
			return SubPacketLen;
...
void CInputMain::ItemGive(LPCHARACTER ch, const char* c_pData)
{
	...
	if (to_ch)
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_item_is_not_tradeble_exchange"));
}
...
{	
	...
	if (uiBytes < sizeof(TPacketCGMyShop) + iExtraLen)
 	...
 	if (ch->GetGold() >= GOLD_MAX)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_have_gold_limit"));
...
void CInputMain::Refine(LPCHARACTER ch, const char* c_pData)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO,  LC_TEXT("you_have_open_shop"));
		ch->ClearRefineMode();
		return;
	}
	if (p->type == 255)
	...
	else if (p->type == REFINE_TYPE_MONEY_ONLY)
	{
		...
		if (NULL != item)
		{
			...
			else
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "refine_items_tower_msg");
				}
			}
		}
	}
...
#endif
#if defined(ENABLE_RENEW_MESSENGER_WHISPER)
		case HEADER_CG_CONTACT_PROFILE_MANAGER:
			CProfileManager::instance().Manager(ch, c_pData);
			break;
#endif
#ifdef ENABLE_SHOW_CHEST_DROP

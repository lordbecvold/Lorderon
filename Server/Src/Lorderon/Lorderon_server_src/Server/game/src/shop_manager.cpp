...
bool CShopManager::StartShopping(LPCHARACTER pkChr, LPCHARACTER pkChrShopKeeper, int iShopVnum)
{
	...
	{
		pkChr->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("shop_manager_msg_1"));
		return false;
	}
...
#endif
{
	...
	if (0 != g_BuySellTimeLimitValue)
	{
		if (get_dword_time() < ch->GetLastBuySellTime()+g_BuySellTimeLimitValue)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("shop_manager_msg_2"));
			return;
		}
	}
	ch->SetLastBuySellTime(get_dword_time());
	...
	if (DISTANCE_APPROX(ch->GetX()-ch->GetShopOwner()->GetX(), ch->GetY()-ch->GetShopOwner()->GetY())>2000)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("shop_mgr_msg_3"));
		return;
	}
#ifdef ENABLE_SPECIAL_STORAGE
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("shop_manager_msg_4"));
		return;
	}
	...
	if (GOLD_MAX <= nTotalMoney)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("shop_manager_msg_a"));
		return;
	}
	sys_log(0, "SHOP: SELL: %s item name: %s(x%d):%u price: %lld", ch->GetName(), item->GetName(), bCount, item->GetID(), dwPrice);
	if (iVal > 0)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("shop_manager %d %% msg_8"), iVal);

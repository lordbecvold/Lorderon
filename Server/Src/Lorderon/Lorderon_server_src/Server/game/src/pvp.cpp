...
void CPVPManager::Insert(LPCHARACTER pkChr, LPCHARACTER pkVictim)
{
	...
	if ((pkPVP = Find(kPVP.m_dwCRC)))
	{
		...
		{
			pkVictim->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s duel_started_p"), pkChr->GetName());
			pkChr->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s duel_started_p"), pkVictim->GetName());
		}
		return;
	}
	...
	snprintf(msg, sizeof(msg), LC_TEXT("%s pvp_msg"), pkChr->GetName());
	pkVictim->ChatPacket(CHAT_TYPE_INFO, msg);
	pkChr->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s pvp_msg_f"), pkVictim->GetName());

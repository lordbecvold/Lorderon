...
{
	DWORD dwMapIndex;
	regen_mob_event_info() : dwMapIndex( 0 )
	{ }
};
...
void CThreeWayWar::onDead(LPCHARACTER pChar, LPCHARACTER pkKiller)
{
	...
	{
		char szBuf[64 + 1];
		snprintf(szBuf, sizeof(szBuf), LC_TEXT("points:%d points_1:%d points_2:%d"), GetKillScore(1), GetKillScore(2), GetKillScore(3));
	...
	if (0 == GetRegenFlag())
	{
		...
		const std::string Script(LC_TEXT("you_is_devil") + Nation + LC_TEXT("war_e_info_1") + "[ENTER][DONE]");
		CHARACTER_MANAGER::instance().SendScriptToMap(pChar->GetMapIndex(), Script);
		char szNotice[512+1];
		snprintf(szNotice, sizeof(szNotice), LC_TEXT("empier %s destroy"), Nation.c_str());
	...
	else if (1 == GetRegenFlag())
	{
		...
		if (NULL != pSecMap)
		{
			...
			const std::string Script(EmpireName + LC_TEXT("war_info_king") + "[ENTER][DONE]");
		...
		for (int n = 0; n < quest::CQuestManager::instance().GetEventFlag("threeway_war_boss_count");)
		{
			...
			if (y < 0)
				y = pChar->GetY();
			LPCHARACTER ch = CHARACTER_MANAGER::instance().SpawnMob( GetEventSungZiMapInfo().m_iBossMobVnum, pChar->GetMapIndex(), x, y, 0, false);
			if (NULL != ch)
			{
				ch->SetAggressive();
				++n;
...
const char* GetSungziMapPath()
{
	...
	snprintf(s_szMapPath, sizeof(s_szMapPath), "%s/map/%s/", LocaleService_GetBasePath().c_str(), CThreeWayWar::instance().GetEventSungZiMapInfo().m_stMapName.c_str());
	return s_szMapPath;
}
...
const char* GetPassMapPath( BYTE bEmpire )
{
	...
	{
		static char s_szMapPath[128];
		snprintf(s_szMapPath, sizeof(s_szMapPath), "%s/map/%s/", LocaleService_GetBasePath().c_str(), CThreeWayWar::instance().GetEventPassMapInfo().m_stMapName[bEmpire-1].c_str());
		return s_szMapPath;
	}
	return NULL;
}

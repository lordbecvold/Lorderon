...
void AuctionManager::enroll_auction (LPCHARACTER ch, LPITEM item, BYTE empire, int bidPrice, int immidiatePurchasePrice)
{
	...
	if (item->IsEquipped())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "auction_manager_msg_1");
		return;
	}
	if (GetAuctionItemInfo (item->GetID()))
	{
		sys_err ("Item %d is already in auction.", item->GetID());
		ch->ChatPacket(CHAT_TYPE_INFO, "auction_manager_msg_2");
		return;
	}
	if (item->GetWindow() == AUCTION)
	{
		sys_err ("Item %d is already in auction.", item->GetID());
		ch->ChatPacket(CHAT_TYPE_INFO, "auction_manager_msg_3");
		return;
	}
...
void AuctionManager::enroll_sale (LPCHARACTER ch, LPITEM item, DWORD wisher_id, int salePrice)
{
	...
	if (item->IsEquipped())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "auction_manager_msg_4");
		return;
	}
	if (GetSaleItemInfo (item->GetID()))
	{
		sys_err ("Item %d is already in auction.", item->GetID());
		ch->ChatPacket(CHAT_TYPE_INFO, "auction_manager_msg_5");
		return;
	}
	if (item->GetWindow() == AUCTION)
	{
		sys_err ("Item %d is already in auction.", item->GetID());
		ch->ChatPacket(CHAT_TYPE_INFO, "auction_manager_msg_6");
		return;
	}
...
void AuctionManager::bid (LPCHARACTER ch, DWORD item_id, int bid_price)
{
	...
	if (mb.first != -1)
	{
		ch->ChatPacket (CHAT_TYPE_INFO, "auction_manager_msg_7");
	}
	if (ch->GetGold() < bid_price)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "auction_manager_msg_8");
		return;
	}
...
void AuctionManager::immediate_purchase (LPCHARACTER ch, DWORD item_id)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "auction_manager_msg_9");
		return;
	}
	if (ch->GetGold() < item_info->get_impur_price())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "auction_manager_msg_10");
		return;
	}
...
void AuctionManager::get_auctioned_item (LPCHARACTER ch, DWORD item_id, DWORD item_num)
{
	...
	if (pos == -1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "auction_manager_msg_11");
		return;
	}
...
void AuctionManager::rebid (LPCHARACTER ch, DWORD item_id, int bid_price)
{
	...
	if (lock)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "auction_manager_msg_12");
		return;
	}
	if (ch->GetGold() + money < bid_price)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "auction_manager_msg_13");
		return;
	}
...
void AuctionManager::bid_cancel (LPCHARACTER ch, DWORD item_id)
{
	...
	if (lock)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "auction_manager_msg_14");
		return;
	}
	TAuctionItemInfo* item_info = GetAuctionItemInfo(item_id);
	if (item_info->get_bidder_id() == ch->GetPlayerID())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "auction_manager_msg_15");
		return;
	}
...
void AuctionManager::recv_result_auction (DWORD commander_id, TPacketDGResultAuction* cmd_result)
{
		...
		case AUCTION_FAIL:
			{
				if (ch != NULL)
				{
					...
					ch->ChatPacket(CHAT_TYPE_INFO, "auction_manager_msg_16");
				}
				break;
			}
		}
		...
		case AUCTION_SUCCESS:
			{
				...
				if (ch != NULL)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "auction_manager_msg_17");
				}
				break;
			}
		case AUCTION_FAIL:
			{
				if (ch != NULL)
				{
					...
					LPITEM item = ITEM_MANAGER::instance().CreateItem(player_item->vnum, player_item->count, player_item->id);
					ch->AutoGiveItem (item, true);
					ch->ChatPacket(CHAT_TYPE_INFO, "auction_manager_msg_18");
				}
				break;
			}
		}
		...
		case AUCTION_SUCCESS:
			{
				...
				if (ch != NULL)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "auction_manager_msg_19");
				}
				break;
			}
		case AUCTION_FAIL:
			{
				if (ch != NULL)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "auction_manager_msg_20");
				}
				break;
			}
		}
	...
	case AUCTION_BID:
		...
		else
		{
			...
			if (ch != NULL)
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "auction_manager_msg_21");
			}
		}
		break;
	case AUCTION_IMME_PUR:
		...
		else
		{
			...
			if (ch != NULL)
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "auction_manager_msg_22");
			}
		}
		break;
	...
	case AUCTION_CANCEL_SALE:
		...
		else
		{
			...
			if (ch != NULL)
			{
				...
				ch->AutoGiveItem (item, true);
				ch->ChatPacket(CHAT_TYPE_INFO, "auction_manager_msg_23");
	...
	case AUCTION_CANCEL_WISH:
		...
		else
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "auction_manager_msg_24");
			}
		}
		break;
	case AUCTION_REBID:

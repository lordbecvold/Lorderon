...
void Initialize()
{
	...
	snprintf(szFishingFileName, sizeof(szFishingFileName), "%s/files/fishing.txt", LocaleService_GetBasePath().c_str());
	...
	{
		sys_log(0, "FISH: %-24s vnum %5lu prob %4d %4d %4d %4d len %d %d %d", fish_info[i].name, fish_info[i].vnum, fish_info[i].prob[0], fish_info[i].prob[1], fish_info[i].prob[2], fish_info[i].prob[3], fish_info[i].length_range[0], fish_info[i].length_range[1], fish_info[i].length_range[2]);
	}
...
void FishingPractice(LPCHARACTER ch)
{
	...
	if (rod && rod->GetType() == ITEM_ROD)
	{
		if ( rod->GetRefinedVnum()>0 && rod->GetSocket(0) < rod->GetValue(2) && number(1,rod->GetValue(1))==1 )
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_points_plus (%d/%d)"),rod->GetSocket(0), rod->GetValue(2));
			if (rod->GetSocket(0) == rod->GetValue(2))
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_rod_is_not_refinable"));
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_fishing_rod_must_be_refined_go_fisherman"));
			}
		}
	}
	rod->SetSocket(2, 0);
...
int GetFishLength(int fish_id)
{
	if (number(0,99))
	{
		return (int)(fish_info[fish_id].length_range[0] + (fish_info[fish_id].length_range[1] - fish_info[fish_id].length_range[0]) * (number(0,2000)+number(0,2000)+number(0,2000)+number(0,2000)+number(0,2000))/10000);
	}
	else
	{
		return (int)(fish_info[fish_id].length_range[1] + (fish_info[fish_id].length_range[2] - fish_info[fish_id].length_range[1]) * 2 * asin(number(0,10000)/10000.) / M_PI);
	}
}
			...
			case -1: 
				{
					int map_idx = ch->GetMapIndex();
					int prob_idx = GetProbIndexByMapIndex(map_idx);
					LogManager::instance().FishLog( ch->GetPlayerID(), prob_idx, info->fish_id, GetFishingLevel(ch), ms);
				}
				FishingFail(ch);
				break;
			case 0:
				if (item_vnum)
				{
					...
					if (item)
					{
						...
						if (test_server)
						{
							ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("fish_size_is %.2fcm"), item->GetSocket(0)/100.f);
						}
						if (quest::CQuestManager::instance().GetEventFlag("fishevent") > 0 && (info->fish_id == 5 || info->fish_id == 6))
						{
							...
							if (info->fish_id == 5)
							{
								strlcpy(p.szBoard, LC_TEXT("event_fish_1"), sizeof(p.szBoard));
							}
							else if (info->fish_id == 6)
							{
								strlcpy(p.szBoard, LC_TEXT("event_fish_2"), sizeof(p.szBoard));
							}
					...
					int map_idx = ch->GetMapIndex();
					int prob_idx = GetProbIndexByMapIndex(map_idx);
					LogManager::instance().FishLog(ch->GetPlayerID(), prob_idx, info->fish_id, GetFishingLevel(ch), ms, true, item ? item->GetSocket(0) : 0);
				}
				else
				{
					int map_idx = ch->GetMapIndex();
...
void Simulation(int level, int count, int prob_idx, LPCHARACTER ch)
{
	...
	for (std::map<std::string,int>::iterator it = fished.begin(); it != fished.end(); ++it)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s : %d player"), it->first.c_str(), it->second);
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%d catch %d types"), fished.size(), total_count);
}
	...
	else
	{
		...
		switch (fish_info[idx].used_table[u_index])
		{
			...
			case USED_WATER_STONE:	
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("fish_lave_in_wather"));
				break;
			case USED_SHELLFISH:	
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_can_open_fish"));
				ch->AutoGiveItem(SHELLFISH_VNUM);
				break;
			case USED_EARTHWARM:	
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("fishing_worm_is_avilible"));
...
void Grill(LPCHARACTER ch, LPITEM item)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s fired"), item->GetName());
	item->SetCount(0);
...
int main(int argc, char **argv)
{
	...
	for (int i = 0; i < MAX_FISH; ++i)
	{
		printf("%s\t%u\t%u\t%u\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d", fish_info[i].name, fish_info[i].vnum, fish_info[i].dead_vnum, fish_info[i].grill_vnum, fish_info[i].prob[0], fish_info[i].prob[1], fish_info[i].prob[2], fish_info[i].difficulty, fish_info[i].time_type, fish_info[i].length_range[0], fish_info[i].length_range[1], fish_info[i].length_range[2]);

...
namespace quest
{
	...
	CQuestManager::CQuestManager() : m_pSelectedDungeon(NULL), m_dwServerTimerArg(0), m_iRunningEventIndex(0), L(NULL), m_bNoSend (false), m_CurrentRunningState(NULL), m_pCurrentCharacter(NULL), m_pCurrentNPCCharacter(NULL), m_pCurrentPartyMember(NULL), m_pCurrentPC(NULL),  m_iCurrentSkin(0), m_bError(false), m_pOtherPCBlockRootPC(NULL)
	{ }
	...
	void CQuestManager::QuestInfo(unsigned int pc, unsigned int quest_index)
	{
		...
		if ((pPC = GetPC(pc)))
		{
			if (!CheckQuestLoaded(pPC))
			{
				...
				if (ch)
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("game_exiting_you_wait"));
				return;
			}
			m_bQuestInfoFlag = 1;
	...
	bool CQuestManager::Click(unsigned int pc, LPCHARACTER pkChrTarget)
	{
		...
		if (pPC)
		{
			...
			if (pkChrTarget->IsNPC())
			{
				...
				if (it == m_mapNPC.end())
				{
					sys_log(0, "CQuestManager::Click(pid=%d, target_npc_name=%s) - NOT EXIST NPC RACE VNUM[%d]", pc, pkChrTarget->GetName(), dwCurrentNPCRace); 
					return false;
				}

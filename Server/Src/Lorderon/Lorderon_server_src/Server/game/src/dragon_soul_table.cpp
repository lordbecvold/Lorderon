...
bool DragonSoulTable::ReadDragonSoulTableFile(const char * c_pszFileName)
{
	...
	if (CheckApplyNumSettings() && CheckWeightTables() && CheckRefineGradeTables() && CheckRefineStepTables() && CheckRefineStrengthTables() && CheckDragonHeartExtTables() && CheckDragonSoulExtTables())
	{
		return true;
	}
	else
	...
	else
	{
		for (size_t i = 0; i < m_vecDragonSoulTypes.size(); i++)
		{
			for (int j = 0; j < DRAGON_SOUL_GRADE_MAX; j++)
			{
				...
				{
					sys_err ("In %s group of ApplyNumSettings, values in Grade(%s) row is invalid.", m_vecDragonSoulNames[i].c_str(), g_astGradeName[j].c_str());
					return false;
				}
			}
		}
	...
	else
	{
		for (size_t i = 0; i < m_vecDragonSoulTypes.size(); i++)
		{
			for (int j = 0; j < DRAGON_SOUL_GRADE_MAX - 1; j++)
			{
				...
				{
					sys_err ("In %s group of RefineGradeTables, values in Grade(%s) row is invalid.", m_vecDragonSoulNames[i].c_str(), g_astGradeName[j].c_str());
					return false;
				}
				if (need_count < 1)
				{
					sys_err ("In %s group of RefineGradeTables, need_count of Grade(%s) is less than 1.", m_vecDragonSoulNames[i].c_str(), g_astGradeName[j].c_str());
					return false;
				}
				if (fee < 0)
				{
					sys_err ("In %s group of RefineGradeTables, fee of Grade(%s) is less than 0.", m_vecDragonSoulNames[i].c_str(), g_astGradeName[j].c_str());
					return false;
				}
	...
	for (size_t i = 0; i < m_vecDragonSoulTypes.size(); i++)
	{
		for (int j = MATERIAL_DS_REFINE_NORMAL; j <= MATERIAL_DS_REFINE_HOLLY; j++)
		{
			...
			for (int k = 0; k < DRAGON_SOUL_STRENGTH_MAX -1; k++)
			{
				...
				{
					sys_err ("In %s group of RefineStrengthTables, value(Material(%s), Strength(%d)) or fee are invalid.", m_vecDragonSoulNames[i].c_str(), g_astMaterialName[j].c_str(), k);
					return false;
				}
				if (fee < 0)
				{
					sys_err ("In %s group of RefineStrengthTables, fee of Material(%s) is less than 0.", m_vecDragonSoulNames[i].c_str(), g_astMaterialName[j].c_str());
					return false;
				}
				if (prob < 0.f)
				{
					sys_err ("In %s group of RefineStrengthTables, probability(Material(%s), Strength(%d)) is less than 0.", m_vecDragonSoulNames[i].c_str(), g_astMaterialName[j].c_str(), k);
					return false;
				}
			}
		}
...
bool DragonSoulTable::CheckDragonHeartExtTables()
{
	...
	for (size_t i = 0; i < m_vecDragonSoulTypes.size(); i++)
	{
		for (int j = 0; j < DRAGON_SOUL_GRADE_MAX; j++)
		{
			...
			{
				sys_err ("In %s group of DragonHeartExtTables, CHARGING row or Grade(%s) row are invalid.", m_vecDragonSoulNames[i].c_str(), g_astGradeName[j].c_str());
				return false;
			}
			if (vec_chargings.size() != vec_probs.size())
			{
				sys_err ("In %s group of DragonHeartExtTables, CHARGING row size(%d) are not equal Grade(%s) row size(%d).", m_vecDragonSoulNames[i].c_str(), vec_chargings.size(), vec_probs.size());
				return false;
			}
			for (size_t k = 0; k < vec_chargings.size(); k++)
			{
				...
				{
					sys_err ("In %s group of DragonHeartExtTables, CHARGING value(index : %d) is less than 0", m_vecDragonSoulNames[i].c_str(), k);
					return false;
				}
			}
			for (size_t k = 0; k < vec_probs.size(); k++)
			{
				if (vec_probs[k] < 0.f)
				{
					sys_err ("In %s group of DragonHeartExtTables, Probability(Grade : %s, index : %d) is less than 0", m_vecDragonSoulNames[i].c_str(), g_astGradeName[j].c_str(), k);
					return false;
				}
			}
		}
...
bool DragonSoulTable::CheckDragonSoulExtTables()
{
	...
	for (size_t i = 0; i < m_vecDragonSoulTypes.size(); i++)
	{
		for (int j = 0; j < DRAGON_SOUL_GRADE_MAX; j++)
		{
			...
			{
				sys_err ("In %s group of DragonSoulExtTables, Grade(%s) row is invalid.", m_vecDragonSoulNames[i].c_str(), g_astGradeName[j].c_str());
				return false;
			}
			if (prob < 0.f)
			{
				sys_err ("In %s group of DragonSoulExtTables, Probability(Grade : %s) is less than 0", m_vecDragonSoulNames[i].c_str(), g_astGradeName[j].c_str());
				return false;
			}
			if (0 != by_product && NULL == ITEM_MANAGER::instance().GetTable(by_product))
			{
				sys_err ("In %s group of DragonSoulExtTables, ByProduct(%d) of Grade %s is not exist.", m_vecDragonSoulNames[i].c_str(), by_product, g_astGradeName[j].c_str());
				return false;
			}
		}
	}
...
bool DragonSoulTable::GetRefineStrengthValues(BYTE ds_type, BYTE material_type, BYTE strength_idx, OUT int& fee, OUT float& prob)
{
	...
	{
		sys_err ("Invalid fee. DragonSoulGroup(%s) Material(%s)", stDragonSoulName.c_str(), g_astMaterialName[material_type].c_str());
		return false;
	}
	...
	{
		sys_err ("Invalid prob. DragonSoulGroup(%s) Material(%s) Strength(%d)", stDragonSoulName.c_str(), g_astMaterialName[material_type].c_str(), strength_idx);
		return false;
	}
	return true;
}
...
bool DragonSoulTable::GetDragonHeartExtValues(BYTE ds_type, BYTE grade_idx, OUT std::vector<float>& vec_chargings, OUT std::vector<float>& vec_probs)
{
	...
	if (n != m)
	{
		sys_err ("Invalid row size(%d). It must be same CHARGING row size(%d). DragonSoulGroup(%s) Grade(%s)", m, n, stDragonSoulName.c_str(), g_astGradeName[grade_idx].c_str());
		return false;
	}
	vec_probs.resize(m);
	for (int i = 0; i < m; i++)
...
bool DragonSoulTable::GetDragonSoulExtValues(BYTE ds_type, BYTE grade_idx, OUT float& prob, OUT DWORD& by_product)
{
	...
	{
		sys_err ("Invalid Prob. DragonSoulGroup(%s) Grade(%s)", stDragonSoulName.c_str(), g_astGradeName[grade_idx].c_str(), g_astGradeName[grade_idx].c_str());
		return false;
	}
	if (!m_pDragonSoulExtTableNode->GetGroupValue(stDragonSoulName, g_astGradeName[grade_idx], "byproduct", by_product))
	{
		sys_err ("Invalid fee. DragonSoulGroup(%s) Grade(%d)", stDragonSoulName.c_str(), g_astGradeName[grade_idx].c_str(), grade_idx);
		return false;
	}
	return true;
}

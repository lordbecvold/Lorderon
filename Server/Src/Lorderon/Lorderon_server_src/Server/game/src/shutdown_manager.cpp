	...
	if(!m_bShutdownAlarm)	
	{
		if(m_pTime->tm_hour >= 23 && m_pTime->tm_min >= 50)
		{
			for(std::list<LPDESC>::iterator i = m_lstDesc.begin(); i != m_lstDesc.end(); i++)	
			{
				if((*i) != NULL)
				{
					(*i)->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("shutdown_manager_msg_"));
				}
			}
...
bool CShutdownManager::CheckCorrectSocialID(const char* szSocialID)
{
	...
	int nSum = CharToInt(szSocialID[0]) * 2 + CharToInt(szSocialID[1]) * 3 + CharToInt(szSocialID[2]) * 4 + CharToInt(szSocialID[3]) * 5 + CharToInt(szSocialID[4]) * 6 + CharToInt(szSocialID[5]) * 7 + CharToInt(szSocialID[6]) * 8 + CharToInt(szSocialID[7]) * 9 + CharToInt(szSocialID[8]) * 2 + CharToInt(szSocialID[9]) * 3 + CharToInt(szSocialID[10]) * 4 + CharToInt(szSocialID[11]) * 5;

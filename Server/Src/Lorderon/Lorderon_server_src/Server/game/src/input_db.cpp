...
EVENTFUNC(fix_shop_event)
{
	...
	if (pRes->uiNumRows>0)
	{
		...
		while ((row = mysql_fetch_row(pRes->pSQLResult)) != NULL)
		{
			...
			char query[1024];
			sprintf(query, "INSERT INTO player_gift SET owner_id=%d,vnum=%d,count=%d,reason='%s',`from`=replace(\"%s\",' ','_'),status='WAIT',date_add=NOW()", pid, vnum,count, LC_TEXT("SHOP_REASON"), row[3]);
...
void CInputDB::Boot(const char* data)
{
	...
	snprintf(szCommonDropItemFileName, sizeof(szCommonDropItemFileName), "%s/files/common_drop_item.txt", LocaleService_GetBasePath().c_str());
	snprintf(szETCDropItemFileName, sizeof(szETCDropItemFileName), "%s/files/etc_drop_item.txt", LocaleService_GetBasePath().c_str());
	snprintf(szMOBDropItemFileName, sizeof(szMOBDropItemFileName), "%s/files/mob_drop_item.txt", LocaleService_GetBasePath().c_str());
	snprintf(szSpecialItemGroupFileName, sizeof(szSpecialItemGroupFileName), "%s/files/special_item_group.txt", LocaleService_GetBasePath().c_str());
	snprintf(szDropItemGroupFileName, sizeof(szDropItemGroupFileName), "%s/files/drop_item_group.txt", LocaleService_GetBasePath().c_str());
	snprintf(szMapIndexFileName, sizeof(szMapIndexFileName), "%s/index", LocaleService_GetMapPath().c_str());
	snprintf(szItemVnumMaskTableFileName, sizeof(szItemVnumMaskTableFileName), "%s/files/ori_to_new_table.txt", LocaleService_GetBasePath().c_str());
	snprintf(szDragonSoulTableFileName, sizeof(szDragonSoulTableFileName), "%s/files/dragon_soul_table.txt", LocaleService_GetBasePath().c_str());
...
void CInputDB::SafeboxLoad(LPDESC d, const char * c_pData)
{
	...
	{
		d->GetCharacter()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("exchange_window_is_open" ) );
		d->GetCharacter()->CancelSafeboxLoad();
		return;
	}
	if (d->GetCharacter()->GetPremiumRemainSeconds(PREMIUM_SAFEBOX) > 0 || d->GetCharacter()->IsEquipUniqueGroup(UNIQUE_GROUP_LARGE_SAFEBOX)) bSize = 3;
...
void CInputDB::SafeboxChangePasswordAnswer(LPDESC d, const char* c_pData)
{
	...
	if (p->flag)
	{
		d->GetCharacter()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_item_has_not_movable"));
	}
	else
	{
		d->GetCharacter()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_typed_wrong_password"));
	}
}
...
void CInputDB::BillingExpire(const char * c_pData)
{
	...
	else
	{
		if ((p->dwRemainSeconds - d->GetBillingExpireSecond()) > 60)
		{
			...
			if (ch)
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_play_item_is %d min_in_game"), (p->dwRemainSeconds / 60));
		}
	}
}
...
void CInputDB::AddMonarchMoney(LPDESC d, const char * data )
{
	...
	{
		if (number(1, 100) > 95)
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s have %d"), EMPIRE_NAME(Empire), CMonarch::instance().GetMoney(Empire));
	}
}
...
void CInputDB::TakeMonarchMoney(LPDESC d, const char * data)
{
	...
	if (!CMonarch::instance().DecMoney(Money, Empire))
	{
		...
		if (!d->GetCharacter())
			return;
		LPCHARACTER ch = d->GetCharacter();
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_have_gold_8"));
	}
}
...
{
	char notice[256];
	snprintf(notice, sizeof(notice), LC_TEXT("%s has %s changed"), EMPIRE_NAME(info->bEmpire), info->szName);
	SendNotice(notice);
}

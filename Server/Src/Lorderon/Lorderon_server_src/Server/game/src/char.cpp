...
void CHARACTER::OpenMyShop(const char * c_pszSign, TShopItemTable * pTable, BYTE bItemCount)
{
	if (!CanHandleItem()) 
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("char_msg_1"));
		return;
	}
#ifndef ENABLE_OPEN_SHOP_WITH_ARMOR
	if (GetPart(PART_MAIN) > 2)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("open_shop_with_armor"));
		return;
	}
#endif
	if (GetMyShop())	
	...
	if (GOLD_MAX <= nTotalMoney)
	{
		...
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("shop_yang_cancel"));
		return;
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_open_private_shop_banword"));
		return;
	}
	...
	for (BYTE i = 0; i < bItemCount; ++i)
	{
		...
		if (pkItem)
		{
			...
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_sell_item_in_private_shop"));
				return;
			}
			if (pkItem->IsEquipped() == true)
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_sell_equiped_item"));
				return;
			}
			if (true == pkItem->isLocked())
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_sell_locked_item"));
				return;
			}
...
void CHARACTER::OpenMyShop(const char * c_pszSign, TShopItemTable * pTable, BYTE bItemCount, DWORD id)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("char_shop_msg_2"));
		return;
	}
	if (GetMyShop())
	{
	...
	if (m_mapshops.size() >= (gShopMax ? gShopMax : 10)) //Max shop count.
	{
	...
	{
		sys_err("[OVERFLOW_GOLD] Overflow (GOLD_MAX) id %u name %s", GetPlayerID(), GetName());
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("yang_limit_msg_shop"));
		return;
	}
#endif
	quest::PC * pPC = quest::CQuestManager::instance().GetPCForce(GetPlayerID());
		...
		switch (pos)
		{
			case POS_FIGHTING:
				if (!IsState(m_stateBattle))
					MonsterLog("[BATTLE] char_log_1");
				GotoState(m_stateBattle);
				break;

			default:
				if (!IsState(m_stateIdle))
					MonsterLog("[IDLE] char_log_2");
				GotoState(m_stateIdle);
				break;
		}
	}
...
bool CHARACTER::Goto(long x, long y)
{
	...
	if (!IsState(m_stateMove))
	{
		MonsterLog("[MOVE] %s", GetVictim() ? "char_log_4" : "char_log_5");
	...
	switch (type)
	{
		...
		case POINT_LEVEL:
			...
			switch (val)
			{
				...
				case 100:	case 105:	case 110:	case 115:	case 120:
					char szNoticeLevelUp[QUERY_MAX_LEN];
					snprintf(szNoticeLevelUp, sizeof(szNoticeLevelUp), "<Lorderon> [CH%d]: [%s] doexpil level [%u]. Gratulujeme!", g_bChannel, GetName(), GetLevel()); 
...
void CHARACTER::mining_cancel()
{
	if (m_pkMiningEvent)
	{
		...
		event_cancel(&m_pkMiningEvent);
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("MINING_CANCEL"));
	}
}
...
void CHARACTER::mining(LPCHARACTER chLoad)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Buy_pickaxe"));
		return;
	}
...
void CHARACTER::fishing()
{
	...
	}
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_fishing_here"));
			return;
		}
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("equip_worm"));
		return;
	}
	if (0 == rod->GetSocket(2))
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("worm_equiped_in_fishing_rod"));
		return;
	}
	float fx, fy;
...
void CHARACTER::fishing_take()
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_catch_fish"));
	}
	event_cancel(&m_pkFishingEvent);
}
...
bool CHARACTER::RequestToParty(LPCHARACTER leader)
{
	...
	if (!leader)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("grupe_leader_is_offline"));
		return false;
	}
	...
	switch (errcode)
	{
		case PERR_NONE:
			break;
		case PERR_SERVER:
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("server_cant_work_party_invite"));
			return false;
		case PERR_DIFFEMPIRE:
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_invite_other_emipers"));
			return false;
		case PERR_DUNGEON:
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_invite_players_form_tower"));
			return false;
		case PERR_OBSERVER:
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_invite_grupe"));
			return false;
		case PERR_LVBOUNDARY:
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("grupe_level_limit_reached"));
			return false;
		case PERR_LOWLEVEL:
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_inv_level_30_grupe"));
			return false;
		case PERR_HILEVEL:
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_have_level_30_grupe"));
			return false;
		case PERR_ALREADYJOIN:
			return false;
		case PERR_PARTYISFULL:
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("grupe_has_full"));
			return false;
		default:
	...
	ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s send_party_request"), leader->GetName());
	return true;
}
...
void CHARACTER::AcceptToParty(LPCHARACTER member)
{
	...
	event_cancel(&member->m_pkPartyRequestEvent);
	if (!GetParty())
		member->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_pick_this_item"));
	else
	{
		...
		switch (errcode)
		{
			...
			case PERR_SERVER: member->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("server_cant_work_party_invite")); break;
			case PERR_DUNGEON: member->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_invite_players_form_tower")); break;
			case PERR_OBSERVER: member->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_invite_grupe")); break;
			case PERR_LVBOUNDARY: member->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("grupe_level_limit_reached")); break;
			case PERR_LOWLEVEL: member->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_inv_level_30_grupe")); break;
			case PERR_HILEVEL: member->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_have_level_30_grupe")); break;
			case PERR_ALREADYJOIN: break;
			case PERR_PARTYISFULL: 
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("grupe_has_full"));
				member->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("grupe_has_full"));
				break;
			}
...
void CHARACTER::PartyInvite(LPCHARACTER pchInvitee)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_have_perm_to_invite"));
		return;
	}
	else if (pchInvitee->IsBlockMode(BLOCK_PARTY_INVITE))
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("player %s dont_accept_grupe_inv"), pchInvitee->GetName());
		return;
	}
#ifdef ENABLE_PVP_ADVANCED
	...
	switch (errcode)
	{
		...
		case PERR_HILEVEL:
			...
			return;
		case PERR_ALREADYJOIN:
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("grupe_destroy"), pchInvitee->GetName());
			return;
		case PERR_PARTYISFULL:
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("grupe_has_full"));
			return;
...
void CHARACTER::PartyInviteAccept(LPCHARACTER pchInvitee)
{
	...
	switch (errcode)
	{
		case PERR_NONE:
			break;
		case PERR_SERVER:
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("server_cant_work_party_invite"));
			return;
		case PERR_DUNGEON:
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_invite_players_form_tower"));
			return;
		case PERR_OBSERVER:
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_invite_grupe"));
			return;
		case PERR_LVBOUNDARY:
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("grupe_level_limit_reached"));
			return;
		case PERR_LOWLEVEL:
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_inv_level_30_grupe"));
			return;
		case PERR_HILEVEL:
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_have_level_30_grupe"));
			return;
		case PERR_ALREADYJOIN:
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("grupe_destroy"));
			return;
		case PERR_PARTYISFULL:
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("your_owen_grupe_has_full"));
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("grupe_has_full"));
			return;
		default:
...
void CHARACTER::PartyInviteDeny(DWORD dwPID)
{
	...
	if (pchInvitee)
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("request_has_been_cancel"), pchInvitee->GetName());
}

void CHARACTER::PartyJoin(LPCHARACTER pLeader)
{
	pLeader->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("player %s connect_to_grupe"), GetName());
	ChatPacket(CHAT_TYPE_INFO, LC_TEXT("party_leader %s you_is_member"), pLeader->GetName());
		...
		if (!CTargetManager::instance().GetTargetInfo(pkChrCauser->GetPlayerID(), TARGET_TYPE_VID, GetVID()) || GetRaceNum() == 30000)
		{
			if (GetMyShop())
			{
				...
				if (pkChrCauser == this) 
				{
					...
					{
						pkChrCauser->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("char_msg_5"));
						return;
					}
				}
				else 
				{
					...
					{
						pkChrCauser->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("char_msg_6"));
						return;
					}
					if ((GetExchange() || IsOpenSafebox() || IsCubeOpen()))
					{
						pkChrCauser->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_player_is_with_other_player"));
						return;
					}
				}
#ifdef OFFLINE_SHOP
...
bool CHARACTER::Follow(LPCHARACTER pkChr, float fMinDistance)
{
	...
	if (pkChr->IsState(pkChr->m_stateMove) && GetMobBattleType() != BATTLE_TYPE_RANGE && GetMobBattleType() != BATTLE_TYPE_MAGIC && false == IsPet())
	{
...
void CHARACTER::ReqSafeboxLoad(const char* pszPassword)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("please_chack_you_data"));
		return;
	}
	else if (m_pkSafebox)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("safebox_is_open"));
		return;
	}
	int iPulse = thecore_pulse();
	if (iPulse - GetSafeboxLoadTime() < PASSES_PER_SEC(10))
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_must_wait_10_sec_safebox"));
		return;
	}
	else if (GetDistanceFromSafeboxOpen() > 1000)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_is_long_after_safebox"));
		return;
	}
...
void CHARACTER::QuerySafeboxSize()
{
	if (m_iSafeboxSize == -1)
	{
		DBManager::instance().ReturnQuery(QID_SAFEBOX_SIZE, GetPlayerID(), NULL, "SELECT size FROM safebox%s WHERE account_id = %u", get_table_postfix(), GetDesc()->GetAccountTable().id);
	}
}
...
void CHARACTER::SetNowWalking(bool bWalkFlag)
{
	if (m_bNowWalking != bWalkFlag)
	{
		...
		{
			if (m_bNowWalking)
				MonsterLog("char_log_7");
			else
				MonsterLog("char_log_8");
		}
	}
}
...
bool CHARACTER::WarpToPID(DWORD dwPID)
{
	...
	if ((victim = (CHARACTER_MANAGER::instance().FindByPID(dwPID))))
	{
		...
		if (IS_SUMMONABLE_ZONE(mapIdx))
		{
			...
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_teleport_on_character"));
				return false;
			}
		}
		else
	...
	else
	{
		...
		if (!pcci)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("player_is_not_online"));
			return false;
		}
		if (pcci->bChannel != g_bChannel)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("player_is_on_channel (%d)"), pcci->bChannel, g_bChannel);
			return false;
		}
...
bool CHARACTER::IsHack(bool bSendMsg, bool bCheckShopOwner, int limittime)
{
	...
	{
		if (bSendMsg)
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_change_character"), limittime);
		if (test_server)
			ChatPacket(CHAT_TYPE_INFO, "[TEST-SERVER-INFO]Pulse %d LoadTime %d PASS %d", iPulse, GetSafeboxLoadTime(), PASSES_PER_SEC(limittime));
		return true;
	}
	if (bCheckShopOwner)
	{
		...
		{
			if (bSendMsg)
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_must_wait_10_sec"));
	...
	{
		if (bSendMsg)
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("10_sec_protection_limit"), limittime );
		return true;
	}
	...
	{
		if (bSendMsg)
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_can_use_agin_1"), limittime);
		return true;
	}
	return false;
}
...
void CHARACTER::OpenAcce(bool bCombination)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("the_acce_window_already_opened."));
		return;
	}
	if (bCombination)
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("before_you_may_close_the_acce_absorption_window."));
			return;
		}
	...
	else
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("before_you_may_close_the_acce_combine_window"));
			return;
		}
...
void CHARACTER::AddAcceMaterial(TItemPos tPos, BYTE bPos)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_add_locked_items"));
		return;
	}
	#ifdef __SOULBINDING_SYSTEM__
	else if ((pkItem->IsBind()) || (pkItem->IsUntilBind()))
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_add_binded_items"));
		return;
	}
	#endif
	else if ((m_bAcceCombination) && (bPos == 1) && (!AcceIsSameGrade(pkItem->GetValue(ACCE_GRADE_VALUE_FIELD))))
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_can_combine_just_accees_of_same_grade"));
		return;
	}
	else if ((m_bAcceCombination) && (pkItem->GetSocket(ACCE_ABSORPTION_SOCKET) >= ACCE_GRADE_4_ABS_MAX))
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_acce_got_already_maximum_absorption_chance"));
		return;
	}
	else if ((bPos == 1) && (m_bAcceAbsorption))
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_can_absorb_just_the_bonuses_from_armors_and_weapons"));
			return;
		}
...
void CHARACTER::RefineAcceMaterials()
{
	...
	if (bCan == 1)
	{
		...
		if (GetGold() < dwPrice)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_have_enough_Yang"));
			return;
		}		
		...
		if (bSucces)
		{
			...
			pkItem->AttrLog();
			if (lVal == 4)
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("new_absorption_rate: %d%"), dwAbs);
			else
		...
		else
		{
			...
			if (lVal == 4)
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("new_absorption_rate: %d%"), pkItemMaterial[0]->GetSocket(ACCE_ABSORPTION_SOCKET));
			else

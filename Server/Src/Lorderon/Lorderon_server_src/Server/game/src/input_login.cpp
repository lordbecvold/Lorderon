...
static void _send_bonus_info(LPCHARACTER ch)
{
	...
	if (item_drop_bonus)
	{
		ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("drop_chance %d%% plus"), item_drop_bonus);
	}
	if (gold_drop_bonus)
	{
		ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("gold_drop %d%% rate"), gold_drop_bonus);
	}
	if (gold10_drop_bonus)
	{
		ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("gold_drop_rate_1 %d%% rate_gold"), gold10_drop_bonus);
	}
	if (exp_bonus)
	{
		ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("exp %d%% rate_online"), exp_bonus);
	}
}
...
bool NewPlayerTable(TPlayerTable * table, const char * name, BYTE job, BYTE shape, BYTE bEmpire, BYTE bCon, BYTE bInt, BYTE bStr, BYTE bDex)
{
	if (job >= JOB_MAX_NUM)
		return false;
...
void CInputLogin::CharacterCreate(LPDESC d, const char * data)
{
	...
	sys_log(0, "PlayerCreate: name %s account_id %d, TPlayerCreatePacketSize(%d), Packet->Gold %d", pinfo->name, pinfo->index, sizeof(TPlayerCreatePacket), player_create_packet.player_table.gold);
...
void CInputLogin::Entergame(LPDESC d, const char * data)
{
	...
	if (g_bCheckClientVersion)
	{
		...
		else
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("you_client_is_not_corect"));
	...
	if (g_noticeBattleZone)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("inpu_login_msg_1"));
			ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("inpu_login_msg_2"));
			ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("inpu_login_msg_3"));
		}
	}
}
...
void CInputLogin::GuildSymbolCRC(LPDESC d, const char* c_pData)
{
	...
	if (pkGS->raw.size() != CGPacket.size || pkGS->crc != CGPacket.crc)
	{
		...
		sys_log(0, "SendGuildSymbolHead %02X%02X%02X%02X Size %d", pkGS->raw[0], pkGS->raw[1], pkGS->raw[2], pkGS->raw[3], pkGS->raw.size());
	}
}

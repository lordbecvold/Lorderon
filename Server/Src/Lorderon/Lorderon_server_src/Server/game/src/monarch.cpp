...
struct FHealMyEmpire
{
	...
	void operator () (LPENTITY ent)
	{
		if (ent->IsType(ENTITY_CHARACTER))
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("king_bels_msg"));
...
int CMonarch::HealMyEmpire(LPCHARACTER ch ,DWORD price)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%d king_bels_msg_1"), ch->GetMCLTime(CHARACTER::MI_HEAL));
		return 0;
	}
	if (!IsMoneyOk(price, Empire))
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("here : %u taxes : %u"), GetMoney(Empire), price);
		return 0;
	}
	...
	if (test_server)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[TEST_ONLY]idk_msg : %d"), GetMoney(Empire) - price);
	return 1;
}

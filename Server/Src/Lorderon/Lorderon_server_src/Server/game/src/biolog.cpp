...
const char *pTableStringGlobal[5][10] = {
	/* Actual count*/	{"biolog_lv30.collect_count_actual", "biolog_lv40.collect_count_actual", "biolog_lv50.collect_count_actual", "biolog_lv60.collect_count_actual", "biolog_lv70.collect_count_actual", "biolog_lv80.collect_count_actual", "biolog_lv85.collect_count_actual", "biolog_lv90.collect_count_actual", "biolog_lv92.collect_count_actual", "biolog_lv94.collect_count_actual"},
	/* Remaining time to send*/	{"biolog_lv30.left_time", "biolog_lv40.left_time", "biolog_lv50.left_time", "biolog_lv60.left_time", "biolog_lv70.left_time", "biolog_lv80.left_time", "biolog_lv85.left_time", "biolog_lv90.left_time", "biolog_lv92.left_time", "biolog_lv94.left_time"},
	{"biolog_lv30.duration_send", "biolog_lv40.duration_send", "biolog_lv50.duration_send", "biolog_lv60.duration_send", "biolog_lv70.duration_send", "biolog_lv80.duration_send", "biolog_lv85.duration_send", "biolog_lv90.duration_send", "biolog_lv92.duration_send", "biolog_lv94.duration_send"},
	{"biolog_lv30.completed", "biolog_lv40.completed", "biolog_lv50.completed", "biolog_lv60.completed", "biolog_lv70.completed", "biolog_lv80.completed", "biolog_lv85.completed", "biolog_lv90.completed", "biolog_lv92.completed", "biolog_lv94.completed"}
};
...
#define TIME_FOR_WAIT_TO_USE_AGAIN	6*60*60
	...
	if (curTime && get_global_time() < curTime) {
		...
		c /= 60;
		int mHours = c % 24;
		ch->ChatPacket (CHAT_TYPE_INFO, "<Biolog> Musíš poèkat [%d H] [%d M] [%d S] pro další odevzdání.", mHours, mMin, mSec);
		return;
	}
	for (int i = 0; i <= 9; i++) {
		for (int j = 1; j <= 2; j++) {
	...
	ch->ChatPacket (CHAT_TYPE_INFO, "<Biolog> Biolog dokonèil analízu mùžeš odevzdat další pøedmìt.");
...
{
	std::string pRewardType = iRewardType;
	boost::algorithm::replace_all (pRewardType, " ", "$");
	std::string pBonusName_1 = iBonusName_1;
	boost::algorithm::replace_all (pBonusName_1, " ", "$");
	std::string pBonusName_2 = iBonusName_2;
	boost::algorithm::replace_all (pBonusName_2, " ", "$");
	char buf[2048];
	...
	switch (iType) {
		case 1:
			if (pGetCompleted[0] != 1) {
				std::string arg1 = (const char *) pPopUp[8][0];
				boost::algorithm::replace_all (arg1, " ", "$");
				std::string arg2 = (const char *) pPopUp[8][1];
				boost::algorithm::replace_all (arg2, " ", "$");
				std::string arg3 = (const char *) pPopUp[8][2];
				boost::algorithm::replace_all (arg3, " ", "$");
				std::string arg4 = (const char *) pPopUp[8][3];
				boost::algorithm::replace_all (arg4, " ", "$");
				char buf[512 + 1];
		...
		case 2:
			if (pGetCompleted[1] != 1) {
				std::string arg1 = (const char *) pPopUp[9][0];
				boost::algorithm::replace_all (arg1, " ", "$");
				std::string arg2 = (const char *) pPopUp[9][1];
				boost::algorithm::replace_all (arg2, " ", "$");
				std::string arg3 = (const char *) pPopUp[9][2];
				boost::algorithm::replace_all (arg3, " ", "$");
				std::string arg4 = (const char *) pPopUp[9][3];
				boost::algorithm::replace_all (arg4, " ", "$");
				char buf[512 + 1];
...
bool SendBiolog_Data (LPCHARACTER ch, int argument)
{
	...
	if (ch->CountSpecifyItem (pTableVnum[argument]) > 0) {
		...
		ch->SetBiologState (pTableStringGlobal[0][argument], pTableString[0][argument]);
		ch->SetBiologState (pTableStringGlobal[1][argument], init_biologTime() + pTableTime[argument]);
		ch->SetBiologState (pTableStringGlobal[2][argument], init_biologTime());
		ch->RemoveSpecifyItem (pTableVnum[argument], 1);
		BiologManager::instance().SendUpdate_Binary (ch);
...
bool BiologManager::SendUpdate_Binary (LPCHARACTER ch)
{
	...
	for (int count = 0; count < abs (_countof (pxCountTable)); count++) {

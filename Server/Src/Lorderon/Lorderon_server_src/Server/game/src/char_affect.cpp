...
EVENTINFO(load_affect_login_event_info)
{
	...
	DWORD count;
	char* data;
	load_affect_login_event_info() : pid( 0 ), count( 0 ), data( 0 )
	{ }
};
...
bool CHARACTER::AddAffect(DWORD dwType, BYTE bApplyOn, long lApplyValue, DWORD dwFlag, long lDuration, long lSPCost, bool bOverride, bool IsCube )
{
	...
	switch (dwType)
	{
...
#endif
		{
			...
			{
				ChatPacket(CHAT_TYPE_INFO, "skill_msg_not_aply", CSkillManager::instance().Get(dwType)->szName, lApplyValue, pkAffect->lApplyValue);
				return false;
			}
		}
		break;
...
bool CHARACTER::RemoveAffect(DWORD dwType)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("chat_block_remove"));
	}
	bool flag = false;
	CAffect * pkAff;

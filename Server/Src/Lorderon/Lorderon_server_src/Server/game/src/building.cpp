...
void CObject::Destroy()
{
	if (m_pProto)
	{
		SECTREE_MANAGER::instance().ForAttrRegion(GetMapIndex(), GetX() + m_pProto->lRegion[0], GetY() + m_pProto->lRegion[1], GetX() + m_pProto->lRegion[2], GetY() + m_pProto->lRegion[3], (long)m_data.zRot, ATTR_OBJECT, ATTR_REGION_MODE_REMOVE);
	}
	CEntity::Destroy();
	if (GetSectree())
...
bool CObject::Show(long lMapIndex, long x, long y)
{
	...
	UpdateSectree();
	SECTREE_MANAGER::instance().ForAttrRegion(lMapIndex, x + m_pProto->lRegion[0], y + m_pProto->lRegion[1], x + m_pProto->lRegion[2], y + m_pProto->lRegion[3], (long)m_data.zRot, ATTR_OBJECT, ATTR_REGION_MODE_SET);
	return true;
}
...
void CObject::RegenNPC()
{
	...
	m_chNPC = CHARACTER_MANAGER::instance().SpawnMob(m_pProto->dwNPCVnum, GetMapIndex(), GetX() + newX, GetY() + newY, GetZ(), false, (int)m_data.zRot);
	if (!m_chNPC)
	{

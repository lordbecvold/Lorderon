...
void GetFindFileList(LPCTSTR pszDirectory, LPCTSTR pszFilter, std::vector <std::string> &vtList)
{
	...
	{
		if( ( FindFileData.cFileName[0]!=_T('.') || lstrlen(FindFileData.cFileName)>2) && ( 0 != _stricmp( FindFileData.cFileName, "Replay")))
		{
			lstrcpy(buf,pszDirectory);
			lstrcat(buf,_T("\\"));
				...
				switch (bType)
				{
					case COLLISION_TYPE_PLANE:
						printf("P");
						{
							...
							for (int iPairIndex = 0; iPairIndex < 6; iPairIndex++)
							{
								...
								if (fabs(PlaneData.v3QuadPosition[iFrom].x - PlaneData.v3QuadPosition[iTo].x) > 100000.0f || fabs(PlaneData.v3QuadPosition[iFrom].y - PlaneData.v3QuadPosition[iTo].y) > 100000.0f)
								{
									sys_log(0, "Blcok too big: %d %d %d %d", iBaseX + (int)  PlaneData.v3QuadPosition[iFrom].x, iBaseY + (int) -PlaneData.v3QuadPosition[iFrom].y, iBaseX + (int)  PlaneData.v3QuadPosition[iTo].x, iBaseY + (int) -PlaneData.v3QuadPosition[iTo].y);
								}
								else
								{
									printf("Block %d %d %d %d\n", iBaseX + (int)  PlaneData.v3QuadPosition[iFrom].x, iBaseY + (int) -PlaneData.v3QuadPosition[iFrom].y, iBaseX + (int)  PlaneData.v3QuadPosition[iTo].x, iBaseY + (int) -PlaneData.v3QuadPosition[iTo].y);
									ProcessLine(iBaseX + (int)  PlaneData.v3QuadPosition[iFrom].x, iBaseY + (int) -PlaneData.v3QuadPosition[iFrom].y, iBaseX + (int)  PlaneData.v3QuadPosition[iTo].x, iBaseY + (int) -PlaneData.v3QuadPosition[iTo].y, BlockAttribute);
								}
							}
						}
						break;
					case COLLISION_TYPE_SPHERE:
						printf("S");
						{
							...
							ProcessSphere(iBaseX + SphereData.v3Position.x, iBaseY - SphereData.v3Position.y, SphereData.fRadius, BlockAttribute);
						}
						break;
					case COLLISION_TYPE_CYLINDER:
						printf("C");
						{
							...
							ProcessSphere(iBaseX + CylinderData.v3Position.x, iBaseY - CylinderData.v3Position.y, CylinderData.fRadius, BlockAttribute);
						}
						break;
					default:
						printf("invalid type!");
...
void ConvertAttribute(const char * c_pszCollisionDataFileName, const char * c_pszMapDirectory)
{
	...
	for (y = 0; y < iMapHeight; ++y)
		for (x = 0; x < iMapWidth; ++x)
		{
			...
			id.coord.y = y+by;
			LPSECTREE pSec = g_pkMapSectree->Find(id.package);
			LZOManager::instance().Compress((unsigned char *) pSec->GetAttributePtr()->GetDataPtr(), uiBlockSize, (unsigned char *) pAttr, &uiCompSize);

...
int g_server_id = 0;
string g_strWebMallURL = "www.lorderon.eu";
...
void map_allow_add(int index)
{
	...
	{
		fprintf(stdout, "[Lorderon-CORE]:!!! FATAL ERROR !!! multiple MAP_ALLOW setting!!\n");
		exit(1);
	}
	fprintf(stdout, "[Lorderon-CORE]:MAP ALLOW %d\n", index);
...
bool GetIPInfo()
{
...
#ifndef __WIN32__
			fprintf(stderr, "[Lorderon-CORE]:INTERNAL_IP: %s interface %s\n", netip, ifap->ifa_name);
#else
			fprintf(stderr, "[Lorderon-CORE]:INTERNAL_IP: %s\n", netip);
#endif
		}
...
#ifndef __WIN32__
			fprintf(stderr, "[Lorderon-CORE]:PUBLIC_IP: %s interface %s\n", netip, ifap->ifa_name);
#else
			fprintf(stderr, "[Lorderon-CORE]:PUBLIC_IP: %s\n", netip);
#endif
		}
	}
#ifndef __WIN32__
	...
	else
	{
		...
		else
		{
			...
			fprintf(stderr, "[Lorderon-CORE]:INTERNAL_IP -> PUBLIC_IP: %s\n", g_szPublicIP);
			return true;
		}
#else
		return false;
...
static bool __LoadConnectConfigFile(const char* configName)
	...
	*log_db = '\0';
	...
	{
		fprintf(stderr, "[Lorderon-CORE]:Can not open [%s]\n", configName);
		exit(1);
	}
	while (fgets(buf, 256, fpOnlyForDB))
	{
		...
		TOKEN("hostname")
		{
			...
			fprintf(stdout, "[Lorderon-CORE]:HOSTNAME: %s\n", g_stHostname.c_str());
			continue;
		}
		TOKEN("channel")
		{
		...
		TOKEN("player_sql")
		{
			...
			{
				fprintf(stderr, "[Lorderon-CORE]:PLAYER_SQL syntax: logsql <host user password db>\n");
				exit(1);
			}
			char buf[1024];
		...
		TOKEN("common_sql")
		{
			...
			{
				fprintf(stderr, "[Lorderon-CORE]:COMMON_SQL syntax: logsql <host user password db>\n");
				exit(1);
			}
			char buf[1024];
		...
		TOKEN("log_sql")
		{
			...
			{
				fprintf(stderr, "[Lorderon-CORE]:LOG_SQL syntax: logsql <host user password db>\n");
				exit(1);
			}
			char buf[1024];
	...
	{
		fprintf(stderr, "[Lorderon-CORE]:cannot start server while no common sql connected\n");
		exit(1);
	}
	fprintf(stdout, "[Lorderon-CORE]:CommonSQL connected\n");
	{
		char szQuery[512];
		...
		{
			fprintf(stderr, "[Lorderon-CORE]:COMMON_SQL: DirectQuery failed : %s\n", szQuery);
			exit(1);
		}
		MYSQL_ROW row;
		while (NULL != (row = mysql_fetch_row(pMsg->Get()->pSQLResult)))
		{
			if (strcasecmp(row[0], "LOCALE") == 0)
			{
				...
				{
					fprintf(stderr, "[Lorderon-CORE]:COMMON_SQL: invalid locale key %s\n", row[1]);
					exit(1);
				}
			}
		}
	}
	fprintf(stdout, "[Lorderon-CORE]:Setting DB to locale %s\n", g_stLocale.c_str());
	...
	{
		fprintf(stderr, "[Lorderon-CORE]:PlayerSQL.ConnectError\n");
		exit(1);
	}
	fprintf(stdout, "[Lorderon-CORE]:PlayerSQL connected\n");
	if (false == g_bAuthServer) 
	{
		...
		{
			fprintf(stderr, "[Lorderon-CORE]:LogSQL.ConnectError\n");
			exit(1);
		}
		fprintf(stdout, "[Lorderon-CORE]:LogSQL connected\n");
	...
	}
	{
		...
		if (pMsg->Get()->uiNumRows == 0)
		{
			fprintf(stderr, "[Lorderon-CORE]:[SKILL_PERCENT] Query failed: %s", szQuery);
			exit(1);
		}
		MYSQL_ROW row;
		...
		fprintf(stdout, "[Lorderon-CORE]:SKILL_POWER_BY_LEVEL %s\n", p);
		while (*p != '\0' && cnt < (SKILL_MAX_LEVEL + 1))
		{
			...
			if (*p == '\0')
			{
				...
				{
					fprintf(stderr, "[Lorderon-CORE]:[SKILL_PERCENT] locale table has not enough skill information! (count: %d query: %s)", cnt, szQuery);
					exit(1);
				}
				fprintf(stdout, "[Lorderon-CORE]:SKILL_POWER_BY_LEVEL: Done! (count %d)\n", cnt);
				break;
			}
		}
		for (int job = 0; job < JOB_MAX_NUM * 2; ++job)
		{
			...
			fprintf(stdout, "[Lorderon-CORE]:SKILL_POWER_BY_JOB %d %s\n", job, p);
			while (*p != '\0' && cnt < (SKILL_MAX_LEVEL + 1))
			{
				p = one_argument(p, num, sizeof(num));
				...
				if (*p == '\0')
				{
					...
					fprintf(stdout, "[Lorderon-CORE]:SKILL_POWER_BY_JOB: Done! (job: %d count: %d)\n", job, cnt);
					break;
				}
			}
...
static bool __LoadDefaultConfigFile(const char* configName)
{
	...
	while (fgets(buf, 256, fp))
	{
		...
		TOKEN("auth_server")
		{
			...
			{
				fprintf(stderr, "[Lorderon-CORE]:AUTH_SERVER: syntax error: <ip|master> <port>\n");
				exit(1);
			}
			g_bAuthServer = true;
			LoadBanIP("BANIP");
			if (!strcasecmp(szIP, "master"))
				fprintf(stdout, "[Lorderon-CORE]:AUTH_SERVER: I am the master\n");
			else
			{
				g_stAuthMasterIP = szIP;
				str_to_number(g_wAuthMasterPort, szPort);
				fprintf(stdout, "[Lorderon-CORE]:AUTH_SERVER: master %s %u\n", g_stAuthMasterIP.c_str(), g_wAuthMasterPort);
			}
			continue;
		}
		TOKEN("teen_addr")
...
static bool __LoadGeneralConfigFile(const char* configName)
{
	...
	while (fgets(buf, 256, fp))
	{
		...
		TOKEN("WEB_AUTH")
		{
			...
			{
				fprintf(stderr, "[Lorderon-CORE]:WEB_AUTH syntax error (ex: WEB_AUTH <host(metin2.co.kr) uri(/kyw/gameauth.php)>\n");
				exit(1);
			}
			char buf[1024];
			openid_server = 1;
		...
		{
			printf("-----------------------------------------------\n");
			printf("[Lorderon-CORE]:SKILLBOOKS NEED %s EXP PER BOOK\n", value_string);
			printf("-----------------------------------------------\n");
		...
		{
			printf("-----------------------------------------------\n");
			printf("[Lorderon-CORE]:SPEED_SERVER 0\n");
			printf("-----------------------------------------------\n");
		...
		{
			printf("-----------------------------------------------\n");
			printf("[Lorderon-CORE]:AUCTION_SERVER\n");
			printf("-----------------------------------------------\n");
		...
		TOKEN("item_count_limit")
		{
			...
			fprintf(stdout, "[Lorderon-CORE]:ITEM_COUNT_LIMIT: %d\n", g_bItemCountLimit);
			continue;
		}
		TOKEN("disable_shop_price_3x")
		{
			g_bEmpireShopPriceTripleDisable = true;
			fprintf(stdout, "[Lorderon-CORE]:EMPIRE_SHOP_PRICE_3x: DISABLED\n");
			continue;
		}
		TOKEN("shop_price_3x_tax") 
		{
			...
			fprintf(stdout, "[Lorderon-CORE]:SHOP_PRICE_3X_TAX: %s\n", (!g_bEmpireShopPriceTripleDisable)?"ENABLED":"DISABLED");
			continue;
		}
		...
		TOKEN("disable_change_attr_time")
		{
			...
			fprintf(stdout, "[Lorderon-CORE]:CHANGE_ATTR_TIME_LIMIT: DISABLED\n");
			continue;
		}
		TOKEN("change_attr_time_limit") 
		{
			...
			fprintf(stdout, "[Lorderon-CORE]:CHANGE_ATTR_TIME_LIMIT: %u\n", g_dwItemBonusChangeTime);
			continue;
		}
		TOKEN("disable_prism_item")
		{
			g_bDisablePrismNeed = true;
			fprintf(stdout, "[Lorderon-CORE]:PRISM_ITEM_REQUIREMENT: DISABLED\n");
			continue;
		}
		TOKEN("prism_item_require") 
		{
			...
			fprintf(stdout, "[Lorderon-CORE]:PRISM_ITEM_REQUIRE: %s\n", (!g_bDisablePrismNeed)?"ENABLED":"DISABLED");
			continue;
		}
		TOKEN("enable_global_shout")
		{
			g_bGlobalShoutEnable = true;
			fprintf(stdout, "[Lorderon-CORE]:GLOBAL_SHOUT: ENABLED\n");
			continue;
		}
		TOKEN("global_shout") 
		{
			...
			fprintf(stdout, "[Lorderon-CORE]:GLOBAL_SHOUT: %s\n", (g_bGlobalShoutEnable)?"ENABLED":"DISABLED");
			continue;
		}
		TOKEN("disable_emotion_mask")
		{
			g_bDisableEmotionMask = true;
			fprintf(stdout, "[Lorderon-CORE]:EMOTION_MASK_REQUIREMENT: DISABLED\n");
			continue;
		}
		TOKEN("emotion_mask_require")
		{
			...
			fprintf(stdout, "[Lorderon-CORE]:EMOTION_MASK_REQUIRE: %s\n", (g_bDisableEmotionMask)?"ENABLED":"DISABLED");
			continue;
		}
		TOKEN("enable_bootary_check")
		{
			g_bEnableBootaryCheck = true;
			fprintf(stdout, "[Lorderon-CORE]:ENABLE_BOOTARY_CHECK: ENABLED\n");
			continue;
		}
		TOKEN("bootary_check")
		{
			...
			fprintf(stdout, "[Lorderon-CORE]:BOOTARY_CHECK: %s\n", (g_bEnableBootaryCheck)?"ENABLED":"DISABLED");
			continue;
		}
		TOKEN("status_point_get_level_limit")
		{
			...
			fprintf(stdout, "[Lorderon-CORE]:STATUS_POINT_GET_LEVEL_LIMIT: %d\n", g_iStatusPointGetLevelLimit);
			continue;
		}
		TOKEN("status_point_set_max_value")
		{
			...
			fprintf(stdout, "[Lorderon-CORE]:STATUS_POINT_SET_MAX_VALUE: %d\n", g_iStatusPointSetMaxValue);
			continue;
		}
		TOKEN("shout_limit_level")
		{
			...
			fprintf(stdout, "[Lorderon-CORE]:SHOUT_LIMIT_LEVEL: %d\n", g_iShoutLimitLevel);
			continue;
		}
		TOKEN("db_log_level")
		{
			...
			str_to_number(flag, value_string);
			g_iDbLogLevel = flag;
			fprintf(stdout, "[Lorderon-CORE]:DB_LOG_LEVEL: %d\n", g_iDbLogLevel);
			continue;
		}
		TOKEN("sys_log_level")
		{
			...
			str_to_number(flag, value_string);
			g_iSysLogLevel = flag;
			fprintf(stdout, "[Lorderon-CORE]:SYS_LOG_LEVEL: %d\n", g_iSysLogLevel);
			continue;
		}
		TOKEN("item_destroy_time_autogive")
		{
			...
			fprintf(stdout, "[Lorderon-CORE]:ITEM_DESTROY_TIME_AUTOGIVE: %d\n", g_aiItemDestroyTime[ITEM_DESTROY_TIME_AUTOGIVE]);
			continue;
		}
		TOKEN("item_destroy_time_dropgold")
		{
			...
			fprintf(stdout, "[Lorderon-CORE]:ITEM_DESTROY_TIME_DROPGOLD: %d\n", g_aiItemDestroyTime[ITEM_DESTROY_TIME_DROPGOLD]);
			continue;
		}
		TOKEN("item_destroy_time_dropitem")
		{
			...
			fprintf(stdout, "[Lorderon-CORE]:ITEM_DESTROY_TIME_DROPITEM: %d\n", g_aiItemDestroyTime[ITEM_DESTROY_TIME_DROPITEM]);
			continue;
		}
		TOKEN("check_version_server")
		{
			...
			fprintf(stdout, "[Lorderon-CORE]:CHECK_VERSION_SERVER: %d\n", g_bCheckClientVersion);
			continue;
		}
		TOKEN("check_version_value")
		{
			g_stClientVersion = value_string;
			fprintf(stdout, "[Lorderon-CORE]:CHECK_VERSION_VALUE: %s\n", g_stClientVersion.c_str());
			continue;
		}
		TOKEN("enable_hack_check")
		{
			...
			fprintf(stdout, "[Lorderon-CORE]:ENABLE_HACK_CHECK: %d\n", gHackCheckEnable);
			continue;
		}
		TOKEN("gm_host_check")
		{
			...
			str_to_number(flag, value_string);
			g_bGMHostCheck = !!flag;
			fprintf(stdout, "[Lorderon-CORE]:GM_HOST_CHECK: %d\n", g_bGMHostCheck);
			continue;
		}
		TOKEN("guild_invite_limit")
		{
			...
			fprintf(stdout, "[Lorderon-CORE]:GUILD_INVITE_LIMIT: %d\n", g_bGuildInviteLimit);
			continue;
		}
		TOKEN("guild_infinite_members")
		{
			...
			fprintf(stdout, "[Lorderon-CORE]:GUILD_INFINITE_MEMBERS: %d\n", g_bGuildInfiniteMembers);
			continue;
		}
		TOKEN("empire_language_check")
		{
			...
			fprintf(stdout, "[Lorderon-CORE]:EMPIRE_LANGUAGE_CHECK: %s\n", (g_bDisableEmpireLanguageCheck)?"DISABLED":"ENABLED");
			continue;
		}
		TOKEN("skillbook_nextread_min")
		{
			...
			fprintf(stdout, "[Lorderon-CORE]:SKILLBOOK_NEXTREAD_MIN: %u\n", g_dwSkillBookNextReadMin);
			continue;
		}
		TOKEN("skillbook_nextread_max")
		{
			...
			fprintf(stdout, "[Lorderon-CORE]:SKILLBOOK_NEXTREAD_MAX: %u\n", g_dwSkillBookNextReadMax);
			continue;
		}
#endif
	...
	if ((fp = fopen(cmdName, "r")))
	{
		while (fgets(buf, 256, fp))
		{
			...
			if (!*cmd || !*levelname)
			{
				...
				fprintf(stderr, "[Lorderon-CORE]:CMD syntax error: <cmd> <PLAYER | LOW_WIZARD | WIZARD | HIGH_WIZARD | GOD | IMPLEMENTOR | DISABLE>\n");
#else
				fprintf(stderr, "[Lorderon-CORE]:CMD syntax error: <cmd> <LOW_WIZARD | WIZARD | HIGH_WIZARD | GOD | IMPLEMENTOR | DISABLE>\n");
#endif
				exit(1);
			}
			...
			if (test_server)
				fprintf(stdout, "[Lorderon-CORE]:CMD_REWRITE: [%s] [%s:%d]\n", cmd, levelname, level);
...
void config_init(const string& st_localeServiceName)
{
...
#ifdef ENABLE_GENERAL_CONFIG
	{
		...
		if (__LoadGeneralConfigFile(szFileName))
			fprintf(stderr, "[Lorderon-CORE]:GENERAL CONFIG LOAD OK [%s]\n", szFileName);
	}
	{
		char szFileName[256];
	...
	if (0 == db_port)
	{
		fprintf(stderr, "[Lorderon-CORE]:DB_PORT not configured\n");
		exit(1);
	}
	if (0 == g_bChannel)
	{
		fprintf(stderr, "[Lorderon-CORE]:CHANNEL not configured\n");
		exit(1);
	}
	if (g_stHostname.empty())
	{
		fprintf(stderr, "[Lorderon-CORE]:HOSTNAME must be configured.\n");
		exit(1);
	}
	...
	{
		fprintf(stderr, "[Lorderon-CORE]:Failed to Load ExpTable from DB so exit\n");
	}
#endif
...
#ifdef ENABLE_GENERAL_CMD
	{
		...
		if (__LoadDefaultCMDFile(szFileName))
			fprintf(stdout, "[Lorderon-CORE]:GENERAL CMD LOAD OK [%s]\n", szFileName);
	}
	{
		char szFileName[256];
	...
	if (g_szPublicIP[0] == '0')
	{
		fprintf(stderr, "[Lorderon-CORE]:Can not get public ip address\n");
		exit(1);
	}
}
...
bool LoadClientVersion()
{
	...
	char * p = strchr(buf, '\n');
	if (p) *p = '\0';
	fprintf(stderr, "[Lorderon-CORE]:VERSION: \"%s\"\n", buf);

...
namespace
{
	...
	{
		public:
			FuncFindChrForFlag(LPCHARACTER pkChr) : m_pkChr(pkChr), m_pkChrFind(NULL), m_iMinDistance(INT_MAX) {	
			}
			void operator () (LPENTITY ent)
			{
				...
				if (iDist <= 500 && m_iMinDistance > iDist && !pkChr->IsAffectFlag(AFF_WAR_FLAG1) && !pkChr->IsAffectFlag(AFF_WAR_FLAG2) && !pkChr->IsAffectFlag(AFF_WAR_FLAG3))
				{
		...
		public:
			...
			void operator () (LPENTITY ent)
			{
				...
				if (!pkGuild)
					return;
				int iDist = DISTANCE_APPROX(pkChr->GetX()-m_pkChr->GetX(), pkChr->GetY()-m_pkChr->GetY());
				if (iDist <= 500 && (pkChr->IsAffectFlag(AFF_WAR_FLAG1) || pkChr->IsAffectFlag(AFF_WAR_FLAG2) || pkChr->IsAffectFlag(AFF_WAR_FLAG3)))
				{
					CAffect * pkAff = pkChr->FindAffect(AFFECT_WAR_FLAG);
					sys_log(0, "FlagBase %s dist %d aff %p flag gid %d chr gid %u", pkChr->GetName(), iDist, pkAff, m_pkChr->GetPoint(POINT_STAT), pkChr->GetGuild()->GetID());
					if (pkAff)
					{
						if ((DWORD) m_pkChr->GetPoint(POINT_STAT) == pkGuild->GetID() && m_pkChr->GetPoint(POINT_STAT) != pkAff->lApplyValue)
						{
							CWarMap * pMap = pkChr->GetWarMap();
							BYTE idx;
								...
								return;
							{
								...
								pMap->ResetFlag();
								char buf[256];
								snprintf(buf, sizeof(buf), LC_TEXT("flage_now_is_free"), pMap->GetGuild(idx)->GetName(), pMap->GetGuild(idx_opp)->GetName());
								pMap->Notice(buf);
							}
						}
					}
	...
	{
		public:
			FuncFindGuardVictim(LPCHARACTER pkChr, int iMaxDistance) : m_pkChr(pkChr), m_iMinDistance(INT_MAX), m_iMaxDistance(iMaxDistance), m_lx(pkChr->GetX()), m_ly(pkChr->GetY()), m_pkChrVictim(NULL) {
			};
	...
	else
	{
		...
		if (!IS_SET(m_pointsInstant.dwAIFlag, AIFLAG_NOMOVE))
		{
			...
			if (!number(0, 6))
			{
				...
				GetDeltaByDegree(GetRotation(), fDist, &fx, &fy);
				if (!(SECTREE_MANAGER::instance().IsMovablePosition(GetMapIndex(), GetX() + (int) fx, GetY() + (int) fy) && SECTREE_MANAGER::instance().IsMovablePosition(GetMapIndex(), GetX() + (int) fx / 2, GetY() + (int) fy / 2)))
					return;
				SetNowWalking(true);
   ...
   if (victim && !victim->IsDead())
   {
	...
	if (pkChrProtege)
	{
		if (DISTANCE_APPROX(GetX() - pkChrProtege->GetX(), GetY() - pkChrProtege->GetY()) > 1000)
		{
			...
			{
				MonsterLog("[IDLE] char_state_log_1");
				return;
			}
		}
	}
...
void CHARACTER::StateMove()
{
	...
	if (1.0f == fRate)
	{
		if (IsPC())
		{
			sys_log(1, "isPC %s %d %d", GetName(), x, y);
		...
		else
		{
			if (GetVictim() && !IsCoward())
			{
				if (!IsState(m_stateBattle))
					MonsterLog("[BATTLE] char_state_log_2 %s", GetVictim()->GetName());
				GotoState(m_stateBattle);
				m_dwStateDuration = 1;
			}
			else
			{
				if (!IsState(m_stateIdle))
					MonsterLog("[IDLE] ´ë»óÀÌ char_state_log_3");
				GotoState(m_stateIdle);
				m_dwStateDuration = PASSES_PER_SEC(number(1, 3));
			}
		}
					...
					case BATTLE_TYPE_TANKER:
						{
							...
							if (SECTREE_MANAGER::instance().IsMovablePosition(victim->GetMapIndex(), victim->GetX() + (int) fx, victim->GetY() + (int) fy) && SECTREE_MANAGER::instance().IsMovablePosition(victim->GetMapIndex(), victim->GetX() + (int) fx/2, victim->GetY() + (int) fy/2))
							{
...
void CHARACTER::StateFlag()
{
	...
	pMap->RemoveFlag(idx);
	snprintf(buf, sizeof(buf), LC_TEXT("%s StateFlag %s CHARACTER."), pMap->GetGuild(idx)->GetName(), f.m_pkChrFind->GetName());
	pMap->Notice(buf);
}

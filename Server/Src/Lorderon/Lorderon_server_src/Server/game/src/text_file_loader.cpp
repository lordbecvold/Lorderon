...
CTextFileLoader::CTextFileLoader() : m_dwcurLineIndex(0), mc_pData(NULL)
{
	SetTop();
...
bool CTextFileLoader::LoadGroup(TGroupNode * pGroupNode)
{
	...
	for (; m_dwcurLineIndex < m_fileLoader.GetLineCount(); ++m_dwcurLineIndex)
	{
		...
		else
		{
			...
			{
				sys_err("CTextFileLoader::LoadGroup : must have a value (filename: %s line: %d key: %s)", m_strFileName.c_str(), m_dwcurLineIndex, key.c_str());
				break;
			}

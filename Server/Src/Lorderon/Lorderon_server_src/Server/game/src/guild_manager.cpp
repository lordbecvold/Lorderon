...
DWORD CGuildManager::CreateGuild(TGuildCreateParameter& gcp)
{
	...
	{
		gcp.master->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("guild_name_has_wrong"));
		return 0;
	}
	...
	if (strncmp(__guild_name, gcp.name, strnlen(gcp.name, sizeof(gcp.name))))
		return 0;
	std::auto_ptr<SQLMsg> pmsg(DBManager::instance().DirectQuery("SELECT COUNT(*) FROM guild%s WHERE name = '%s'", get_table_postfix(), __guild_name));
	if (pmsg->Get()->uiNumRows > 0)
	{
		MYSQL_ROW row = mysql_fetch_row(pmsg->Get()->pSQLResult);
		if (!(row[0] && row[0][0] == '0'))
		{
			gcp.master->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("this_guild_name_used"));
			return 0;
		}
	}
	else
	{
		gcp.master->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_have_already_guild"));
		return 0;
	}
...
void CGuildManager::GetHighRankString(DWORD dwMyGuild, char * buffer, size_t buflen)
{
	...
	*buffer = '\0';
	for (int i = 0; i < 8; ++i)
	{
		...
		len2 = snprintf(buffer + len, buflen - len, "%3d | %-12s | %-8d | %4d | %4d | %4d", GetRank(g), g->GetName(), g->GetLadderPoint(), g->GetGuildWarWinCount(), g->GetGuildWarDrawCount(), g->GetGuildWarLossCount());
		if (len2 < 0 || len2 >= (int) buflen - len)
			len += (buflen - len) - 1;
		else
			len += len2;
...
void CGuildManager::DeclareWar(DWORD guild_id1, DWORD guild_id2, BYTE bType)
{
		...
		g2->DeclareWar(guild_id1, bType, GUILD_WAR_RECV_DECLARE))
	{
		{
			char buf[256];
			snprintf(buf, sizeof(buf), LC_TEXT("%s guild %s war_request"), TouchGuild(guild_id1)->GetName(), TouchGuild(guild_id2)->GetName());
			SendNotice(buf);
		}
	}
}
...
void CGuildManager::RefuseWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	if (g1 && g2)
	{
		if (g2->GetMasterCharacter())
			g2->GetMasterCharacter()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("uild %s dont_accept_war"), g1->GetName());
	}
	if ( g1 != NULL )
...
void CGuildManager::WaitStartWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	{
		char buf[256];
		snprintf(buf, sizeof(buf), LC_TEXT("%s guild_war_with %s started"), g1->GetName(), g2->GetName());
		SendNotice(buf);
	}
}
...
void CGuildManager::StartWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	char buf[256];
	snprintf(buf, sizeof(buf), LC_TEXT("%s guild_war_with %s started_starting"), g1->GetName(), g2->GetName());
	SendNotice(buf);
	if (guild_id1 > guild_id2)
...
void SendGuildWarOverNotice(CGuild* g1, CGuild* g2, bool bDraw)
{
	if (g1 && g2)
	{
		...
		if (bDraw)
		{
			snprintf(buf, sizeof(buf), LC_TEXT("%s war %s guild_msg_r"), g1->GetName(), g2->GetName());
		}
		else
		{
			if ( g1->GetWarScoreAgainstTo( g2->GetID() ) > g2->GetWarScoreAgainstTo( g1->GetID() ) )
			{
				snprintf(buf, sizeof(buf), LC_TEXT("%s guild_war %s msg_p"), g1->GetName(), g2->GetName());
			}
			else
			{
				snprintf(buf, sizeof(buf), LC_TEXT("%s guild_war %s msg_p"), g2->GetName(), g1->GetName());
			}
		}
		SendNotice(buf);
	}
...
void CGuildManager::CancelWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	if (g1)
	{
		...
		if (master1)
			master1->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("guild_war_stoped"));
	}
	if (g2)
	{
		LPCHARACTER master2 = g2->GetMasterCharacter();
		if (master2)
			master2->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("guild_war_stoped"));
	}
	if (g1 && g2)
	{
		char buf[256+1];
		snprintf(buf, sizeof(buf), LC_TEXT("%s guild_war %s msg_k"), g1->GetName(), g2->GetName());
		SendNotice(buf);
	}
}
...
void CGuildManager::ChangeMaster(DWORD dwGID)
{
	...
	DBManager::instance().FuncQuery(std::bind1st(std::mem_fun(&CGuild::SendGuildDataUpdateToAllMember), iter->second),"SELECT 1");
}

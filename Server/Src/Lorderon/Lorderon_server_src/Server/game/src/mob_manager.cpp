...
void CMob::AddSkillSplash(int iIndex, DWORD dwTiming, DWORD dwHitDistance)
{
	if (iIndex >= MOB_SKILL_MAX_NUM || iIndex < 0)
		return;
	sys_log(0, "MOB_SPLASH %s idx %d timing %u hit_distance %u", m_table.szLocaleName, iIndex, dwTiming, dwHitDistance);
...
bool CMobManager::Initialize(TMobTable * pTable, int iSize)
{
	...
	snprintf(szGroupFileName, sizeof(szGroupGroupFileName), "%s/files/group.txt", LocaleService_GetBasePath().c_str());
	snprintf(szGroupGroupFileName, sizeof(szGroupGroupFileName), "%s/files/group_group.txt", LocaleService_GetBasePath().c_str());

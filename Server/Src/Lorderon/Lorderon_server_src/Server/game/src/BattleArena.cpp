	...
	if (pInfo != NULL)
	{
		switch (pInfo->state)
		{
			case 0:
				{
					++pInfo->state;
					BroadcastNotice(LC_TEXT("5_min_before_spawn_monster"));
				}
				return test_server ? PASSES_PER_SEC(60) : PASSES_PER_SEC(60*4);
			case 1:
				{
					++pInfo->state;
					BroadcastNotice(LC_TEXT("1_min_before_spawn_monster"));
				}
			...
			case 2:
				{
					...
					BroadcastNotice(LC_TEXT("monster_start_atacking_in_castle"));
			...
			case 3 :
				{
					...
					{
						pInfo->state = 6;
						SendNoticeMap(LC_TEXT("ghost_olt_colect"), pInfo->nMapIndex, false);
					}
					else
					{
						pInfo->wait_count++;
						...
						{
							pInfo->state++;
							SendNoticeMap(LC_TEXT("monster_leave_war"), pInfo->nMapIndex, false);
						}
						else
						{
			...
			case 4 :
				{
					pInfo->state++;
					SendNoticeMap(LC_TEXT("monster_come_here"), pInfo->nMapIndex, false);
					SendNoticeMap(LC_TEXT("you_teleporting_to_town"), pInfo->nMapIndex, false);
			...
			case 6 :
				{
					pInfo->state++;
					pInfo->wait_count = 0;
					SendNoticeMap(LC_TEXT("monster_leader_spawned"), pInfo->nMapIndex, false);
					SendNoticeMap(LC_TEXT("kill_demon_king_30_min"), pInfo->nMapIndex, false);
			...
			case 7 :
				{
					if ( SECTREE_MANAGER::instance().GetMonsterCountInMap(pInfo->nMapIndex) <= 0 )
					{
						SendNoticeMap(LC_TEXT("demon_king_has_killed"), pInfo->nMapIndex, false);
						SendNoticeMap(LC_TEXT("you_teleporting_to_town"), pInfo->nMapIndex, false);
						pInfo->state = 5;
					...
					{
						SendNoticeMap(LC_TEXT("demon_king_spawned"), pInfo->nMapIndex, false);
						SendNoticeMap(LC_TEXT("you_teleporting_to_town"), pInfo->nMapIndex, false);
						SECTREE_MANAGER::instance().PurgeMonstersInMap(pInfo->nMapIndex);
...
bool CBattleArena::Start(int nEmpire)
{
	...
	m_nEmpire = nEmpire;
	char szBuf[1024];
	snprintf(szBuf, sizeof(szBuf), LC_TEXT("%s monster_atack_castle"), EMPIRE_NAME(m_nEmpire));
	BroadcastNotice(szBuf);
	BroadcastNotice(LC_TEXT("attack_castle_start_after_10_min"));
	if (m_pEvent != NULL) {
...
void CBattleArena::SpawnRandomStone()
{
	...
	static const int region_info[3][4] = {{688900, 247600, 692700, 250000}, {740200, 251000, 744200, 247700}, {791400, 250900, 795900, 250400} };
...
void CBattleArena::SpawnLastBoss()
{
	...
	static const int position[3][2] = { { 691000, 248900 }, { 742300, 249000 }, { 793600, 249300 } };
	int idx = m_nMapIndex - 190;
	if ( idx < 0 || idx >= 3 ) return;

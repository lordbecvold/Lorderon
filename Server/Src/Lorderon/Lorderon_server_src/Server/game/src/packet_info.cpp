...
void CPacketInfo::Log(const char * c_pszFileName)
{
	...
	while (it != m_pPacketMap.end())
	{
		...
		++it;
		fprintf(fp, "%-16s %-10d %-10u %.2f\n", p->stName.c_str(), p->iCalled, p->dwLoad, p->iCalled != 0 ? (float) p->dwLoad / p->iCalled : 0.0f);
	}
	fclose(fp);
}
...
CPacketInfoCG::CPacketInfoCG()
{
	...
	Set(HEADER_CG_CONTACT_PROFILE_MANAGER, sizeof(TPacketCGContactProfileManager), "ContactProfileManager", true);
	...
	Set(HEADER_CG_COMBAT_ZONE_REQUEST_ACTION, sizeof(TPacketCGCombatZoneRequestAction), "TPacketCGCombatZoneRequestAction", false);
#endif

...
bool DSManager::ExtractDragonHeart(LPCHARACTER ch, LPITEM pItem, LPITEM pExtractor)
{
	...
	if (pItem->IsEquipped())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_make_this_with_dragon_stone"));
		return false;
	}
	...
	else
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("action_ds_filed"));
		return true;
	}
}
...
bool DSManager::PullOut(LPCHARACTER ch, TItemPos DestCell, LPITEM& pItem, LPITEM pExtractor)
{
	...
	if (!IsValidCellForThisItem(pItem, DestCell))
	{
		...
		if (iEmptyCell < 0)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_give_time_potion_ds"));
			return false;
		}
		else
		{
	...
	}
	{
		...
		if (bSuccess)
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ds_has_removed"));
			pItem->AddToCharacter(ch, DestCell);
			return true;
		}
		else
		{
			...
			if (dwByProduct)
			{
				...
				if (pByProduct)
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ds_remove_filed %s"), pByProduct->GetName());
				else
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DragonSoul_msg_1"));
			}
			else
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DragonSoul_msg_2"));
		}
	}
	return bSuccess;
}
...
bool DSManager::DoRefineGrade(LPCHARACTER ch, TItemPos (&aItemPoses)[DRAGON_SOUL_REFINE_GRID_SIZE])
{
	...
	if (!ch->DragonSoul_RefineWindow_CanRefine())
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "DragonSoul_msg_error");
		return false;
	}
	...
	for (int i = 0; i < DRAGON_SOUL_REFINE_GRID_SIZE; i++)
	{
		...
		if (NULL != pItem)
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DragonSoul_msg_3"));
	...
	std::set <LPITEM>::iterator it = set_items.begin();
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DragonSoul_msg_4"));
			SendRefineResultPacket(ch, DS_SUB_HEADER_REFINE_FAIL_INVALID_MATERIAL, TItemPos(pItem->GetWindow(), pItem->GetCell()));
			return false;
		}
	}
	while (++it != set_items.end())
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DragonSoul_msg_5"));
			SendRefineResultPacket(ch, DS_SUB_HEADER_REFINE_FAIL_INVALID_MATERIAL, TItemPos(pItem->GetWindow(), pItem->GetCell()));
			return false;
		}
	}
	...
	if (result_grade > grade_idx)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ds_refine_success"));
	...
	else
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ds_refine_filed"));
...
bool DSManager::DoRefineStep(LPCHARACTER ch, TItemPos (&aItemPoses)[DRAGON_SOUL_REFINE_GRID_SIZE])
{
	...
	{
		sys_err ("%s do not activate DragonSoulRefineWindow. But how can he come here?", ch->GetName());
		ch->ChatPacket(CHAT_TYPE_INFO, "DragonSoul_msg_error_1");
		return false;
	}
	std::set <LPITEM> set_items;
	for (int i = 0; i < DRAGON_SOUL_REFINE_GRID_SIZE; i++)
	{
		...
		if (NULL != pItem)
		{
			if (!pItem->IsDragonSoul())
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DragonSoul_msg_6"));
				SendRefineResultPacket(ch, DS_SUB_HEADER_REFINE_FAIL_INVALID_MATERIAL, TItemPos(pItem->GetWindow(), pItem->GetCell()));
				return false;
			}
			set_items.insert(pItem);
	...
	std::set <LPITEM>::iterator it = set_items.begin();
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DragonSoul_msg_7"));
			SendRefineResultPacket(ch, DS_SUB_HEADER_REFINE_FAIL_INVALID_MATERIAL, TItemPos(pItem->GetWindow(), pItem->GetCell()));
			return false;
		}
	}
	while(++it != set_items.end())
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DragonSoul_msg_8"));
			SendRefineResultPacket(ch, DS_SUB_HEADER_REFINE_FAIL_INVALID_MATERIAL, TItemPos(pItem->GetWindow(), pItem->GetCell()));
			return false;
		}
	}
	...
	if (ch->GetGold() < fee)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_dont_have_gold_refine"));
		SendRefineResultPacket(ch, DS_SUB_HEADER_REFINE_FAIL_NOT_ENOUGH_MONEY, NPOS);
		return false;
	}
	...
	if (result_step > step_idx)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ds_random_msg_1"));
		SendRefineResultPacket(ch, DS_SUB_HEADER_REFINE_SUCCEED, TItemPos (pResultItem->GetWindow(), pResultItem->GetCell()));
		return true;
	}
	else
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ds_random_msg_2"));
		SendRefineResultPacket(ch, DS_SUB_HEADER_REFINE_FAIL, TItemPos (pResultItem->GetWindow(), pResultItem->GetCell()));
		return false;
	}
}
...
bool DSManager::DoRefineStrength(LPCHARACTER ch, TItemPos (&aItemPoses)[DRAGON_SOUL_REFINE_GRID_SIZE])
{
	...
	for (std::set <LPITEM>::iterator it = set_items.begin(); it != set_items.end(); it++)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DragonSoul_msg_9"));
			SendRefineResultPacket(ch, DS_SUB_HEADER_REFINE_FAIL_INVALID_MATERIAL, TItemPos(pItem->GetWindow(), pItem->GetCell()));
			return false;
		}
	}
	...
	if (NULL != pDragonSoul)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DragonSoul_msg_12"));
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DragonSoul_msg_11"));
			SendRefineResultPacket(ch, DS_SUB_HEADER_REFINE_FAIL_MAX_REFINE, TItemPos(pDragonSoul->GetWindow(), pDragonSoul->GetCell()));
			return false;
		}
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DragonSoul_msg_10"));

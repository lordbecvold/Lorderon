...
CDungeon::CDungeon(IdType id, long lOriginalMapIndex, long lMapIndex) : m_id(id), m_lOrigMapIndex(lOriginalMapIndex), m_lMapIndex(lMapIndex), m_map_Area(SECTREE_MANAGER::instance().GetDungeonArea(lOriginalMapIndex))
{
	Initialize();
}
...
struct FWarpToPosition
{
	...
	long x;
	long y;
	FWarpToPosition(long lMapIndex, long x, long y) : lMapIndex(lMapIndex), x(x), y(y)
	{}
...
struct FWarpToPositionForce
{
	...
	long x;
	long y;
	FWarpToPositionForce(long lMapIndex, long x, long y) : lMapIndex(lMapIndex), x(x), y(y)
	{}
...
CDungeonManager::CDungeonManager() : next_id_(0)
{ }
...
struct FNearPosition
{
	...
	FNearPosition(long x, long y, int d) : x(x), y(y), dist(d), ret(true)
	{ }

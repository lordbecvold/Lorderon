...
struct FWarpToDragronLairWithGuildMembers
{
	...
	FWarpToDragronLairWithGuildMembers(DWORD guildID, long map, long X, long Y) : dwGuildID(guildID), mapIndex(map), x(X), y(Y)
	{
	}
...
EVENTFUNC( DragonLair_Collapse_Event )
{
	...
	{
		char buf[512];
		snprintf(buf, 512, LC_TEXT("msg_1 %d DragonLair_1"), pInfo->pLair->GetEstimatedTime());
...
{
	sys_log(0, "DragonLair: sys_log_1");

...
void CPrivManager::GiveGuildPriv(DWORD guild_id, BYTE type, int value, BYTE bLog, time_t end_time_sec)
{
	...
	if (g)
	{
		...
		{
			char buf[100];
			snprintf(buf, sizeof(buf), LC_TEXT("%s msg %s ou %d%% string"), g->GetName(), GetPrivName(type), value);
			SendNotice(buf);
		}
		else
		{
			char buf[100];
			snprintf(buf, sizeof(buf), LC_TEXT("%s msg %s guild_1"), g->GetName(), GetPrivName(type));
			SendNotice(buf);
		}
		if (bLog)
		{
...
void CPrivManager::GiveEmpirePriv(BYTE empire, BYTE type, int value, BYTE bLog, time_t end_time_sec)
{
	...
	{
		char buf[100];
		snprintf(buf, sizeof(buf), LC_TEXT("%s msg %s max_put"), GetEmpireName(empire), GetPrivName(type), value);
		if (empire)
			SendNotice(buf);
		else
			SendLog(buf);
	...
	{
		char buf[100];
		snprintf(buf, sizeof(buf), LC_TEXT("%s msg %s min_put"), GetEmpireName(empire), GetPrivName(type));
		if (empire)
			SendNotice(buf);
		else
			SendLog(buf);

...
void CGuild::GuildWarPacket(DWORD dwOppGID, BYTE bWarType, BYTE bWarState)
{
	...
	for (itertype(m_memberOnline) it = m_memberOnline.begin(); it != m_memberOnline.end(); ++it)
	{
		...
		if (bWarState == GUILD_WAR_ON_WAR)
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("you_cant_kill_monster_in_war"));
		LPDESC d = ch->GetDesc();
		if (d)
		{
...
void CGuild::RequestDeclareWar(DWORD dwOppGID, BYTE type)
{
	...
	if (it == m_EnemyGuild.end())
	{
		...
		{
			sys_err("GuildWar.DeclareWar.NOT_EXIST_MAP id(%d -> %d), type(%d), map(%d)", GetID(), dwOppGID, type, GuildWar_GetTypeMapIndex(type));
			map_allow_log();
			NotifyGuildMaster(LC_TEXT("guild_war_cant_started"));
			return;
		}
		TPacketGuildWar p;
		p.bType = type;
		...
		case GUILD_WAR_RECV_DECLARE:
			{
				...
				{
					sys_err("GuildWar.AcceptWar.NOT_EXIST_MAP id(%d -> %d), type(%d), map(%d)", GetID(), dwOppGID, type, GuildWar_GetTypeMapIndex(type));
					map_allow_log();
					NotifyGuildMaster(LC_TEXT("guild_war_cant_started"));
					return;
				}
				...
				if (test_server)
					p.lInitialScore /= 10;
				db_clientdesc->DBPacket(HEADER_GD_GUILD_WAR, 0, &p, sizeof(p));
				sys_log(0, "GuildWar.WaitStartSendToDB id(%d vs %d), type(%d), bet(%d), map_index(%d)", GetID(), dwOppGID, saved_type, guildWarInfo.iWarPrice, guildWarInfo.lMapIndex);
			}
			break;
		case GUILD_WAR_SEND_DECLARE:
			{
				NotifyGuildMaster(LC_TEXT("player_invited_g"));
			}
			break;
		default:
			sys_err("GuildWar.DeclareWar.UNKNOWN_STATE[%d]: id(%d vs %d), type(%d), guild(%s:%u)", it->second.state, GetID(), dwOppGID, type, GetName(), GetID());
			break;
	}
}
...
void CGuild::GuildWarEntryAccept(DWORD dwOppGID, LPCHARACTER ch)
{
	...
	if (gw.state != GUILD_WAR_ON_WAR)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("war_si_gone"));
		return;
	}
	if (!gw.map_index)
		return;
...
void CGuild::GuildWarEntryAsk(DWORD dwOppGID)
{
	...
	while (it != m_memberOnline.end())
	{
		...
		{
			sys_err("GuildWar.GuildWarEntryAsk.SendLetterToMember.QUEST_ERROR pid(%d), quest_name('guild_war_join.quest')", ch->GetPlayerID(), questIndex);
			break;
		}
	}
}
...
void CGuild::SetLadderPoint(int point)
{
	...
	{
		char buf[256];
		snprintf(buf, sizeof(buf), LC_TEXT("guild_lvl_plus"), point);
		for (itertype(m_memberOnline) it = m_memberOnline.begin(); it!=m_memberOnline.end();++it)
		{
			LPCHARACTER ch = (*it);

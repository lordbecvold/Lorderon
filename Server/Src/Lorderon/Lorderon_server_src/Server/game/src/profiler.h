	...
	public:
		...
		void Print(FILE * fp = stderr)
		{
			...
			for (DWORD k = 0; k < m_vec_Accum.size(); ++k)
			{
				...
				stLine += p->strName;
				fprintf(fp, "%-30s %10d/%-10d %.2f\n", stLine.c_str(), p->iCallingCount, p->iCollapsedTime, p->iCallingCount != 0 ? (float) p->iCollapsedTime / p->iCallingCount : 0.0f);
			}
		}
		...
		void PrintOneAccumData(const char * c_szName)
		{
			...
			if (it == m_ProfileAccumDataMap.end())
				return;
			TProfileAccumData & rData = it->second;
			sys_log(0, "%-10s : [CollapsedTime : %3d] / [CallingCount : %3d]", rData.strName.c_str(), rData.iCollapsedTime, rData.iCallingCount);
		}

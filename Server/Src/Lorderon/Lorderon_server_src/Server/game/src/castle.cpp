...
EVENTINFO(castle_event_info)
{
	int	empire;
	int	pulse;
	castle_event_info() : empire(0), pulse(0)
	{ }
};
...
EVENTFUNC(castle_siege_event)
{
	...
	{
		snprintf(buf, sizeof(buf), LC_TEXT("%sfire_battle_1"), EMPIRE_NAME(GET_SIEGE_EMPIRE()));
		BroadcastNotice(buf);
	...
	switch (GET_SIEGE_STATE())
	{
		...
		case CASTLE_SIEGE_STRUGGLE:
			{
				snprintf(buf, sizeof(buf), LC_TEXT("%scastle_protection"), EMPIRE_NAME(GET_SIEGE_EMPIRE()));
				BroadcastNotice(buf);
				snprintf(buf, sizeof(buf), LC_TEXT("you_get_award_30_min_1"), EMPIRE_NAME(GET_SIEGE_EMPIRE()));
				BroadcastNotice(buf);
		...
		case CASTLE_SIEGE_END:
			BroadcastNotice(LC_TEXT("30_min_left"));
			castle_end_siege();
			break;
	}
	return 0;
...
{
	DWORD vnum[7] = {8012, 8013, 8014, 8024, 8025, 8026, 8027};
...
EVENTINFO(castle_stone_event_info)
{
	int	empire;
	int	spawn_count;
	castle_stone_event_info() : empire(0), spawn_count(0)
	{ }
};
...
EVENTFUNC(castle_stone_event)
{
	...
	if (info->spawn_count < (SPAWN_COUNT * 2))
	{
		for (int i = 0; i < SPAWN_COUNT; ++i)
		{
			...
			CHARACTER_MANAGER::instance().SpawnMobRange(FN_random_stone(), FN_castle_map_index(info->empire), sx, sy, ex, ey);
		}
...
LPCHARACTER castle_spawn_frog(int empire)
{
	...
	LPCHARACTER frog = CHARACTER_MANAGER::instance().SpawnMob(CASTLE_FROG_VNUM, map_index, x, y, 0, false, dir);
	if (frog)
	{
...
LPCHARACTER castle_spawn_guard(int empire, DWORD group_vnum, int region_index)
{
	...
	mob = CHARACTER_MANAGER::instance().SpawnGroup(group_vnum, map_index, sx, sy, ex, ey);
	if (mob)
	{
...
{
	DWORD vnum[5] = {11506, 11507, 11508, 11509, 11510};
...
static void FN_spawn_tower(int empire, LPSECTREE_MAP sectree_map)
{
	DO_ALL_TOWER(i)
	{
		...
		GET_TOWER(empire, i) = CHARACTER_MANAGER::instance().SpawnMobRange(FN_random_tower(), FN_castle_map_index(empire), sx, sy, ex, ey);
...
bool castle_spawn_tower(int empire, int tower_count)
{
	...
	for (int j = 0; j < spawn_count; ++j)
	{
		...
		{
		char buf[1024];
		snprintf(buf, sizeof(buf), LC_TEXT("%sstart_war_types"), EMPIRE_NAME(empire));
		BroadcastNotice(buf);
	}
	return true;
}
...
void castle_tower_die(LPCHARACTER ch, LPCHARACTER killer)
{
	...
	switch (GET_SIEGE_STATE())
	{
		...
		case CASTLE_SIEGE_END:
			{
				int	siege_end = true;
				snprintf(buf, sizeof(buf), LC_TEXT("%syou_must_destroy_all_tower"), EMPIRE_NAME(killer_empire));
				BroadcastNotice(buf);
				...
				if (siege_end)
				{
					...
					{
						snprintf(buf, sizeof(buf), LC_TEXT("%syou_loase_war"), EMPIRE_NAME(GET_SIEGE_EMPIRE()));
						BroadcastNotice(buf);
					}
					else
					{
						snprintf(buf, sizeof(buf), LC_TEXT("%sdestroy_all"), EMPIRE_NAME(GET_SIEGE_EMPIRE()));
						BroadcastNotice(buf);
					}
					castle_end_siege();
				}

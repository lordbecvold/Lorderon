...
void CWarMap::IncMember(LPCHARACTER ch)
{
	...
	if (isWarMember)
	{
		...
		event_cancel(&m_pkTimeoutEvent);
		sys_log(0, "WarMap +m %u(cur:%d, acc:%d) vs %u(cur:%d, acc:%d)", m_TeamData[0].dwID, m_TeamData[0].GetCurJointerCount(), m_TeamData[0].GetAccumulatedJoinerCount(), m_TeamData[1].dwID, m_TeamData[1].GetCurJointerCount(), m_TeamData[1].GetAccumulatedJoinerCount());
	}
	else
	{
		++m_iObserverCount;
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("war_map_msg_1"));
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("war_map_msg_2"));
	}
	UpdateUserCount();
...
void CWarMap::CheckWarEnd()
{
	...
	if (m_TeamData[0].iMemberCount == 0 || m_TeamData[1].iMemberCount == 0)
	{
		...
		if (m_pkTimeoutEvent)
			return;
		Notice(LC_TEXT("war_map_msg_3"));
		Notice(LC_TEXT("war_map_msg_4"));
...
void CWarMap::Timeout()
{
	...
	else
	{
		...
		if (iWinnerIdx == -1)
		{
			...
			sys_err("WarMap: member count is not zero, guild1 %u %d guild2 %u %d, winner %u", m_TeamData[0].dwID, m_TeamData[0].iMemberCount, m_TeamData[1].dwID, m_TeamData[1].iMemberCount, dwWinner);
		}
		else
		{
	...
	sys_log(0, "WarMap: Timeout %u %u winner %u loser %u reward %d map %d", m_TeamData[0].dwID, m_TeamData[1].dwID, dwWinner, dwLoser, iRewardGold, m_kMapInfo.lMapIndex);
	if (dwWinner)
...
bool CWarMap::CheckScore()
{
	...
	if (dwWinner == m_TeamData[0].dwID)
		...
		iRewardGold = GetRewardGold(1);
	sys_log(0, "WarMap::CheckScore end score %d guild1 %u score guild2 %d %u score %d winner %u reward %d", iEndScore, m_TeamData[0].dwID, m_TeamData[0].iScore, m_TeamData[1].dwID, m_TeamData[1].iScore, dwWinner, iRewardGold);
	CGuildManager::instance().RequestWarOver(dwWinner, dwLoser, dwWinner, iRewardGold);
	return true;
}

...
bool CHARACTER::StartRiding()
{
	...
	if (IsDead() == true)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("char_horse_msg_1"));
		return false;
	}
	if (IsPolymorphed())
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("char_horse_msg_2"));
		return false;
	}
#if defined(WJ_COMBAT_ZONE)	
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("char_horse_msg_3"));
		return false;
	}
	...
	{
		if (GetHorseLevel() <= 0)
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("char_horse_msg_4"));
		else if (GetHorseHealth() <= 0)
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("char_horse_msg_5"));
		else if (GetHorseStamina() <= 0)
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("char_horse_msg_6"));
		return false;
	}
	HorseSummon(false);
...
void CHARACTER::HorseSummon(bool bSummon, bool bFromFar, DWORD dwVnum, const char* pPetName)
{
	if ( bSummon )
	{
		...
		if (!m_chHorse)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("char_horse_msg_7"));
			return;
		}
		if (GetHorseHealth() <= 0)
		{
		...
		else
		{
			...
			m_chHorse->m_stName += LC_TEXT("char_horse_msg_8");
		}
...
void CHARACTER::SendHorseInfo()
{
	if (m_chHorse || IsHorseRiding())
	{
		...
		if (GetHorseStamina() * 10 <= GetHorseMaxStamina())
			...
			iStaminaGrade = 3;
		if (m_bSendHorseLevel != GetHorseLevel() || m_bSendHorseHealthGrade != iHealthGrade || m_bSendHorseStaminaGrade != iStaminaGrade)
		{

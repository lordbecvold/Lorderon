...
void DESC::DisconnectOfSameLogin()
{
	if (GetCharacter())
	{
		if (m_pkDisconnectEvent)
			return;
		GetCharacter()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("multi_account_msg"));
		DelayedDisconnect(5);
	}
	else
	{
...
void DESC::SetSecurityKey(const DWORD * c_pdwKey)
{
	...
	sys_log(0, "SetSecurityKey decrypt %u %u %u %u encrypt %u %u %u %u", m_adwDecryptionKey[0], m_adwDecryptionKey[1], m_adwDecryptionKey[2], m_adwDecryptionKey[3], m_adwEncryptionKey[0], m_adwEncryptionKey[1], m_adwEncryptionKey[2], m_adwEncryptionKey[3]);
}
#endif 

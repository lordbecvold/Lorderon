	...
	if ((c = GetPlayerCache(packet->player_id)))
	{
		...
		else
		{
			snprintf(szQuery, sizeof(szQuery),
				...
				"FROM item%s WHERE owner_id=%d AND (window in ('INVENTORY','EQUIPMENT','DRAGON_SOUL_INVENTORY','BELT_INVENTORY','UPGRADE_INVENTORY','BOOK_INVENTORY','STONE_INVENTORY'))", GetTablePostfix(), pTab->id);
...
void CClientManager::QUERY_ADD_AFFECT(CPeer * peer, TPacketGDAddAffect * p)
{
	...
	snprintf(queryStr, sizeof(queryStr),
			...
			"VALUES(%u, %u, %u, %ld, %u, %ld, %ld)", GetTablePostfix(), p->dwPID, p->elem.dwType, p->elem.bApplyOn, p->elem.lApplyValue, p->elem.dwFlag, p->elem.lDuration, p->elem.lSPCost);
	CDBManager::instance().AsyncQuery(queryStr);
}

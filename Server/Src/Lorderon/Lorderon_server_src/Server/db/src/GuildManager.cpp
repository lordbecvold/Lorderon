...
bool CGuildManager::IsHalfWinLadderPoint(DWORD dwGuildWinner, DWORD dwGuildLoser)
{
	...
	if (it != m_mapGuildWarEndTime[GID1].end() && it->second + GUILD_WAR_LADDER_HALF_PENALTY_TIME > CClientManager::instance().GetCurrentTime())
		return true;
	return false;
}
...
bool CGuildManager::TakeBetPrice(DWORD dwGuildTo, DWORD dwGuildFrom, long lWarPrice)
{
	...
	{
		sys_log(0, "TakeBetPrice: not enough gold %u %d to %u %d", dwGuildFrom, it_from->second.gold, dwGuildTo, it_to->second.gold);
		return false;
	}
...
bool CGuildManager::WaitStart(TPacketGuildWar * p)
{
	...
	sys_log(0, "GuildWar: WaitStart g1 %d g2 %d price %d start at %u", p->dwGuildFrom, p->dwGuildTo, p->lWarPrice, dwCurTime + GetGuildWarWaitStartDuration());
	return true;
}
...
bool CGuildManager::ReserveWar(TPacketGuildWar * p)
{
	...
	char szQuery[512];
	snprintf(szQuery, sizeof(szQuery), "INSERT INTO guild_war_reservation (guild1, guild2, time, type, warprice, initscore, power1, power2, handicap) " "VALUES(%u, %u, DATE_ADD(NOW(), INTERVAL 180 SECOND), %u, %ld, %ld, %ld, %ld, %ld)", GID1, GID2, p->bType, p->lWarPrice, p->lInitialScore, t.lPowerFrom, t.lPowerTo, t.lHandicap);
	std::auto_ptr<SQLMsg> pmsg(CDBManager::instance().DirectQuery(szQuery));
...
bool CGuildManager::Bet(DWORD dwID, const char * c_pszLogin, DWORD dwGold, DWORD dwGuild)
{
	...
	if (it == m_map_kWarReserve.end())
	{
		...
		snprintf(szQuery, sizeof(szQuery), "INSERT INTO item_award (login, vnum, socket0, given_time) VALUES('%s', %d, %u, NOW())", c_pszLogin, ITEM_ELK_VNUM, dwGold);
		CDBManager::instance().AsyncQuery(szQuery);
		return false;
	}
...
bool CGuildWarReserve::Bet(const char * pszLogin, DWORD dwGold, DWORD dwGuild)
{
	...
	snprintf(szQuery, sizeof(szQuery), "INSERT INTO guild_war_bet (war_id, login, gold, guild) VALUES(%u, '%s', %u, %u)", m_data.dwID, pszLogin, dwGold, dwGuild);
	std::auto_ptr<SQLMsg> pmsg(CDBManager::instance().DirectQuery(szQuery));
	...
	snprintf(szQuery, sizeof(szQuery), "UPDATE guild_war_reservation SET bet_from=%u,bet_to=%u WHERE id=%u", m_data.dwBetFrom, m_data.dwBetTo, m_data.dwID);
	CDBManager::instance().AsyncQuery(szQuery);
...
void CGuildWarReserve::End(int iScoreFrom, int iScoreTo)
{
	...
	while (1)
	{
		...
		while (it != mapBet.end())
		{
			...
			if (iRow == 0)
				iLen += snprintf(szQuery + iLen, sizeof(szQuery) - iLen, "('%s', %d, %u, NOW())", it->first.c_str(), ITEM_ELK_VNUM, dwGold);
			else
				iLen += snprintf(szQuery + iLen, sizeof(szQuery) - iLen, ",('%s', %d, %u, NOW())", it->first.c_str(), ITEM_ELK_VNUM, dwGold);
			++it;
			if (iLen > 384)

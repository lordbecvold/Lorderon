...
bool CMonarch::VoteMonarch(DWORD pid, DWORD selectdpid)
{
	...
	if (it == m_map_MonarchElection.end())
	{
		...
		char szQuery[256];
		snprintf(szQuery, sizeof(szQuery), "INSERT INTO monarch_election(pid, selectedpid, electiondata) VALUES(%d, %d, now())", pid, selectdpid);
...
bool CMonarch::AddCandidacy(DWORD pid, const char * name)
{
	...
	char szQuery[256];
	snprintf(szQuery, sizeof(szQuery), "INSERT INTO monarch_candidacy(pid, date) VALUES(%d, now())", pid);
	CDBManager::instance().AsyncQuery(szQuery);
	return true;
}
...
bool CMonarch::DelCandidacy(const char * name)
{
	...
	for (; it != m_vec_MonarchCandidacy.end(); ++it)
	{
		...
		{
			char szQuery[256];
			snprintf(szQuery, sizeof(szQuery), "DELETE FROM monarch_candidacy WHERE pid=%d ", it->pid);
			CDBManager::instance().AsyncQuery(szQuery);
...
bool CMonarch::TakeMoney(int Empire, DWORD pid, int64_t Money)
{
	...
	m_MonarchInfo.money[Empire] -= Money;
	char szQuery[1024];
	snprintf(szQuery, sizeof(szQuery), "UPDATE monarch SET money=%lld WHERE empire=%d", m_MonarchInfo.money[Empire], Empire);
	CDBManager::instance().AsyncQuery(szQuery);
	if (g_test_server)
...
bool CMonarch::SetMonarch(const char * name)
{
    ...
    delete pMsg;
	snprintf(szQuery, sizeof(szQuery), "REPLACE INTO monarch (empire, name, windate, money) VALUES(%d, %d, now(), %lld)", Empire, p->pid[Empire], p->money[Empire]);
